...
public class MVConv extends MVUtil {
  ...
  public static final String _strDBPassword = "pgoldenb";
  public static final Hashtable _tableStatHeaders = new Hashtable();
  public static final DecimalFormat _formatPerc = new DecimalFormat("0.000");
  public static final Hashtable _tableStatNc = new Hashtable();
  public static final Hashtable _tableStatBc = new Hashtable();
  ...
  static {
    for (String _listLineDataTable : _listLineDataTables) {
      _tableLineCounts.put(_listLineDataTable, new Integer(0));
    }
  ...
  public static void main(String[] argv) {
    ...
    try {
      ...
      if (_conSrc.isClosed()) {
        throw new Exception("failed to connect to source database " + _strSrc);
      }
      ...
      if (_conDest.isClosed()) {
        throw new Exception("failed to connect to destination database " + _strDest);
      }
      ...
      while (resId.next()) {
        ...
        for (String _listLineDataTable : _listLineDataTables) {
          copyTableRecord(_listLineDataTable, sh);
        }
      ...
      for (String _listLineDataTable : _listLineDataTables) {
        ...
        intNumSrc = getTableSize(_conSrc, _listLineDataTable);
  ...
  /**
   * Read the stat_header record from the src database with the input stat_header_id and either build or retrieve the stat_header record in the destination
   * database.  Create a data structure that contains the time information from the src stat_header record and return it.
   *
  ...
  public static MVConvStatHeader convertStatHeader(int intStatId) throws Exception {
    ...
    if (!resStat.next()) {
      throw new Exception("src stat_header select failed for stat_header_id " + intStatId);
    }
    ...
    if (!boolStatHeaderExists) {
      ...
      if (1 != intStatIns) {
        throw new Exception("unexpected number of records inserted into dest stat_header: " + intStatIns);
      }
      stmtStatIns.close();
    }
  ...
  /**
   * Create and execute a SELECT COUNT(*) for the input table against the input database connection.  If no result is found, -1 is returned.  Otherwise, the
   * size of the table is returned.
   *
  ...
  /**
   * Query the source database for a stat_group with the input stat_header_id and stat_group_lu_id.  Build a string of values intended for an insert statement
   * for the stat, including CI information, if appropriate.
   *
  ...
  /**
   * Build tables of stat_group_lu_id / CI usage pairs for normal and bootstrap CIs.  The information to build the tables is gathered from the source database
   * stat_group_lu table and stored in the _tableStatNc and _tableStatBc data members.
   *
   * @throws Exception
   */
  ...
  /**
   * Copy all line_data tables which point at the input source stat_header_id to the destination database. The records in the destination database will all
   * point to the appropriate stat_header record there.  Variable length records will also be copied.
   *
  ...
  public static void copyTableRecord(String strTable, MVConvStatHeader sh) throws Exception {
    ...
    while (resStat.next()) {
      ...
      int intFieldIndex = 6;
      if (_tableLineCovThresh.containsKey(strTable)) {
        strValueList += ", '" + resStat.getString(intFieldIndex++) + "'";
      }
      if (_tableLineAlpha.containsKey(strTable)) {
        strValueList += ", " + resStat.getString(intFieldIndex++);
      }
      ...
      if (!strTable.equals("line_data_cnt") && !strTable.equals("line_data_rhist")) {
        ...
        for (int i = intFieldIndex; i <= intNumFields; i++) {
          strValueList += ", '" + resStat.getString(i) + "'";
        }
      }
      ...
      if (_tableLineStats.containsKey(strTable)) {
        ...
        if (strTable.equals("line_data_rhist")) {
          strValueList += ", " + resStat.getString(6);
        }
      }
      ...
      if (1 != intDestRes) {
        throw new Exception("unexpected result from " + strTable + " insert: " + intDestRes);
      }
      stmtDest.close();
