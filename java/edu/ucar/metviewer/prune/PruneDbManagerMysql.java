/**
 * PruneDbManagerMysql.java Copyright UCAR (c) 2019. University Corporation for Atmospheric Research
...
/**
 * @author : tatiana $
 * @version : 1.0 : 2019-02-08 09:15 $
 */
public class PruneDbManagerMysql extends PruneDbManager {

  private static final Logger logger = LogManager.getLogger("PruneDbManagerMysql");
  private final List<Table> tables;

  public PruneDbManagerMysql(final MysqlDatabaseManager databaseManager) throws SQLException {
  ...
  public void pruneData(MVPruneDB mvPruneDB) {
    ...
    try (Connection con = databaseManager.getConnection(mvPruneDB.getDatabaseName())) {
  ...
  /**
   * Finds all ID in the header table that don't have foreign keys equivalents in the dependant
   * tables
   *
   * @param con
   * @param type
  ...
  /**
   * removes all records fro the header tables that  don't have foreign keys equivalents in the
   * dependant tables
   *
   * @param con
   * @param type
