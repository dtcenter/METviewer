/**
 * MariaDbLoadDatabaseManager.java Copyright UCAR (c) 2017. University
 * Corporation
 * for Atmospheric
 ...
 */
public class MariaDbLoadDatabaseManager extends edu.ucar.metviewer.db.mariadb.MariaDbDatabaseManager implements edu.ucar.metviewer.db.LoadDatabaseManager {

  protected static final java.time.format.DateTimeFormatter DB_DATE_STAT_FORMAT
      = java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");
  private static final org.apache.logging.log4j.Logger logger =
          org.apache.logging.log4j.LogManager.getLogger(
                  "MariaDbLoadDatabaseManager");
  ...
  private final java.util.regex.Pattern patIndexName = java.util.regex.Pattern.compile("#([\\w\\d]+)#([\\w\\d]+)");
  private final java.util.Map<String, Integer> tableVarLengthLineDataId = new java.util.HashMap<>();
  private final java.util.Map<String, Integer> statHeaders = new java.util.HashMap<>();
  private final java.util.Map<String, Integer> modeHeaders = new java.util.HashMap<>();
  private final java.util.Map<String, Integer> mtdHeaders = new java.util.HashMap<>();
   ...
   */
  private final java.util.Map<String, String> tableVarLengthTable;
  /*
   * data_file_lu_id values for each MET output type
   */
  private final java.util.Map<String, Integer> tableDataFileLU;
  private edu.ucar.metviewer.MVOrderedMap mapIndexes;

  public MariaDbLoadDatabaseManager(edu.ucar.metviewer.db.DatabaseInfo databaseInfo) throws Exception {
    super(databaseInfo);
    mapIndexes = new edu.ucar.metviewer.MVOrderedMap();
    ...
    tableVarLengthTable = new java.util.HashMap<>();
    ...
    tableDataFileLU = new java.util.HashMap<>();
  ...
  private void applyIndexes(boolean drop) throws Exception {
    ...
    logger.info("    ==== indexes ====\n" + (drop ? "  dropping..." : ""));
    java.util.Map.Entry[] listIndexes = mapIndexes.getOrderedEntries();
    for (java.util.Map.Entry listIndex : listIndexes) {
      ...
      java.util.regex.Matcher matIndex = patIndexName.matcher(strIndexKey);
      if (!matIndex.matches()) {
      ...
      logger.info(edu.ucar.metviewer.MVUtil.padBegin(strIndexName + ": ", 36) + edu.ucar.metviewer.MVUtil.formatTimeSpan(intIndexTime));
    }
    logger.info("");
  }
  ...
  private void initVarLengthLineDataIds() throws Exception {
    ...
    java.util.Set<String> lineTypes = tableVarLengthTable.keySet();
   ...
   */
  @Override
  public java.util.Map<String, Long> loadStatFile(final edu.ucar.metviewer.DataFileInfo info) throws Exception {
    java.util.Map<String, Long> timeStats = new java.util.HashMap<>();
    //  initialize the insert data structure
    edu.ucar.metviewer.MVLoadStatInsertData insertData = new edu.ucar.metviewer.MVLoadStatInsertData();
    ...
    try (
      ...
      while (reader.ready()) {
        ...
        if (1 > listToken.length || listToken[0].equals("VERSION")) {
          headerNames = java.util.Arrays.asList(listToken);
          continue;
        }


        //  if the line type load selector is activated, check that the current line type is on the list
        insertData.setLineType(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "LINE_TYPE"));
        if (!edu.ucar.metviewer.MVUtil.isValidLineType(insertData.getLineType())) {
          logger.warn(
        ...
        java.time.LocalDateTime fcstValidBeg = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VALID_BEG"),
            DB_DATE_STAT_FORMAT);


        java.time.LocalDateTime fcstValidEnd = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VALID_END"),
            DB_DATE_STAT_FORMAT);


        java.time.LocalDateTime obsValidBeg = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VALID_BEG"),
            DB_DATE_STAT_FORMAT);


        java.time.LocalDateTime obsValidEnd = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VALID_END"),
            DB_DATE_STAT_FORMAT);
        ...
        String fcstLeadStr = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEAD");
        ...
        java.time.LocalDateTime fcstInitBeg = java.time.LocalDateTime.from(fcstValidBeg);
        ...
        String strInterpPnts = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTERP_PNTS");
        ...
        java.util.List<Object> statHeaderValueList = new java.util.ArrayList<>();
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "VERSION"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "MODEL"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "DESC"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VAR"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEV"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VAR"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEV"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBTYPE"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "VX_MASK"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTERP_MTHD"));
        statHeaderValueList.add(strInterpPnts);
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_THRESH"));
        statHeaderValueList.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_THRESH"));


        //  build a where clause for searching for duplicate stat_header records
        String statHeaderWhere = BINARY + "  model = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "MODEL")
                                     + "' AND " + BINARY + "descr = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "DESC")
                                     + "'  AND " + BINARY + "fcst_var = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VAR")
                                     + "'  AND " + BINARY + "fcst_lev = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEV")
                                     + "'  AND " + BINARY + "obtype = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBTYPE")
                                     + "'  AND " + BINARY + "vx_mask = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "VX_MASK")
                                     + "'  AND " + BINARY + "interp_mthd = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTERP_MTHD")
                                     + "'  AND interp_pnts = " + strInterpPnts
                                     + "  AND " + BINARY + "fcst_thresh = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_THRESH")
                                     + "'  AND " + BINARY + "obs_thresh = '"
                                     + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_THRESH")
                                     + "'";
        ...
        for (int i = 0; i < statHeaderValueList.size(); i++) {
          ...
          if (edu.ucar.metviewer.MVUtil.isNumeric(value)) {
            csvBuilder.append(value);
          } else {
            csvBuilder.append("'").append(value).append("'");
          }
          if (i != statHeaderValueList.size() - 1) {
            csvBuilder.append(edu.ucar.metviewer.MVUtil.SEPARATOR);
          }
        }
        ...
        if (insertData.getLineType().equals("RHIST")) {
          ...
          boolean isInt = edu.ucar.metviewer.MVUtil.isInteger(listToken[indexOfNrank], 10);
          isMet8 = isInt
        ...
        if (statHeaderId != null) {
          ...
          boolean hasVarLengthGroups = edu.ucar.metviewer.MVUtil.lengthGroupIndices
          ...
          if (hasVarLengthGroups) {
            ...
            int[] listVarLengthGroupIndices1 = edu.ucar.metviewer.MVUtil.lengthGroupIndices
                                                   .get(insertData.getLineType());
            int[] listVarLengthGroupIndices = java.util.Arrays.copyOf(listVarLengthGroupIndices1,
          ...
          java.util.List<Object> lineDataValues = new java.util.ArrayList<>();
          ...
          lineDataValues.add(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEAD"));
          lineDataValues.add(obsValidBegStr);
          lineDataValues.add(obsValidEndStr);

          //  if the line data requires a cov_thresh value, add it
          String covThresh = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "COV_THRESH");
          if (edu.ucar.metviewer.MVUtil.covThreshLineTypes.containsKey(insertData.getLineType())) {
            lineDataValues.add(replaceInvalidValues(covThresh));
          }

          //  if the line data requires an alpha value, add it
          String alpha = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "ALPHA");
          if (edu.ucar.metviewer.MVUtil.alphaLineTypes.containsKey(insertData.getLineType())) {
            if (alpha.equals("NA")) {
          ...
          if (lineType.equals("RHIST")) {
            ...
            if (!isMet8) {
              ...
              java.util.List<Object> ecntLineDataValues = new java.util.ArrayList<>(lineDataValues);
              ...
              if (!insertData.getTableLineDataValues().containsKey("ECNT")) {
                insertData.getTableLineDataValues().put("ECNT", new java.util.ArrayList<>());
              }
          ...
          if (lineType.equals("ORANK")) {
            ...
            int[] varLengthGroupIndices1 = edu.ucar.metviewer.MVUtil.lengthGroupIndices.get(insertData.getLineType());
            int[] varLengthGroupIndices = java.util.Arrays.copyOf(varLengthGroupIndices1,
          ...
          if (!insertData.getTableLineDataValues().containsKey(insertData.getLineType())) {
            insertData.getTableLineDataValues().put(insertData.getLineType(), new java.util.ArrayList<>());
          }
          ...
          if (hasVarLengthGroups) {
            ...
            java.util.List<String> threshValues = insertData.getTableVarLengthValues()
                                            .get(insertData.getLineType());
            if (null == threshValues) {
              threshValues = new java.util.ArrayList<>();
            }
    ...
    if (info.verbose) {
      logger.info(edu.ucar.metviewer.MVUtil.padBegin("file lines: ", 36) + (intLine - 1) + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("stat_header records: ", 36) + headerRecords + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("stat_header inserts: ", 36) + headerInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("line_data records: ", 36) + dataRecords + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("line_data inserts: ", 36) + dataInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("line_data skipped: ", 36) + intLineDataSkipped + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("var length records: ", 36) + lengthRecords + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("var length inserts: ", 36) + lengthInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("total load time: ", 36) + edu.ucar.metviewer.MVUtil.formatTimeSpan(
          statHeaderLoadTime) + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("stat_header search time: ", 36) + edu.ucar.metviewer.MVUtil.formatTimeSpan(
          headerSearchTime) + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("lines / msec: ", 36) + edu.ucar.metviewer.MVUtil.formatPerf.format(
          dblLinesPerMSec) + "\n\n");
    }
    return timeStats;
  }
   ...
   * @throws Exception
   */
  private int[] commitStatData(edu.ucar.metviewer.MVLoadStatInsertData mvLoadStatInsertData)
      throws Exception {
    ...
    for (java.util.Map.Entry<String, java.util.List<String>> entry : mvLoadStatInsertData.getTableLineDataValues()
                                                     .entrySet()) {
      mvLoadStatInsertData.setLineType(entry.getKey());
      java.util.ArrayList listValues = (java.util.ArrayList) entry.getValue();
    ...
    java.util.Set<String> strings = mvLoadStatInsertData.getTableVarLengthValues().keySet();
    String[] varLengthTypes = strings.toArray(new String[strings.size()]);


    for (String listVarLengthType : varLengthTypes) {
      String[] listVarLengthValues = edu.ucar.metviewer.MVUtil.toArray(
      ...
      mvLoadStatInsertData.getTableVarLengthValues().put(listVarLengthType, new java.util.ArrayList<>());
    }
  ...
  @Override
  public java.util.Map<String, Long> loadStatFileVSDB(edu.ucar.metviewer.DataFileInfo info) throws Exception {

    java.util.Map<String, Long> timeStats = new java.util.HashMap<>();

    //  initialize the insert data structure
    edu.ucar.metviewer.MVLoadStatInsertData mvLoadStatInsertData = new edu.ucar.metviewer.MVLoadStatInsertData();
    ...
    try (java.io.FileReader fileReader = new java.io.FileReader(
        strFilename); java.io.BufferedReader reader = new java.io.BufferedReader(fileReader)) {
      java.util.List<String> allMatches;

      java.time.format.DateTimeFormatter formatStatVsdb = java.time.format.DateTimeFormatter.ofPattern("yyyyMMddHH");
      ...
      while (reader.ready()) {
        ...
        try {
          ...
          java.util.regex.Matcher m = java.util.regex.Pattern.compile("\\d-0\\.").matcher(
          ...
          java.time.LocalDateTime fcstValidBeg = java.time.LocalDateTime.parse(listToken[3], formatStatVsdb);
          ...
          if (statHeaderId != null) {
            ...
            if (edu.ucar.metviewer.MVUtil.covThreshLineTypes.containsKey(mvLoadStatInsertData.getLineType())) {
              lineDataValueList += ", '" + replaceInvalidValues(strCovThresh) + "'";
            }

            //  if the line data requires an alpha value, add it
            String alpha = "-9999";
            if (edu.ucar.metviewer.MVUtil.alphaLineTypes.containsKey(mvLoadStatInsertData.getLineType())) {
              if (alpha.equals("NA")) {
            ...
            //  add the values list to the line type values map
            java.util.List<String> listLineTypeValues = new java.util.ArrayList<>();
            ...
            if (hasVarLengthGroups) {
              ...
              java.util.List<String> listThreshValues = mvLoadStatInsertData.getTableVarLengthValues()
                                                  .get(mvLoadStatInsertData.getLineType());
              if (null == listThreshValues) {
                listThreshValues = new java.util.ArrayList<>();
              }
    ...
    if (info.verbose) {
      logger.info(edu.ucar.metviewer.MVUtil.padBegin("file lines: ", 6) + (intLine - 1) + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("stat_header records: ", 36) + headerRecords + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("stat_header inserts: ", 36) + headerInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("line_data records: ", 36) + dataRecords + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("line_data inserts: ", 36) + dataInserts + "\n" +
   ...
   */
  @Override
  public java.util.Map<String, Long> loadModeFile(edu.ucar.metviewer.DataFileInfo info) throws Exception {
    java.util.Map<String, Long> timeStats = new java.util.HashMap<>();

    //  data structure for storing mode object ids
    java.util.Map<String, Integer> tableModeObjectId = new java.util.HashMap<>();
    ...
    java.util.List<String> headerNames = new java.util.ArrayList<>();
    try (
            java.io.FileReader fileReader = new java.io.FileReader(strFilename);
            java.io.BufferedReader reader = new java.io.BufferedReader(fileReader)) {
      ...
      while (reader.ready()) {
        ...
        String objectId = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBJECT_ID");
        java.util.regex.Matcher matModeSingle = edu.ucar.metviewer.MVUtil.patModeSingleObjectId.matcher(objectId);
        java.util.regex.Matcher matModePair = edu.ucar.metviewer.MVUtil.patModePairObjectId.matcher(objectId);
        int modeCts = 19;
        int modeSingle = 17;
        int modePair = 18;
        if (2 == dataFileLuId) {
          ...
          throw new Exception("METviewer load error: loadModeFile() unable to determine line type "
                                  + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBJECT_ID")
                                  + "\n        " + strFileLine);
        }
        ...
        java.time.LocalDateTime fcstValidBeg = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VALID"),
            DB_DATE_STAT_FORMAT);


        java.time.LocalDateTime obsValidBeg = java.time.LocalDateTime.parse(
            edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VALID"),
            DB_DATE_STAT_FORMAT);
        ...
        //  calculate the number of seconds corresponding to fcst_lead
        String strFcstLead = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEAD");
        ...
        String modeHeaderValueList =
            "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "VERSION")
                + "', " + "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "MODEL") + "', "
                + "";

        if ("NA".equals(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "N_VALID"))) {
          modeHeaderValueList = modeHeaderValueList + "NULL" + ", ";      //  N_VALID
        } else {
          modeHeaderValueList = modeHeaderValueList
                                    + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "N_VALID")
                                    + ", ";      //  N_VALID
        }
        if ("NA".equals(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "GRID_RES"))) {
          modeHeaderValueList = modeHeaderValueList + "NULL" + ", ";      //  GRID_RES
        } else {
          modeHeaderValueList = modeHeaderValueList
                                    + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "GRID_RES")
                                    + ", ";      //  GRID_RES
        }
        ...
        modeHeaderValueList = modeHeaderValueList
                                  + "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "DESC")
                                  + "', " +      //  GRID_RES
                                  "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
                                                         "FCST_LEAD")
                                  + "', " +      //  fcst_lead
                                  "'" + fcstValidBegStr + "', ";      //  fcst_valid
        if ("NA".equals(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_ACCUM"))) {
          modeHeaderValueList = modeHeaderValueList + "NULL" + ", ";      //  fcst_accum
        } else {
          modeHeaderValueList = modeHeaderValueList
                                    + "'"
                                    + edu.ucar.metviewer.MVUtil
        ...
        modeHeaderValueList = modeHeaderValueList + "'" + fcstInitStr + "', "
                                  +  // fcst_init
                                  "'"
                                  + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEAD")
                                  + "', " +      //  obs_lead
                                  "'" + obsValidBegStr + "', ";      //  obs_valid
        if ("NA".equals(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_ACCUM"))) {
        ...
        modeHeaderValueList = modeHeaderValueList
                                  + "'"
                                  + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_RAD")
                                  + "', " +      //  fcst_rad
                                  "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
                                                         "FCST_THR") + "', " +      //  fcst_thr
                                  "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
        ...
        String headerWhere = BINARY +
                                 "  version = '" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
                                                                    "VERSION") + "'\n" +
                                 "  AND model = '" + edu.ucar.metviewer.MVUtil
        ...
        headerWhere = headerWhere
                          ...
                          + "'\n"
                          + "  AND fcst_lead = "
                          + Integer.valueOf(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEAD"))
                          + "\n" +
        ...
        Integer accum = null;
        try {
          accum = Integer.valueOf(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_ACCUM"));
        } catch (Exception e) {
        }
        if (accum == null) {
        ...
        headerWhere = headerWhere + "\n"
                          ...
                          + "  AND obs_lead = "
                          + Integer.valueOf(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEAD"))
                          + "\n"
                          + "  AND obs_valid = '" + obsValidBegStr + "'\n"
                          + "  AND obs_accum = ";

        try {
          accum = Integer.valueOf(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_ACCUM"));
        } catch (Exception e) {
        }
        if (accum == null) {
          headerWhere = headerWhere + "NULL";
        ...
        headerWhere = headerWhere
                          ...
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
                                             "FCST_THR") + "'\n"
                          + "  AND obs_rad = "
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_RAD") + "\n"
                          + "  AND " + BINARY + "obs_thr = '"
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_THR")
                          + "'\n"
                          + "  AND " + BINARY + "fcst_var = '"
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_VAR")
                          + "'\n"
                          ...
                          + "'\n"
                          + "  AND " + BINARY + "obs_var = '"
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VAR")
                          + "'\n"
                          + "  AND " + BINARY + "obs_lev = '"
                          + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames,
                                             "OBS_LEV") + "'";
          ...
          if (info.modeHeaderDBCheck) {
            ...
            try (java.sql.Connection con = getConnection();
                 java.sql.Statement stmt = con.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,
                                                      java.sql.ResultSet.CONCUR_READ_ONLY);
                 java.sql.ResultSet res = stmt.executeQuery(modeHeaderSelect)) {
              if (res.next()) {
        ...
        if (modeCts == lineTypeLuId) {
          ...
          String ctsValueList = modeHeaderId + ", '"
                                    + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FIELD")
                                    + "'";
         ...
         */
          ...
          for (String header : modeObjSingleColumns) {
            singleValueList += ", '" + replaceInvalidValues(
                edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, header)) + "'";
          }
          ...
          int matchedFlag = 0;
          String[] objCatArr = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBJECT_CAT")
                                   .split("_");
         ...
         */
          ...
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "CENTROID_DIST"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "BOUNDARY_DIST"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "CONVEX_HULL_DIST"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "ANGLE_DIFF"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "ASPECT_DIFF"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "AREA_RATIO"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTERSECTION_AREA"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "UNION_AREA"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "SYMMETRIC_DIFF"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTERSECTION_OVER_AREA"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "CURVATURE_RATIO"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "COMPLEXITY_RATIO"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "PERCENTILE_INTENSITY_RATIO"));
          pairValueList += ", " + replaceInvalidValues(
              edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "INTEREST"));
    ...
    if (info.verbose) {
      ...
      logger.info(edu.ucar.metviewer.MVUtil.padBegin("mode_header inserts: ", 36) + headerInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("mode_cts inserts: ", 36) + ctsInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("mode_obj_single inserts: ", 36) + objSingleInserts + "\n" +
                      edu.ucar.metviewer.MVUtil.padBegin("mode_obj_pair inserts: ", 36) + objPairInserts + "\n" +
                      (info.modeHeaderDBCheck ? edu.ucar.metviewer.MVUtil.padBegin("mode_header search time: ",
                                                                36) + edu.ucar.metviewer.MVUtil.formatTimeSpan(
                          timeStats.get("headerSearchTime")) + "\n" : "") +
                      edu.ucar.metviewer.MVUtil.padBegin("total load time: ", 36) + edu.ucar.metviewer.MVUtil.formatTimeSpan(
   ...
   */
  @Override
  public java.util.Map<String, Long> loadMtdFile(edu.ucar.metviewer.DataFileInfo info) throws Exception {
    java.util.Map<String, Long> timeStats = new java.util.HashMap<>();
    ...
    try (
      ...
      while (reader.ready()) {
        ...
        //  calculate the number of seconds corresponding to fcst_lead
        String fcstLead = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEAD");
        ...
        String fcstLeadInsert = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_LEAD");
        ...
        String obsLeadInsert = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEAD");
        ...
        String mtdHeaderValueList = "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "VERSION")
                                        + "', " + "'"
                                        + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "MODEL")
                                        + "', " + "'"
                                        + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "DESC")
        ...
        if ("NA".equals(edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "T_DELTA"))) {
          mtdHeaderValueList = mtdHeaderValueList + "NULL" + ", ";
        } else {
          mtdHeaderValueList = mtdHeaderValueList + "'"
                                   + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "T_DELTA")
                                   + "', ";
        }
        mtdHeaderValueList = mtdHeaderValueList
                                 + "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_RAD")
                                 + "', " +
                                 "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "FCST_THR")
                                 + "', " +
                                 "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_RAD")
                                 + "', " +
                                 "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_THR")
                                 ...
                                 "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_VAR")
                                 + "', " +
                                 "'" + edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBS_LEV")
                                 + "'";
        ...
        //  if the mtd_header does not yet exist, create one
          ...
          if (info.mtdHeaderDBCheck) {
            ...
            try (java.sql.Connection con = getConnection();
                 java.sql.Statement stmt = con.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,
                                                      java.sql.ResultSet.CONCUR_READ_ONLY);
                 java.sql.ResultSet res = stmt.executeQuery(strMtdHeaderSelect)) {
              if (res.next()) {
        ...
        if (mtd3dSingle == lineTypeLuId) {
          ...
          String objCat = edu.ucar.metviewer.MVUtil.findValue(listToken, headerNames, "OBJECT_CAT");
          Integer num = null;
          try {
    ...
    if (info.verbose) {
      ...
      logger.info(
          edu.ucar.metviewer.MVUtil.padBegin("mtd_header inserts: ", 36)
              + headerInserts + "\n"
              + edu.ucar.metviewer.MVUtil.padBegin("mtd_3d_obj_single inserts: ", 36)
              + obj3dSingleInserts + "\n"
              + edu.ucar.metviewer.MVUtil.padBegin("mtd_3d_obj_pair inserts: ", 36)
              + obj3dPairInserts + "\n"
              + edu.ucar.metviewer.MVUtil.padBegin("mtd_2d_obj inserts: ", 36) + obj2dInserts + "\n"
              + (info.mtdHeaderDBCheck ? edu.ucar.metviewer.MVUtil.padBegin("mtd_header search time: ", 36)
                                             + edu.ucar.metviewer.MVUtil.formatTimeSpan(
              timeStats.get("headerSearchTime"))
                                             + "\n" : "")
              + edu.ucar.metviewer.MVUtil.padBegin("total load time: ", 36)
              + edu.ucar.metviewer.MVUtil.formatTimeSpan(intMtdHeaderLoadTime) + "\n\n");
    }
    return timeStats;
  }
  ...
  private int executeBatch(final java.util.List<String> listValues, final String table) throws Exception {
    ...
    java.util.stream.IntStream intStream = null;
    try {
      con = getConnection();
      ...
      for (int i = 0; i < listValues.size(); i++) {
        ...
        if (i != 0 && i % 20000 == 0) {
          ...
          intStream = java.util.stream.IntStream.of(updateCounts);
      ...
      int[] updateCounts = ps.executeBatch();
      intStream = java.util.stream.IntStream.of(updateCounts);
      totalInsert = totalInsert + java.util.stream.IntStream.of(updateCounts).sum();
      intStream.close();
   ...
   */
  @Override
  public edu.ucar.metviewer.DataFileInfo processDataFile(java.io.File file, boolean forceDupFile) throws Exception {
    ...
    try (
            ...
            java.sql.Statement stmt = con.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,
                                             java.sql.ResultSet.CONCUR_READ_ONLY);
            java.sql.ResultSet res = stmt.executeQuery(dataFileQuery)) {
      ...
      if (res.next()) {
        ...
        if (forceDupFile) {
          edu.ucar.metviewer.DataFileInfo info = new edu.ucar.metviewer.DataFileInfo(dataFileId,
                                               strFile
              , strPath, loadDate,
    ...
    try (
            ...
            java.sql.Statement stmt = con.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,
                                             java.sql.ResultSet.CONCUR_READ_ONLY);
            java.sql.ResultSet res = stmt.executeQuery("SELECT MAX(data_file_id) FROM data_file;")) {
    ...
    return new edu.ucar.metviewer.DataFileInfo(dataFileId, strFile, strPath,
                            loadDate,
  ...
  @Override
  public void updateInfoTable(String strXML, edu.ucar.metviewer.MVLoadJob job) throws Exception {
    //  get the instance_info information to insert
    int instInfoIdNext = getNextId("instance_info", "instance_info_id");
    String updater = "";
    try {
      updater = edu.ucar.metviewer.MVUtil.sysCmd();
    } catch (Exception e) {
      try {
        updater = edu.ucar.metviewer.MVUtil.sysCmd();
      } catch (Exception e2) {
    ...
    String updateDate = DATE_FORMAT_1.format(java.time.LocalDateTime.now());
    String updateDetail = job.getLoadNote();

    //  read the load xml into a string, if requested
    String loadXmlStr = "";
    if (job.getLoadXML()) {
      try (java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.FileReader(strXML))) {
        while (reader.ready()) {
