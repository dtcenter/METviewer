...
$("#fcst_stat_mode_"+a+"_"+b),p="",m="";if(0<d.length){m=d.split("_");for(c=0;c<m.length-1;c++)p+=m[c],c!==m.length-2&&(p+="_");m=m[m.length-1]}d=!1;l.empty();try{l.multiselect("destroy")}catch(q){}for(c=0;c<listStatMtd.length;c++)g=listStatMtd[c],(h=g===p)&&(d=!0),g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(l);l.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:225,height:300,noneSelectedText:"Select attribute stat",allUnselected:!d,click:function(a,
...
function updateStats(a,b,d){var e=$("#fcst_stat_"+a+"_"+b),c=$("#fcst_stat_mode_"+a+"_"+b),f=$("#plot_data").multiselect("getChecked").val(),g=getSelectedDatabases();if(g){try{var h=$("#fcst_var_"+a+"_"+b).multiselect("getChecked").val()}catch(p){h=$("#fcst_var_"+a+"_"+b+" option:first-child").val()}if("stat"===f){e.empty();try{c.multiselect("destroy")}catch(p){}c.css("display","none");$("#fcst_stat_mode_config_"+a+"_"+b).css("display","none");try{e.multiselect("option","noneSelectedText","Select attribute stat")}catch(p){e.prop("disabled",
"disabled")}}else if(f){try{c.multiselect("enable")}catch(p){c.prop("disabled","")}try{e.multiselect("option","noneSelectedText","Select ratio stat")}catch(p){e.prop("disabled","disabled")}}else{e.empty();try{e.multiselect("option","noneSelectedText","Select value")}catch(p){e.prop("disabled","disabled")}}if("N/A"===h)try{e.multiselect("disable")}catch(p){e.prop("disabled","disabled")}else{try{e.multiselect("enable")}catch(p){e.prop("disabled","")}h&&(h=h.replace("&","&amp;").replace(">","&gt;").replace("<",
...
c+"</db_con><list_val><"+f+"_field>"+h+"</"+f+"_field>"+g+"</list_val></request>",dataType:"xml",processData:!1,contentType:"application/xml",error:function(){},success:function(c){"y1"===a?seriesY1VarValResponse[b]=c:seriesY2VarValResponse[b]=c;var g=$(c).find("val");c=[];if(0<g.length){for(var f=0;f<g.length;f++){var h=$(g[f]);var l="string"===$.type(d)?h.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&")===d:0<=$.inArray(h.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;",
"&"),d);0===f||0!==f&&h.text()!==$(g[f-1]).text()?(l=$("<option />",{value:h.text(),text:h.text(),selected:l}),c.push(l)):0!==f&&h.text()===$(g[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}l=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(g=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,g===l?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(u){}}else{l=
$("<option />",{value:"N/A",text:"N/A"});l.appendTo(e);try{e.multiselect("refresh")}catch(u){}}1===d.length&&-1!==d[0].indexOf(",")&&$("#group_series_var_"+a+"_"+b).prop("checked",!0);d=[]}})}}
...
"y1"),0);0===n.length&&(n[0]="");if("Roc"===currentTab||"Rely"===currentTab||"Eclv"===currentTab){try{var r=$("#summary_curve").multiselect("getChecked")}catch(w){r=[]}for(k=0;k<r.length;k++)"none"!=r[k].value&&n.push(r[k].value)}for(var y=0;y<n.length;y++){r=n[y]+" "+A;var B=!1;for(k=0;k<b.length;k++)if(b[k].title==r){e={};e.title=r;e.y_axis="Y1";e.order=d+1;e.hide=b[k].hide;e.plot_ci=b[k].plot_ci;e.show_signif=b[k].show_signif;e.color=b[k].color;e.pch=b[k].pch;e.type=b[k].type;e.lty=b[k].lty;e.lwd=
...
{},firstSeriesFormatting),e.title=r,e.y_axis="Y1",e.id=d+1,e.order=d+1));d++;f.push(e)}for(k=0;k<lines.length;k++)e=jQuery.extend(!0,{},firstSeriesFormatting),e.title=lines[k].type+"("+lines[k].line_pos+")",e.y_axis="Y1",e.order="",e.id=d+1,lines[k].hasOwnProperty("color")&&(e.color=lines[k].color),lines[k].hasOwnProperty("lty")&&(e.lty=lines[k].lty),lines[k].hasOwnProperty("lwd")&&(e.lwd=lines[k].lwd),d++,f.push(e);if(null==initXML)for(p=rainbow(f.length),k=0;k<f.length;k++)f[k].color=p[k];f.sort(SortByOrder);
for(k=0;k<f.length;k++)a.jqGrid("addRowData",k+1,f[k]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
...
function updateSeriesEns(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d,e=0,c={},f=$("#seriesLock").is(":checked"),g=[],h=[],l=[],p=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A=[];if(null!=initXML){h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");l=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");p=initXML.find("plot").find("show_signif").text().replace("c(",
...
"").replace(")","").replace(/"/g,"").split(",");for(var n=0;n<l.length;n++)"TRUE"==l[n].trim()?l[n]="No":l[n]="Yes","TRUE"==p[n].trim()?p[n]="No":p[n]="Yes";m=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(n=0;n<m.length;n++)m[n]=m[n].replace(/FF$/,"");q=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");t=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,
"").split(",");u=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("legend").text().replace("c(",
"").replace(/\)$/,"").replace(/"/g,"").split(",");if(z.length!=v.length)for(d=0;d<v.length;d++)z[d]=""}d=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var k=0;k<d.length;k++){var r=d[k],y=!1;for(n=0;n<b.length;n++)if(b[n].title==r){c={};c.title=r;c.y_axis="";c.order=e+1;c.hide=b[n].hide;c.plot_ci=b[n].plot_ci;c.show_signif=b[n].show_signif;c.color=b[n].color;c.pch=b[n].pch;c.type=b[n].type;c.lty=b[n].lty;c.lwd=b[n].lwd;c.con_series=b[n].con_series;c.legend=b[n].legend;
c.id=e+1;y=!0;break}y||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=r,c.y_axis=void 0,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=r,c.y_axis=void 0,c.order=v[e]&&""!=v[e]?parseInt(v[e]):parseInt(n+1),c.hide=l[e],c.plot_ci=h[e].trim(),c.show_signif=p[e],c.pch=q[e].trim(),c.type=A[e].trim(),c.lty=t[e].trim(),c.lwd=u[e].trim(),c.con_series=x[e].trim(),c.legend=z[e].trim(),c.color=m[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=r,c.y_axis=void 0,
...
function updateSeries(a){var b=$("#listdt");b.saveCell(lastSelRow,lastSelCol);var d=b.jqGrid("getRowData");b.jqGrid("clearGridData");group_name_to_value_map={};var e=0,c={},f=$("#seriesLock").is(":checked"),g=[],h=$("#plot_data").multiselect("getChecked").val(),l=[],p=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A=[],n=[];if(null!=initXML){l=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");p=initXML.find("plot").find("plot_disp").text().replace("c(",
...
else for(z=[],k=0;k<q.length;k++)z.push(k+1);n=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(A.length!=z.length)for(k=0;k<z.length;k++)A[k]=""}for(var r=1;2>=r;r++){var y="y"+r;if(1==r){var B=fcst_var_y1_indexes;var w=permuteSeries(createSeriesMapForPermutation(series_var_y1_indexes,y),0)}else B=fcst_var_y2_indexes,w=permuteSeries(createSeriesMapForPermutation(series_var_y2_indexes,
...
h=r+' ("'+c+H+h+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=h,c.y_axis="Y1",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=h,c.y_axis="Y1",c.order=parseInt(z[e]),c.hide=p[e],c.plot_ci=l[e].trim(),c.show_signif=m[e].trim(),c.pch=t[e].trim(),c.type=n[e].trim(),c.lty=u[e].trim(),c.lwd=x[e].trim(),c.con_series=v[e].trim(),c.legend=A[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=h,c.y_axis="Y1",c.id=e+1,c.order=e+1),e++,g.push(c))}seriesDiffY1=
d;d=[];for(k=0;k<seriesDiffY2.length;k++){c=seriesDiffY2[k].split(",")[0];h=seriesDiffY2[k].split(",")[1];r=seriesDiffY1[k].split(",")[2];B=y=!1;"DIFF"===r?H='"-"':"RATIO"===r&&(H='"/"');for(w=0;w<g.length;w++)c==g[w].title&&(y=!0),h==g[w].title&&(B=!0);y&&B&&(d.push(seriesDiffY2[k]),h=r+' ("'+c+H+h+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=h,c.y_axis="Y2",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=h,c.y_axis="Y2",c.order=parseInt(z[e]),c.hide=p[e],c.plot_ci=l[e].trim(),
c.show_signif=m[e],c.pch=t[e].trim(),c.type=n[e].trim(),c.lty=u[e].trim(),c.lwd=x[e].trim(),c.con_series=v[e].trim(),c.legend=A[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=h,c.y_axis="Y2",c.id=e+1,c.order=e+1),e++,g.push(c))}seriesDiffY2=d;for(k=0;k<lines.length;k++)c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=lines[k].type+"("+lines[k].line_pos+")",c.y_axis="Y1",c.order="",c.id=e+1,lines[k].hasOwnProperty("color")&&(c.color=lines[k].color),
lines[k].hasOwnProperty("lty")&&(c.lty=lines[k].lty),lines[k].hasOwnProperty("lwd")&&(c.lwd=lines[k].lwd),e++,g.push(c);if(null==initXML){if("Contour"===currentTab)for(k=0;k<g.length;k++)q.push("#000000");else q=rainbow(g.length);e=[];if(a)e=q;else{for(k=0;k<q.length;k++)for(w=0;w<g.length;w++)g[w].color&&0<g[w].color.length&&g[w].color.substr(g[w].color.length-6).toUpperCase()===q[k].substr(q[k].length-6).toUpperCase()&&k!=w&&(e[w]=q[k],e[k]=q[w]);for(k=0;k<q.length;k++)e[k]||(e[k]=q[k])}for(k=0;k<
g.length;k++)g[k].color&&0!=g[k].color.length||(g[k].color=e[k])}"Contour"===currentTab?g.reverse():g.sort(SortByOrder);for(k=0;k<g.length;k++)b.jqGrid("addRowData",k+1,g[k]),"Contour"===currentTab&&0!==k&&$("#"+(k+1),"#gbox_listdt").css({display:"none"});outerLayout.sizePane("south",$("#gbox_listdt").height())}function SortByOrder(a,b){return a.order-b.order}
...
e));a.append($("<caption_size />").text($("#caption_size").val()));a.append($("<caption_offset />").text($("#caption_offset").val()));a.append($("<caption_align />").text($("#caption_align").val()));a.append($("<ci_alpha />").text($("#ci_alpha").val()));a.append($("<eqbound_low />").text($("#eqbound_low").val()));a.append($("<eqbound_high />").text($("#eqbound_high").val()));var g=sortSeries();b=[];d=[];c=[];f=[];var h=[],l=[],p=[],m=[],q=[],t=[],u=[],x=[];for(e=0;e<g.length;e++)if(""!==g[e].order)b.push('"'+
g[e].plot_ci+'"'),"No"===g[e].hide?d.push("TRUE"):d.push("FALSE"),"No"===g[e].show_signif?u.push("FALSE"):u.push("TRUE"),c.push('"'+g[e].color+'FF"'),f.push(g[e].pch),h.push('"'+g[e].type+'"'),l.push(g[e].lty),p.push(g[e].lwd),m.push(g[e].con_series),q.push(g[e].order),t.push('"'+g[e].legend+'"');else{var v=g[e].title,z=v.split("(")[0];v=v.split("(")[1].split(")")[0];x.push($("<line />").attr("type",z).attr("line_pos",v).attr("color",g[e].color).attr("lty",g[e].lty).attr("lwd",g[e].lwd))}if(0<x.length){e=
$("<lines />");for(g=0;g<x.length;g++)e.append(x[g]);a.append(e)}a.append($("<plot_ci />").text("c("+b.join()+")"));a.append($("<show_signif />").text("c("+u.join()+")"));a.append($("<plot_disp />").text("c("+d.join()+")"));a.append($("<colors />").text("c("+c.join()+")"));a.append($("<pch />").text("c("+f.join()+")"));a.append($("<type />").text("c("+h.join()+")"));a.append($("<lty />").text("c("+l.join()+")"));a.append($("<lwd />").text("c("+p.join()+")"));a.append($("<con_series />").text("c("+
...
a).find("label").size()&&(c=$('<label for="fix_var_event_equal_'+b+'">Equalize</label>'));$("body").append(p);$("#fixed_var_table").append($("<tr>").append($("<td>").append(d)).append($("<td>").append(f)).append($("<td>").append(g)).append($("<td>").append(h)).append($("<td>").append(m).append(l)).append($("<td>").append(e).append(c)))}createValDatePeriodDialog("fixed_var_val",b);g.multiselect({selectedList:100,noneSelectedText:"Select value",position:{my:"center center",at:"right center"}});f.multiselect({multiple:!1,
...
!c),0<$("#fix_var_event_equal_"+a).find("label").size()&&(d=$('<label for="fix_var_event_equal_'+b+'">Equalize</label>')),$("body").append(m),$("#fixed_var_table").append($("<tr>").append($("<td>").append(e)).append($("<td>").append(f)).append($("<td>").append(g)).append($("<td>").append(h)).append($("<td>").append(p).append(l)).append($("<td>").append(c).append(d))),e.button({icons:{primary:"ui-icon-trash"},text:!1}).click(function(){removeFixedVar($(this).attr("id"))}));createValDatePeriodDialog("fixed_var_val",
...
b).val();0===g.length&&(g=3600);g=parseFloat(g);"min"===h?g*=60:"hours"===h?g*=3600:"days"===h&&(g*=86400);var l;"fixed_var_val"===a?l=$(fixVarValResponse[b]).find("val"):"series_var_val_y1"===a?l=$(seriesY1VarValResponse[b]).find("val"):"series_var_val_y2"===a&&(l=$(seriesY2VarValResponse[b]).find("val"));for(c=c.clone();c<=f;){h=c.format("YYYY-MM-DD HH:mm:ss");for(var p=!1,m=0;m<l.length;m++)if($(l[m]).text()==h){p=!0;break}p&&$("#"+a+"_"+b).find('option[value="'+h+'"]').prop("selected",!0);c.add(g,
...
3600);f=parseInt(f);"min"===g?f*=60:"hours"===g?f*=3600:"days"===g&&(f*=86400);g=$(previousIndVarValResponse).find("val");for(e=e.clone();e<=c;){for(var h=e.format("YYYY-MM-DD HH:mm:ss"),l=!1,p=0;p<g.length;p++)if($(g[p]).text()===h){l=!0;break}l&&d.find('option[value="'+h+'"]').prop("selected",!0);e.add(f,"seconds")}try{d.multiselect("refresh")}catch(m){}}$(this).dialog("close")}},open:function(e,c){e=d.val();null==e&&(e=[]);populateIndyVarVal(e);e=$(previousIndVarValResponse).find("val");c=a.val();
...
close:function(){}});$("#listdt").jqGrid("navButtonAdd","#pagerdt",{caption:"Add Derived Curve",title:"Add Derived Curve",buttonicon:"ui-icon-plus",onClickButton:function(){"Roc"===currentTab||"Rely"===currentTab||"Ens_ss"===currentTab||"Perf"===currentTab||"Hist"===currentTab||"Eclv"===currentTab||"Contour"===currentTab?d.dialog("open"):1<$("#listdt").jqGrid("getRowData").length?a.dialog("open"):c.dialog("open")}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Remove Derived Curve",title:"Remove Derived Curve",
buttonicon:"ui-icon-trash",onClickButton:function(){var a=$(this).jqGrid("getGridParam","selrow");if(a){var b=$(this).getRowData(a);if(b.title.startsWith("DIFF")||b.title.startsWith("RATIO")||b.title.startsWith("SS")||b.title.startsWith("ETB")){$(this).jqGrid("delRowData",a);$("#1","#gbox_listdt").css({display:""});a=b.title.slice(0,-1);if(b.title.startsWith("DIFF")){var c=a.replace("DIFF (","").split('"-"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",DIFF"}else if(b.title.startsWith("RATIO")){c=
a.replace("RATIO (","").split('"/"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",RATIO"}else if(b.title.startsWith("SS")){c=a.replace("SS (","").split('"and"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",SS"}else{c=a.replace("ETB (","").split('"and"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",ETB"}"Y1"==b.y_axis?(b=seriesDiffY1.indexOf(a),-1<b&&seriesDiffY1.splice(b,1)):(b=seriesDiffY2.indexOf(a),-1<b&&seriesDiffY2.splice(b,1));b=$("#listdt").jqGrid("getRowData");
...
$("#listdt").setGridWidth($(window).width()-20);var d=$("#unavailableDiffCurveDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){d.dialog("close")}}}),e=$("#unavailableLineDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){e.dialog("close")}}}),c=$("#incorrectDiffCurveDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){c.dialog("close")}}});$(window).bind("resize",function(){$("#listdt").setGridWidth($(window).width()-
20)}).trigger("resize");$("#listdt").jqGrid("sortableRows",{update:function(a,b){a=$("#listdt").jqGrid("getRowData");for(b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",a[b].id).order=b+1}});$("#pagerdt_left table.navtable tbody tr").append('<td><div ><input type="checkbox"  style="margin:0 4px;" id="seriesLock"/><label for="seriesLock">Lock Formatting</label></div></td>');$("#seriesLock").change(function(){$(this).is(":checked")?$(this).attr("checked",
"checked"):$(this).removeAttr("checked")});$("#generate_plot").button({}).click(function(){sendXml()});var f=$("#upload_file_dialog").dialog({modal:!0,autoOpen:!1,buttons:{OK:function(){$("#formUpload").submit()},Cancel:function(){f.dialog("close")}}});$("#load_xml").button({}).click(function(){f.dialog("open")});$("#reload_databases").button({}).click(function(){requestDBUpdate()});$("input[name=show_history_choice][value=show_history_success]").prop("checked",!0);$("#show_history_choice").buttonset();
...
$("#reset_formatting").button({icons:{primary:"ui-icon-arrowrefresh-1-w"},text:!1}).click(function(){resetFormatting()});$("#plot_image").click(function(){"IMG"===document.getElementById("plot_image").firstChild.tagName&&viewImage(resultName.replace("plot_",""))});var g=0;if(null!=initXML){var h=initXML.find("plot").find("template")[0].innerHTML;"series_plot.R_tmpl"===h||"revision_series_plot.R_tmpl"===h?currentTab="Series":"box_plot.R_tmpl"===h||"revision_box_plot.R_tmpl"===h?currentTab="Box":"bar_plot.R_tmpl"===
h?currentTab="Bar":"rhist.R_tmpl"===h||"phist.R_tmpl"===h||"relp.R_tmpl"===h?currentTab="Hist":"roc.R_tmpl"===h?currentTab="Roc":"eclv.R_tmpl"===h?currentTab="Eclv":"rely.R_tmpl"===h?currentTab="Rely":"ens_ss.R_tmpl"===h?currentTab="Ens_ss":"performance.R_tmpl"===h?currentTab="Perf":"taylor_plot.R_tmpl"===h?currentTab="Taylor":"contour_plot.R_tmpl"===h&&(currentTab="Contour");var l=$("#plot_config").find("a");for(h=0;h<l.length;h++)if(l[h].text===currentTab){g=h;break}}$("#plot_config").tabs({active:g,
...
d=parseInt(d);"min"===e?d*=60:"hours"===e?d*=3600:"days"===e&&(d*=86400);e=moment.tz(a.date1,"UTC");var h=moment.tz(a.date2,"UTC");g.multiselect("uncheckAll");for(var l=$('[name="multiselect_indy_var_val"]');e<=h;){for(var p=e.format("YYYY-MM-DD HH:mm:ss"),m=!1,q=0;q<c.length;q++)if($(c[q]).text()===p){m=!0;break}if(m){try{var t=e.format(f)}catch(x){console.log("formatting error"),console.error(x),t=p}m=g.find('option[value="'+p+'"]');m.prop("selected",!0);if(!b)for(indy_var_vals_to_attr={},q=0;q<
