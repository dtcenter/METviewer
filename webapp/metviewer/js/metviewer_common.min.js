...
a.append($("<caption_offset />").text($("#caption_offset").val()));a.append($("<caption_align />").text($("#caption_align").val()));a.append($("<ci_alpha />").text($("#ci_alpha").val()));a.append($("<low_eqbound />").text($("#low_eqbound").val()));a.append($("<high_eqbound />").text($("#high_eqbound").val()));b=sortSeries();d=[];c=[];f=[];var g=[],h=[],k=[],n=[],m=[],q=[],t=[],u=[];for(e=0;e<b.length;e++)d.push('"'+b[e].plot_ci+'"'),"No"===b[e].hide?c.push("TRUE"):c.push("FALSE"),"No"===b[e].show_signif?
...
$("#y2_lim_max").val()+")"));return a}function sortSeries(){$("#listdt").saveCell(lastSelRow,lastSelCol);for(var a=$("#listdt").jqGrid("getRowData"),b=[],d=0;d<a.length;d++)"none"!==$("#"+(d+1),"#gbox_listdt").css("display")&&b.push(a[d]);1===b.length?b[0].order=1:b=b.sort(function(a,b){return parseInt(a.id)>parseInt(b.id)?1:parseInt(a.id)<parseInt(b.id)?-1:0});return b}
...
"true"===$(a[b]).attr("success")?"#000000":"#B39A9A";$("#history_content").append($("<tr/>").append($("<td/>").append($("<button />").attr("id",e).addClass("view-image").text("Image"))).append($("<td/>").append($("<button />").attr("id",e).addClass("load-image").text("XML"))).append($("<td/>").css("color",d).text(e)))}$(".view-image").button({icons:{primary:"ui-icon-image"},text:!1}).click(function(){viewImage($(this).attr("id"))});$(".load-image").button({icons:{primary:"ui-icon-document"},text:!1}).click(function(){$("#uploadLocalId").val($(this).prop("id"));
...
(b+1)).css("display","none");updateSeriesVarValHist(b+1,d)}else d=$("#series_var_y1_1").first().val(),updateSeriesVarValHist(1,d);updatePlotFix();if(initXML.find("plot").find("summary_curve")&&0<initXML.find("plot").find("summary_curve").children().length){a=initXML.find("plot").find("summary_curve").children();for(b=0;b<a.length;b++)$("#summary_curve option[value='"+a[b]+"']").prop("selected",!0);try{$("#summary_curve").multiselect("refresh")}catch(c){}}b=!0;b=$(initXML.find("plot").find("rely_event_hist")).text();
...
e&&(currentTab="Contour");var c=$("#plot_config").find("a");for(e=0;e<c.length;e++)if(c[e].text===currentTab){d=e;break}}$("#plot_config").tabs({active:d,beforeActivate:function(a,b){a=b.oldTab.attr("aria-controls");$("#"+a).empty();cleanUp();currentTab=b.newTab.text()},beforeLoad:function(a,b){$.ajaxSetup({cache:!0});b.jqXHR.error(function(){b.panel.html("Couldn't load this tab. We'll try to fix this as soon as possible. If this wouldn't be a demo.")})}});null!=initXML?($("#plot_title").val($(initXML.find("plot").find("tmpl").find("title")).text()),
...
$("#varianceInflationFactor").prop("checked","true"===$(initXML.find("plot").find("varianceinflationfactor")).text()),$("#ci_alpha").val($(initXML.find("plot").find("ci_alpha")).text()),$("#low_eqbound").val($(initXML.find("plot").find("low_eqbound")).text()),$("#high_eqbound").val($(initXML.find("plot").find("high_eqbound")).text()),$("#plot_type").val($(initXML.find("plot").find("plot_type")).text()),$("#plot_height").val($(initXML.find("plot").find("plot_height")).text()),$("#plot_width").val($(initXML.find("plot").find("plot_width")).text()),
