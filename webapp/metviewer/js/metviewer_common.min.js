...
function updateSeriesHist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],m=[],p=[],t=[],r=[],y=[],v=[],w=[],x=[],B=[],q;"Hist"===currentTab?q="Rank Histogram":"Roc"===currentTab?q="ROC Curve":"Rely"===currentTab?q="Reliability Curve":"Taylor"===currentTab&&(q=$("#fcst_var_y1_1").val());if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")",
"").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=0;l<k.length;l++)"TRUE"===k[l].trim()?k[l]="No":k[l]="Yes","TRUE"===m[l].trim()?m[l]="No":m[l]="Yes";p=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(l=0;l<p.length;l++)p[l]=p[l].replace(/FF$/,
"");t=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");r=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(","").replace(")",
"").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(x.length!=w.length)for(c=0;c<w.length;c++)x[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);var u;0==c.length&&(c[0]="");if("Roc"==currentTab||"Rely"==currentTab){try{u=$("#summary_curve").multiselect("getChecked")}catch(z){u=
