...
"</db_con><list_val><stat_field>FCST_VAR</stat_field></list_val></request>",error:function(){},success:function(b){var e=$(b).find("val");if(0<e.length){b=[];for(var d=0;d<e.length;d++){var g=$(e[d]);if(0==d||0!=d&&g.text()!==$(e[d-1]).text()){var h=g.text().formatAll();g=$("<option />",{value:h,text:h,title:value_to_desc_map[g.text()]});b.push(g)}else 0!=d&&g.text()===$(e[d-1]).text()&&b[b.length-1].text(b[b.length-1].text()+"*")}g=$("input[name='multiselect_database']:checked").length-1;for(d=0;d<
b.length;d++)b[d].text()!==b[d][0].value&&(e=b[d].text().substring(b[d][0].value.length,b[d].text().length).length,e===g?b[d].text(b[d][0].value+"*"):b[d].text(b[d][0].value)),b[d].appendTo(a);try{a.multiselect("refresh")}catch(k){}updateStatVal()}}})}
...
b===g?a[e].text(a[e][0].value+"*"):a[e].text(a[e][0].value)),a[e].appendTo(d)}else g=$("<option />",{value:"N/A",text:"N/A"}),g.appendTo(d);try{d.multiselect("refresh")}catch(k){console.log(k)}updateSeries()}})}}
function updateStats(a,b,d){var e=$("#fcst_stat_"+a+"_"+b),c=$("#fcst_stat_mode_"+a+"_"+b),f=$("#plot_data").multiselect("getChecked").val(),g=getSelectedDatabases();if(g){try{var h=$("#fcst_var_"+a+"_"+b).multiselect("getChecked").val()}catch(n){h=$("#fcst_var_"+a+"_"+b+" option:first-child").val()}if("stat"===f){e.empty();try{c.multiselect("destroy")}catch(n){}c.css("display","none");$("#fcst_stat_mode_config_"+a+"_"+b).css("display","none");try{e.multiselect("option","noneSelectedText","Select attribute stat")}catch(n){e.prop("disabled",
...
function updateFixedVarVal(a,b,d){d&&"false"===d&&$("#fix_var_event_equal_"+a).prop("checked",!1);var e=$("#fixed_var_val_"+a);e.empty();if(d=getSelectedDatabases()){if("Perf"===currentTab){var c="stat";var f="<stat><fcst_var><val>FAR</val></fcst_var></stat>"}else"Eclv"===currentTab?(c="eclv",f="<stat><fcst_var><val>NA</val></fcst_var></stat>"):"Taylor"===currentTab?(c="taylor",f=convertVarsAndStatsToXml()):"Contour"===currentTab?(c="stat",f='<stat><fcst_var name="'+$("#fcst_var_y1_1").val()+'" ><val>'+
$("#fcst_stat_y1_1").val()+"</val></fcst_var></stat>"):(c=$("#plot_data").multiselect("getChecked").val(),f=convertVarsAndStatsToXml());try{var g=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(h){g=$("#fixed_var_"+a+" option:first-child").val()}$.ajax({async:!1,url:"servlet",type:"POST",dataType:"xml",processData:!1,contentType:"application/xml",data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+d+"</db_con><list_val><"+c+"_field>"+g+"</"+c+"_field>"+f+"</list_val></request>",
...
function populateIndyVarVal(a){var b=getSelectedDatabases();if(b){if("Perf"===currentTab){var d="stat";var e="<stat><fcst_var><val>FAR</val></fcst_var></stat>"}else"Contour"===currentTab?(d="stat",e='<stat><fcst_var name="'+$("#fcst_var_y1_1").val()+'" ><val>'+$("#fcst_stat_y1_1").val()+"</val></fcst_var></stat>"):(d=$("#plot_data").multiselect("getChecked").val(),e=convertVarsAndStatsToXml());try{var c=$("#indy_var").multiselect("getChecked").val()}catch(f){c=$("#indy_var option:first-child").val()}$.ajax({async:!1,
...
function updateResult(a){resultName=a;$("#plot_display_inner_header").text(resultName.replace("plot_",""));$("#plot_image").empty();$.get(urlOutput+"plots/"+resultName+".html",function(a){$("#plot_image").append(a)}).fail(function(){var a=$("<img>");a.attr("width","100%");a.attr("height","100%");a.attr("src",urlOutput+"plots/"+resultName+".png?"+(new Date).getTime());a.appendTo("#plot_image")});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".xml",dataType:"xml",success:function(a){a="msie"===
jQuery.browser?a.xml:(new XMLSerializer).serializeToString(a);document.getElementById("plot_xml").innerHTML=formatXml(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/ /g,"&nbsp;").replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_xml").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".sql",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("plot_sql").innerHTML=a.responseText.replace(/\n/g,
"<br />")},error:function(){document.getElementById("plot_sql").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"scripts/"+resultName+".R",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("r_script").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("r_script").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".log",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("plot_log").innerHTML=
...
$("#reset_formatting").button({icons:{primary:"ui-icon-arrowrefresh-1-w"},text:!1}).click(function(){resetFormatting()});$("#plot_image").click(function(){});var d=0;if(null!=initXML){var e=initXML.find("plot").find("template")[0].innerHTML;"series_plot.R_tmpl"===e||"revision_series_plot.R_tmpl"===e?currentTab="Series":"box_plot.R_tmpl"===e||"revision_box_plot.R_tmpl"===e?currentTab="Box":"bar_plot.R_tmpl"===e?currentTab="Bar":"rhist.R_tmpl"===e||"phist.R_tmpl"===e||"relp.R_tmpl"===e?currentTab="Hist":
"roc.R_tmpl"===e?currentTab="Roc":"eclv.R_tmpl"===e?currentTab="Eclv":"rely.R_tmpl"===e?currentTab="Rely":"ens_ss.R_tmpl"===e?currentTab="Ens_ss":"performance.R_tmpl"===e?currentTab="Perf":"taylor_plot.R_tmpl"===e?currentTab="Taylor":"contour_plot.R_tmpl"===e&&(currentTab="Contour");var c=$("#plot_config").find("a");for(e=0;e<c.length;e++)if(c[e].text===currentTab){d=e;break}}$("#plot_config").tabs({active:d,beforeActivate:function(a,b){a=b.oldTab.attr("aria-controls");$("#"+a).empty();cleanUp();
currentTab=b.newTab.text()},beforeLoad:function(a,b){$.ajaxSetup({cache:!0});b.jqXHR.error(function(){b.panel.html("Couldn't load this tab. We'll try to fix this as soon as possible. If this wouldn't be a demo.")})}});null!=initXML?($("#plot_title").val($(initXML.find("plot").find("tmpl").find("title")).text()),$("#x_label_title").val($(initXML.find("plot").find("tmpl").find("x_label")).text()),$("#y1_label_title").val($(initXML.find("plot").find("tmpl").find("y1_label")).text()),$("#y2_label_title").val($(initXML.find("plot").find("tmpl").find("y2_label")).text()),
$("#caption").val($(initXML.find("plot").find("tmpl").find("caption")).text()),$(initXML.find("plot").find("tmpl").find("job_title"))&&$("#job_title").val($(initXML.find("plot").find("tmpl").find("job_title")).text()),$(initXML.find("plot").find("tmpl").find("keep_revisions"))&&$("#keep_revisions").prop("checked","true"===$(initXML.find("plot").find("tmpl").find("keep_revisions")).text()),$("#is_python").prop("checked","Python"==$(initXML.find("plot").find("execution_type")).text()),$("#vert_plot").prop("checked",
