...
fcst_var_y2_indexes[a]);for(a=1;a<series_var_y1_indexes.length;a++)removeSeriesVar("series_var_y1_"+series_var_y1_indexes[a]);for(a=0;a<series_var_y2_indexes.length;a++)removeSeriesVar("series_var_y2_"+series_var_y2_indexes[a]);var e=fixed_var_indexes.slice();for(a=0;a<e.length;a++)removeFixedVar("fixed_var_"+e[a]);$("#group_series_var_y1_1").prop("checked",!1);a=$("#database").multiselect("getChecked").val();$.ajax({async:!1,url:"servlet",type:"POST",dataType:"xml",processData:!1,data:"<request><db_con>"+
a+"</db_con><list_val><id>0</id><"+selected_mode+"_field>FCST_VAR</"+selected_mode+"_field></list_val></request>",error:function(){},success:function(a){a=$(a).find("val");var e;if(0<a.length)for(var g=0;g<a.length;g++)e=$(a[g]),e=$("<option />",{value:e.text(),text:e.text(),title:value_to_desc_map[e.text()]}),e.appendTo(b),e.clone().appendTo(c);else e=$("<option />",{value:"N/A",text:"N/A"}),e.appendTo(b),e.clone().appendTo(c);try{b.multiselect("refresh")}catch(h){}try{c.multiselect("refresh")}catch(h){}}})}
function updateMode(a,b,c){var e=$("#fcst_stat_"+a+"_"+b);e.empty();var d,f,g;try{d=$("#fcst_var_"+a+"_"+b).multiselect("getChecked").val()}catch(r){d=$("#fcst_var_"+a+"_"+b+" option:first-child").val()}var h;for(f=0;f<listStatModelRatio.length;f++)g=listStatModelRatio[f],h=$.isArray(c)?0<=$.inArray(g,c):g==c,g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(e);try{e.multiselect("option","noneSelectedText","Select ratio stat"),e.multiselect("refresh")}catch(r){}var k=
$("#fcst_stat_mode_"+a+"_"+b),l="",p="";0<c.length&&2==c.split("_").length&&(l=c.split("_")[0],p=c.split("_")[1]);"ACOVACOV"==l&&(l="ACOV");c=!1;var t=0==k.find("option").length;if(!t)try{k.multiselect("destroy")}catch(r){}for(f=0;f<listStatMode.length;f++)g=listStatMode[f],(h=g==l)&&(c=!0),t?(g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(k)):h&&k.val(g);k.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:225,height:300,noneSelectedText:"Select attribute stat",
allUnselected:!c,click:function(a,b){var e=this.id.split("_");$("#fcst_stat_"+e[e.length-2]+"_"+e[e.length-1]+" option").removeAttr("selected");try{$("#fcst_stat_"+e[e.length-2]+"_"+e[e.length-1]).multiselect("refresh")}catch(c){}e=$("#fcst_stat_mode_config_"+e[e.length-2]+"_"+e[e.length-1]);e.css("display","block");"ACOV"==b.value?e.find(".non-acov").attr("disabled",!0):e.find(".non-acov").removeAttr("disabled");-1<listStatModePair.indexOf(b.value)?e.find(".non-pair").attr("disabled",!0):e.find(".non-pair").removeAttr("disabled");
updateSeries()}});c&&(f=$("#fcst_stat_mode_config_"+a+"_"+b),f.css("display","block"),"ACOV"==l?f.find(".non-acov").attr("disabled",!0):f.find(".non-acov").removeAttr("disabled"),2==p.length&&(p="A"+p),"ACOV"==l?"A"==p[0]?(f.find('[name="mode_stat_fcst"]').prop("checked",!0),f.find('[name="mode_stat_obs"]').prop("checked",!0)):"F"==p[0]?(f.find('[name="mode_stat_fcst"]').prop("checked",!0),f.find('[name="mode_stat_obs"]').prop("checked",!1)):"O"==p[0]&&(f.find('[name="mode_stat_fcst"]').prop("checked",
!1),f.find('[name="mode_stat_obs"]').prop("checked",!0)):("D"==p[0]?f.find('[name="mode_stat_diff"]').prop("checked",!0):"A"==p[0]?(f.find('[name="mode_stat_diff"]').prop("checked",!1),f.find('[name="mode_stat_fcst"]').prop("checked",!0),f.find('[name="mode_stat_obs"]').prop("checked",!0)):"F"==p[0]?(f.find('[name="mode_stat_diff"]').prop("checked",!1),f.find('[name="mode_stat_fcst"]').prop("checked",!0),f.find('[name="mode_stat_obs"]').prop("checked",!1)):"O"==p[0]&&(f.find('[name="mode_stat_diff"]').prop("checked",
!1),f.find('[name="mode_stat_fcst"]').prop("checked",!1),f.find('[name="mode_stat_obs"]').prop("checked",!0)),"A"==p[1]?(f.find('[name="mode_stat_simple"]').prop("checked",!0),f.find('[name="mode_stat_cluster"]').prop("checked",!0)):"S"==p[1]?(f.find('[name="mode_stat_simple"]').prop("checked",!0),f.find('[name="mode_stat_cluster"]').prop("checked",!1)):"C"==p[1]&&(f.find('[name="mode_stat_simple"]').prop("checked",!1),f.find('[name="mode_stat_cluster"]').prop("checked",!0)),"A"==p[2]?(f.find('[name="mode_stat_matched"]').prop("checked",
!0),f.find('[name="mode_stat_unmatched"]').prop("checked",!0)):"M"==p[2]?(f.find('[name="mode_stat_matched"]').prop("checked",!0),f.find('[name="mode_stat_unmatched"]').prop("checked",!1)):"U"==p[2]&&(f.find('[name="mode_stat_matched"]').prop("checked",!1),f.find('[name="mode_stat_unmatched"]').prop("checked",!0))),-1<listStatModePair.indexOf(l)?f.find(".non-pair").attr("disabled",!0):f.find(".non-pair").removeAttr("disabled"));if("N/A"==d){try{e.multiselect("disable")}catch(r){}try{k.multiselect("disable")}catch(r){}}else{try{e.multiselect("enable")}catch(r){}try{k.multiselect("enable")}catch(r){}}e=
$("#series_var_"+a+"_"+b);e.empty();$.each(fix_var_value_to_title_mode_map,function(a,b){e.append('<option value="'+a+'">'+b+"</option>")});try{e.multiselect("refresh")}catch(r){}}
...
h+"</stat_fcst_var></list_stat></request>",error:function(){},success:function(a){a=$(a).find("val");var b;if(0<a.length)for(var d=0;d<a.length;d++){var f=$(a[d]);b=0<=$.inArray(f.text(),c);b=$("<option />",{value:f.text(),text:f.text(),title:value_to_desc_map[f.text()],selected:b});b.appendTo(e)}else b=$("<option />",{value:"N/A",text:"N/A"}),b.appendTo(e);try{e.multiselect("refresh")}catch(g){}c=[]}})}var k=$("#series_var_"+a+"_"+b);k.empty();$.each(series_var_value_to_title_stat_map,function(a,
b){k.append('<option value="'+a+'">'+b+"</option>")});try{k.multiselect("refresh")}catch(l){}}
function updateSeriesVarValEns(a,b){$("#listdt").jqGrid("clearGridData");var c=$("#series_var_val_y1_"+a);c.empty();var e=$("#database").multiselect("getChecked").val(),d;try{d=$("#series_var_y1_"+a).multiselect("getChecked").val()}catch(f){d=$("#series_var_y1_"+a+" option:first-child").val()}$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><ensss_field>"+d+'</ensss_field><stat><fcst_var name="TMP"></fcst_var></stat></list_val></request>',error:function(){},
success:function(e){seriesY1VarValResponse[a]=e;e=$($.parseXML(e)).find("val");var d;if(0<e.length)for(var h=0;h<e.length;h++){var k=$(e[h]);d=0<=$.inArray(k.text(),b);d=$("<option />",{value:k.text(),text:k.text(),selected:d});d.appendTo(c)}else d=$("<option />",{value:"N/A",text:"N/A"}),d.appendTo(c);try{c.multiselect("refresh")}catch(l){}b=[]}})}
function updateSeriesVarVal(a,b,c){var e=$("#series_var_val_"+a+"_"+b);e.empty();var d=$("#database").multiselect("getChecked").val(),f,g;"Perf"==currentTab?(f="stat",g="<stat></stat>"):(f="Taylor"==currentTab?"taylor":$("#plot_data").multiselect("getChecked").val(),g=convertVarsAndStatsToXml());var h;try{h=$("#series_var_"+a+"_"+b).multiselect("getChecked").val()}catch(k){h=$("#series_var_"+a+"_"+b+" option:first-child").val()}$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+d+
"</db_con><list_val><id>0</id><"+f+"_field>"+h+"</"+f+"_field>"+g+"</list_val></request>",error:function(){},success:function(d){"y1"===a?seriesY1VarValResponse[b]=d:seriesY2VarValResponse[b]=d;d=$($.parseXML(d)).find("val");var f;if(0<d.length)for(var g=0;g<d.length;g++){var h=$(d[g]);f="string"==$.type(c)?h.text()==c:0<=$.inArray(h.text(),c);f=$("<option />",{value:h.text(),text:h.text(),selected:f});f.appendTo(e)}else f=$("<option />",{value:"N/A",text:"N/A"}),f.appendTo(e);try{e.multiselect("refresh")}catch(r){}1===
c.length&&-1!==c[0].indexOf(",")&&$("#group_series_var_"+a+"_"+b).prop("checked",!0);c=[]}})}
function updateFixedVarValHist(a,b){var c=$("#fixed_var_val_"+a);c.empty();var e=$("#database").multiselect("getChecked").val(),d;try{d=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(g){d=$("#fixed_var_"+a+" option:first-child").val()}var f;"Rhist"===currentTab?f="rhist":"Phist"===currentTab?f="phist":"Roc"===currentTab?f="roc":"Rely"===currentTab?f="rely":"Ens_ss"===currentTab?f="ensss":"Perf"===currentTab?f="perf":"Taylor"===currentTab&&(f="taylor");$.ajax({async:!1,url:"servlet",type:"POST",
dataType:"xml",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+"_field></list_val></request>",error:function(){},success:function(e){fixVarValResponse[a]=e;e=$(e).find("val");var d;if(0<e.length)for(var f=0;f<e.length;f++){var l=$(e[f]);d=0<=$.inArray(l.text(),b);d=$("<option />",{value:l.text(),text:l.text(),selected:d});d.appendTo(c)}else d=$("<option />",{value:"N/A",text:"N/A"}),d.appendTo(c);try{c.multiselect("refresh")}catch(p){}b=[]}})}
function updateFixedVarVal(a,b,c){c&&"false"===c&&$("#fix_var_event_equal_"+a).prop("checked",!1);var e=$("#fixed_var_val_"+a);e.empty();c=$("#database").multiselect("getChecked").val();var d,f,g;"Perf"==currentTab?(f="stat",g="<stat><fcst_var ><val>FAR</val></fcst_var></stat>"):(f="Taylor"==currentTab?"taylor":$("#plot_data").multiselect("getChecked").val(),g=convertVarsAndStatsToXml());try{d=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(h){d=$("#fixed_var_"+a+" option:first-child").val()}$.ajax({async:!1,
url:"servlet",type:"POST",dataType:"xml",data:"<request><db_con>"+c+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+"_field>"+g+"</list_val></request>",error:function(){try{e.multiselect("refresh")}catch(a){}},success:function(d){fixVarValResponse[a]=d;d=$(d).find("val");var c;if(0<d.length)for(var f=0;f<d.length;f++){var g=$(d[f]);c=0<=$.inArray(g.text(),b);c=$("<option />",{value:g.text(),text:g.text(),selected:c});c.appendTo(e)}else c=$("<option />",{value:"N/A",text:"N/A"}),c.appendTo(e);
...
d+"</list_val></request>",error:function(){},success:function(b){var e=$(b).find("val");if(b!=previousIndVarValResponse){previousIndVarValResponse=b;b=$("#indy_var_val");var d=$('[name="multiselect_indy_var_val"]');if(d&&0<d.length){indy_var_vals_to_attr={};for(var c=0;c<d.length;c++){var l={},p=$(d[c]).attr("id");l.label=$("#"+p+"_label").val();l.plot_val=$("#"+p+"_plot_val").val();indy_var_vals_to_attr[d[c].value]=l}}$("#indy_var_val").multiselect("option","indy_var_vals_to_attr",indy_var_vals_to_attr);
b.empty();if(0<e.length){for(c=0;c<e.length;c++)p=$(e[c]),l=0<=$.inArray(p.text(),a),l=$("<option />",{value:p.text(),text:p.text(),selected:l,class:"indy-var-option"}),l.appendTo(b);try{b.multiselect("refresh"),d&&0<d.length||$("#indy_var_val").multiselect("option","indy_var_vals_to_attr",null)}catch(t){}}else{l=$("<option />",{value:"N/A",text:"N/A"});l.appendTo(b);try{b.multiselect("refresh")}catch(t){}}}}})}
...
function updateSeriesRhist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],l=[],p=[],t=[],r=[],z=[],u=[],x=[],y=[],B=[],q;"Rhist"===currentTab||"Phist"===currentTab?q="Rank Histogram":"Roc"===currentTab?q="ROC Curve":"Rely"===currentTab?q="Reliability Curve":"Taylor"===currentTab&&(q=$("#fcst_var_y1_1").val());if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=0;m<k.length;m++)"TRUE"===k[m].trim()?k[m]="No":k[m]="Yes","TRUE"===l[m].trim()?l[m]="No":l[m]="Yes";p=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(m=0;m<p.length;m++)p[m]=
p[m].replace(/FF$/,"");t=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");r=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");u=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("order_series").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(y.length!=x.length)for(c=0;c<x.length;c++)y[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);var v;0==c.length&&(c[0]="");if("Roc"==currentTab||"Rely"==currentTab){try{v=$("#summary_curve").multiselect("getChecked")}catch(w){v=
[]}for(m=0;m<v.length;m++)"none"!=v[m].value&&c.push(v[m].value)}for(var A=0;A<c.length;A++){v=c[A]+" "+q;for(var n=!1,m=0;m<b.length;m++)if(b[m].title==v){d={};d.title=v;d.y_axis="Y1";d.order=e+1;d.hide=b[m].hide;d.plot_ci=b[m].plot_ci;d.show_signif=b[m].show_signif;d.color=b[m].color;d.pch=b[m].pch;d.type=b[m].type;d.lty=b[m].lty;d.lwd=b[m].lwd;d.con_series=b[m].con_series;d.legend=b[m].legend;d.id=e+1;n=!0;break}n||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=v,d.y_axis=
"Y1",d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=v,d.y_axis="Y1",d.order=x[e]&&""!=x[e]?parseInt(x[e]):parseInt(m+1),d.hide=k[e],d.plot_ci=h[e].trim(),d.show_signif=l[e],d.pch=t[e].trim(),d.type=B[e].trim(),d.lty=r[e].trim(),d.lwd=z[e].trim(),d.con_series=u[e].trim(),d.legend=y[e].trim(),d.color=p[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=v,d.y_axis="Y1",d.id=e+1,d.order=e+1));e++;g.push(d)}if(null==initXML)for(p=rainbow(g.length),m=0;m<g.length;m++)g[m].color=
p[m];g.sort(SortByOrder);for(m=0;m<g.length;m++)a.jqGrid("addRowData",m+1,g[m]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesVarValRhist(a,b){$("#listdt").jqGrid("clearGridData");var c=$("#series_var_val_y1_"+a);c.empty();var e=$("#database").multiselect("getChecked").val(),d;try{d=$("#series_var_y1_"+a).multiselect("getChecked").val()}catch(g){d=$("#series_var_y1_"+a+" option:first-child").val()}var f;"Rhist"===currentTab?f="rhist":"Phist"===currentTab?f="phist":"Roc"===currentTab?f="roc":"Rely"===currentTab?f="rely":"Ens_ss"===currentTab?f="ensss":"Perf"===currentTab?f="perf":"Taylor"===currentTab&&
(f="taylor");$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+'_field><stat><fcst_var name="NA"><val>NA</val></fcst_var></stat></list_val></request>',error:function(){},success:function(e){seriesY1VarValResponse[a]=e;e=$($.parseXML(e)).find("val");var d;if(0<e.length)for(var f=0;f<e.length;f++){var l=$(e[f]);d=0<=$.inArray(l.text(),b);d=$("<option />",{value:l.text(),text:l.text(),selected:d});d.appendTo(c)}else d=$("<option />",
{value:"N/A",text:"N/A"}),d.appendTo(c);try{c.multiselect("refresh")}catch(p){}b=[]}})}
...
function updateSeriesEns(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h,k=[],l=[],p=[],t=[],r=[],z=[],u=[],x=[],y=[],B=[],q=[];if(null!=initXML){for(var k=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),p=initXML.find("plot").find("show_signif").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),m=0;m<l.length;m++)"TRUE"==l[m].trim()?l[m]="No":l[m]="Yes","TRUE"==p[m].trim()?p[m]="No":p[m]="Yes";t=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(m=0;m<t.length;m++)t[m]=t[m].replace(/FF$/,"");r=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");
...
"").replace(/"/g,"").split(",");if(B.length!=y.length)for(c=0;c<y.length;c++)B[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var v=$("input:radio[name='ensss_pts_disp']:checked").val(),A=0;2>A;A++)if(0==A||1==A&&"true"==v){var n;0==A?(n="MSE",h="Y1"):(n="#PTS",h="Y2");for(var w=0;w<c.length;w++){for(var C=c[w]+" "+n,G=!1,m=0;m<b.length;m++)if(b[m].title==C){d={};d.title=C;d.y_axis=h;d.order=e+1;d.hide=b[m].hide;d.plot_ci=b[m].plot_ci;d.show_signif=b[m].show_signif;
d.color=b[m].color;d.pch=b[m].pch;d.type=b[m].type;d.lty=b[m].lty;d.lwd=b[m].lwd;d.con_series=b[m].con_series;d.legend=b[m].legend;d.id=e+1;G=!0;break}G||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=C,d.y_axis=h,d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=C,d.y_axis=h,d.order=y[e]&&""!=y[e]?parseInt(y[e]):parseInt(m+1),d.hide=l[e],d.plot_ci=k[e].trim(),d.show_signif=p[e],d.pch=r[e].trim(),d.type=q[e].trim(),d.lty=z[e].trim(),d.lwd=u[e].trim(),d.con_series=x[e].trim(),
d.legend=B[e].trim(),d.color=t[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=C,d.y_axis=h,d.id=e+1,d.order=e+1));e++;g.push(d)}}if(null==initXML)for(t=rainbow(g.length),m=0;m<g.length;m++)g[m].color=t[m];g.sort(SortByOrder);for(m=0;m<g.length;m++)a.jqGrid("addRowData",m+1,g[m]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesPerf(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],l=[],p=[],t=[],r=[],z=[],u=[],x=[],y=[],B=[];if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=initXML.find("plot").find("show_signif").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),q=0;q<k.length;q++)"TRUE"==k[q].trim()?k[q]="No":k[q]="Yes","TRUE"==l[q].trim()?l[q]="No":l[q]="Yes";p=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(q=0;q<p.length;q++)p[q]=p[q].replace(/FF$/,"");t=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");r=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");
...
"").replace(/"/g,"").split(",");if(y.length!=x.length)for(c=0;c<x.length;c++)y[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var m=0;m<c.length;m++){for(var v=c[m],A=!1,q=0;q<b.length;q++)if(b[q].title==v){d={};d.title=v;d.y_axis="";d.order=e+1;d.hide=b[q].hide;d.plot_ci=b[q].plot_ci;d.show_signif=b[q].show_signif;d.color=b[q].color;d.pch=b[q].pch;d.type=b[q].type;d.lty=b[q].lty;d.lwd=b[q].lwd;d.con_series=b[q].con_series;d.legend=b[q].legend;d.id=e+1;A=
!0;break}A||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=v,d.y_axis=void 0,d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=v,d.y_axis=void 0,d.order=x[e]&&""!=x[e]?parseInt(x[e]):parseInt(q+1),d.hide=k[e],d.plot_ci=h[e].trim(),d.show_signif=l[e],d.pch=t[e].trim(),d.type=B[e].trim(),d.lty=r[e].trim(),d.lwd=z[e].trim(),d.con_series=u[e].trim(),d.legend=y[e].trim(),d.color=p[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=v,d.y_axis=void 0,d.id=e+1,
d.order=e+1));e++;g.push(d)}if(null==initXML)for(p=rainbow(g.length),q=0;q<g.length;q++)g[q].color=p[q];g.sort(SortByOrder);for(q=0;q<g.length;q++)a.jqGrid("addRowData",q+1,g[q]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeries(a){var b=$("#listdt");b.saveCell(lastSelRow,lastSelCol);var c=b.jqGrid("getRowData");b.jqGrid("clearGridData");group_name_to_value_map={};var e,d=0,f={},g=$("#seriesLock").is(":checked"),h=[],k=$("#plot_data").multiselect("getChecked").val(),l,p,t=[],r=[],z=[],u=[],x=[],y=[],B=[],q=[],m=[],v=[],A=[];if(null!=initXML){for(var t=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),r=initXML.find("plot").find("plot_disp").text().replace("c(",
...
else for(m=[],n=0;n<u.length;n++)m.push(n+1);A=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(v.length!=m.length)for(n=0;n<m.length;n++)v[n]=""}for(var w=1;2>=w;w++){l="y"+w;1==w?(p=fcst_var_y1_indexes,e=permuteSeries(createSeriesMapForPermutation(series_var_y1_indexes,l),0)):(p=fcst_var_y2_indexes,e=permuteSeries(createSeriesMapForPermutation(series_var_y2_indexes,
l),0));for(var C=0;C<p.length;C++){var G=$("#fcst_var_"+l+"_"+p[C]).val(),H=[],D;D=$("#fcst_stat_"+l+"_"+p[C]).multiselect("getChecked");0==D.length&&"mode"==k&&(D=$("#fcst_stat_mode_"+l+"_"+p[C]).multiselect("getChecked"));for(n=0;n<D.length;n++)H.push(D[n].value);for(D=0;D<H.length;D++)for(var I=0;I<e.length;I++){n=H[D];"mode"==k&&-1==listStatModelRatio.indexOf(n)&&(n=createStatNameForModeArrt(n,l,p[C]));for(var F=e[I]+" "+G+" "+n,J=!1,n=0;n<c.length;n++)if(c[n].title==F&&c[n].y_axis.match(w+"$")==
w){f={};f.title=F;f.y_axis="Y"+w;f.order=d+1;f.hide=c[n].hide;f.plot_ci=c[n].plot_ci;f.show_signif=c[n].show_signif;f.color=c[n].color;f.pch=c[n].pch;f.type=c[n].type;f.lty=c[n].lty;f.lwd=c[n].lwd;f.con_series=c[n].con_series;f.legend=c[n].legend;f.id=d+1;J=!0;break}J||(g&&0<d&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=F,f.y_axis="Y"+w,f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=F,f.y_axis="Y"+w,f.order=parseInt(m[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=z[d],f.pch=
x[d].trim(),f.type=A[d].trim(),f.lty=y[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=v[d].trim(),f.color=u[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=F,f.y_axis="Y"+w,f.id=d+1,f.order=d+1));d++;h.push(f)}}}for(var c=[],E,n=0;n<seriesDiffY1.length;n++){k=seriesDiffY1[n].split(",")[0];e=seriesDiffY1[n].split(",")[1];w=seriesDiffY1[n].split(",")[2];p=l=!1;E='"and"';"DIFF"===w?E='"-"':"RATIO"===w&&(E='"/"');for(f=0;f<h.length;f++)k==h[f].title&&(l=!0),e==
h[f].title&&(p=!0);l&&p&&(c.push(seriesDiffY1[n]),k=w+' ("'+k+E+e+'")',g&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=k,f.y_axis="Y1",f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=k,f.y_axis="Y1",f.order=parseInt(m[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=z[d].trim(),f.pch=x[d].trim(),f.type=A[d].trim(),f.lty=y[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=v[d].trim(),f.color=u[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=
k,f.y_axis="Y1",f.id=d+1,f.order=d+1),d++,h.push(f))}seriesDiffY1=c;c=[];for(n=0;n<seriesDiffY2.length;n++){k=seriesDiffY2[n].split(",")[0];e=seriesDiffY2[n].split(",")[1];w=seriesDiffY1[n].split(",")[2];p=l=!1;"DIFF"===w?E='"-"':"RATIO"===w&&(E='"/"');for(f=0;f<h.length;f++)k==h[f].title&&(l=!0),e==h[f].title&&(p=!0);l&&p&&(c.push(seriesDiffY2[n]),k=w+' ("'+k+E+e+'")',g&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=k,f.y_axis="Y2",f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=
k,f.y_axis="Y2",f.order=parseInt(m[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=z[d],f.pch=x[d].trim(),f.type=A[d].trim(),f.lty=y[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=v[d].trim(),f.color=u[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=k,f.y_axis="Y2",f.id=d+1,f.order=d+1),d++,h.push(f))}seriesDiffY2=c;if(null==initXML){u=rainbow(h.length);d=[];if(a)d=u;else{for(n=0;n<u.length;n++)for(f=0;f<h.length;f++)h[f].color&&0<h[f].color.length&&h[f].color.substr(h[f].color.length-
6).toUpperCase()===u[n].substr(u[n].length-6).toUpperCase()&&n!=f&&(d[f]=u[n],d[n]=u[f]);for(n=0;n<u.length;n++)d[n]||(d[n]=u[n])}for(n=0;n<h.length;n++)h[n].color&&0!=h[n].color.length||(h[n].color=d[n])}h.sort(SortByOrder);for(n=0;n<h.length;n++)b.jqGrid("addRowData",n+1,h[n]);outerLayout.sizePane("south",$("#gbox_listdt").height())}function SortByOrder(a,b){return a.order-b.order}
...
c=createXMLCommon(c)):"Taylor"===currentTab&&(a=$("<template>taylor_plot.R_tmpl</template>"),c.append(a),c=createXMLTaylor(c),c=createXMLCommon(c));b.append(c);a=$("<root />").append(b).html().replace(/label=\"</g,'label="&lt;').replace(/label=\">/g,'label="&gt;');$.ajax({url:"servlet",processData:!1,contentType:"application/json",type:"POST",data:a,success:function(a){var b;try{b=$.parseXML(a);var c=$(b),g=c.find("plot");if(0==c.find("error").length){updateResult(g.text());var h=c.find("r_error");
h&&0<h.length&&(updateResult(g.text()),$("#r_error_message_text").empty().text(h.text()),$("#r_error_message").dialog("open"))}else updateResult(g.text()),$("#error_message_text").empty().text(c.find("error").text()),$("#error_message").dialog("open")}catch(k){$("#error_message_text").empty().text(a),$("#error_message").dialog("open")}},error:function(a){alert("error creating a plot or Proxy timeout: "+a)}})}
...
p=[],t=[],r=[],z=[],e=0;e<b.length;e++)c.push('"'+b[e].plot_ci+'"'),"No"==b[e].hide?d.push("TRUE"):d.push("FALSE"),"No"==b[e].show_signif?z.push("FALSE"):z.push("TRUE"),f.push('"'+b[e].color+'FF"'),g.push(b[e].pch),h.push('"'+b[e].type+'"'),k.push(b[e].lty),l.push(b[e].lwd),p.push(b[e].con_series),t.push(b[e].order),r.push('"'+b[e].legend+'"');a.append($("<plot_ci />").text("c("+c.join()+")"));a.append($("<show_signif />").text("c("+z.join()+")"));a.append($("<plot_disp />").text("c("+d.join()+")"));
a.append($("<colors />").text("c("+f.join()+")"));a.append($("<pch />").text("c("+g.join()+")"));a.append($("<type />").text("c("+h.join()+")"));a.append($("<lty />").text("c("+k.join()+")"));a.append($("<lwd />").text("c("+l.join()+")"));a.append($("<con_series />").text("c("+p.join()+")"));a.append($("<order_series />").text("c("+t.join()+")"));a.append($("<plot_cmd />").text($("#plot_cmd").val()));a.append($("<legend />").text("c("+r.join()+")"));0==$("#y1_lim_min").val().trim().length||0==$("#y1_lim_max").val().trim().length?
...
"none")}
function addFixedVarRhist(){var a=fixed_var_indexes[fixed_var_indexes.length-1],b=a=0<fixed_var_indexes.length?parseInt(fixed_var_indexes[fixed_var_indexes.length-1]):1,c="none"===$("#fixed_var_table").css("display"),e,d,f,g;c?(0===fixed_var_indexes.length&&fixed_var_indexes.push(b),$("#fixed_var_table").css("display",""),e=$("#fixed_var_"+b),$("#remove_fixed_var_"+b).button({icons:{primary:"ui-icon-trash"},text:!1}),d=$("#fixed_var_val_"+b),f=$("#fixed_var_val_date_period_button_"+b),g=$("#fixed_var_val_date_period_"+b)):
(b=parseInt(a)+1,fixed_var_indexes.push(b),e=$("#fixed_var_"+a).clone(!1),e.prop("id","fixed_var_"+b),e.css("display",""),e.val(e.find(" option:first").val()));c||(c=$("#remove_fixed_var_"+a).clone(!0).attr("id","remove_fixed_var_"+b),d=$("#fixed_var_val_"+a).clone(!1),d.attr("id","fixed_var_val_"+b),d.css("display",""),e.val(e.find(" option:first").val()),f=$("<button>",{text:"Select multiple options",id:"fixed_var_val_date_period_button_"+b}).css("display","none"),g=$("#fixed_var_val_date_period_"+
a).clone(!1).appendTo("body"),g.prop("id","fixed_var_val_date_period_"+b),g.find("#fixed_var_val_date_period_start_"+a).prop("id","fixed_var_val_date_period_start_"+b),g.find("#fixed_var_val_date_period_end_"+a).prop("id","fixed_var_val_date_period_end_"+b),g.find("#fixed_var_val_date_period_by_"+a).prop("id","fixed_var_val_date_period_by_"+b),g.find("#fixed_var_val_date_period_by_unit_"+a).prop("id","fixed_var_val_date_period_by_unit_"+b),$("#fixed_var_table").append($("<tr>").append($("<td>").append(c)).append($("<td>").append(e)).append($("<td>").append(d)).append($("<td>").append(f))));
createValDatePeriodDialog("fixed_var_val",b);d.multiselect({selectedList:100,noneSelectedText:"Select value",position:{my:"center center",at:"right center"}});e.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",height:"auto",click:function(a,d){$("#fixed_var_val_date_period_start_"+b).empty();$("#fixed_var_val_date_period_end_"+b).empty();"fcst_init_beg"===d.value||"fcst_valid_beg"===d.value||"fcst_valid"===d.value||"fcst_init"===d.value?$("#fixed_var_val_date_period_button_"+b).css("display",
"block"):$("#fixed_var_val_date_period_button_"+b).css("display","none");var c=this.id.split("_");updateFixedVarValHist(c[c.length-1],[])}});f.button({icons:{primary:"ui-icon-check",secondary:"ui-icon-circlesmall-plus"},text:!1}).click(function(){$("#fixed_var_val_date_period_"+b).dialog("open")});updateFixedVarValHist(b,[])}
function addFixedVar(){var a,b=a=0<fixed_var_indexes.length?parseInt(fixed_var_indexes[fixed_var_indexes.length-1]):1,c,e,d,f,g,h,k;(e="none"===$("#fixed_var_table").css("display"))?(0===fixed_var_indexes.length&&fixed_var_indexes.push(b),$("#fixed_var_table").css("display",""),c=$("#fixed_var_"+b),$("#remove_fixed_var_"+b).button({icons:{primary:"ui-icon-trash"},text:!1}),d=$("#fixed_var_val_"+b),f=$("#fixed_var_val_date_period_button_"+b),g=$("#fixed_var_val_date_period_"+b),$("#fix_var_event_equal_"+
...
a).clone(!1).appendTo("body"),g.prop("id","fixed_var_val_date_period_"+b),g.find("#fixed_var_val_date_period_start_"+a).prop("id","fixed_var_val_date_period_start_"+b),g.find("#fixed_var_val_date_period_end_"+a).prop("id","fixed_var_val_date_period_end_"+b),g.find("#fixed_var_val_date_period_by_"+a).prop("id","fixed_var_val_date_period_by_"+b),g.find("#fixed_var_val_date_period_by_unit_"+a).prop("id","fixed_var_val_date_period_by_unit_"+b),h=$("#fix_var_event_equal_"+a).clone(!0).prop("id","fix_var_event_equal_"+
b).prop("checked",!0),0<$("#fix_var_event_equal_"+a).find("label").size()&&(k=$('<label for="fix_var_event_equal_'+b+'">Equalize</label>')),$("body").append(g),$("#fixed_var_table").append($("<tr>").append($("<td>").append(e)).append($("<td>").append(c)).append($("<td>").append(d)).append($("<td>").append(f)).append($("<td>").append(h).append(k))));createValDatePeriodDialog("fixed_var_val",b);d.multiselect({selectedList:100,noneSelectedText:"Select value",position:{my:"center center",at:"right center"}});
c.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",height:"auto",click:function(a,d){$("#fixed_var_val_date_period_start_"+b).empty();$("#fixed_var_val_date_period_end_"+b).empty();"fcst_init_beg"===d.value||"fcst_valid_beg"===d.value||"fcst_valid"===d.value||"fcst_init"===d.value?$("#fixed_var_val_date_period_button_"+b).css("display","block"):$("#fixed_var_val_date_period_button_"+b).css("display","none");var c=this.id.split("_");updateFixedVarVal(c[c.length-1],[])}});f.button({icons:{primary:"ui-icon-check",
secondary:"ui-icon-circlesmall-plus"},text:!1}).click(function(){$("#fixed_var_val_date_period_"+b).dialog("open")});updateFixedVarVal(b,[])}
...
try{e.multiselect("uncheckAll")}catch(d){}b.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(){var a=this.id.split("_");updateSeriesVarValRhist(a[a.length-1],[])}})}
...
$("#series_var_val_y1_date_period_"+(a+1))):(b=$("#series_var_y1_"+a).clone(!1),b.attr("id","series_var_y1_"+(a+1)),b.css("display",""));0<a&&(c=$("#remove_series_var_y1_"+a).button("enable").clone(!0).attr("id","remove_series_var_y1_"+(a+1)),e=$("#series_var_val_y1_"+a).clone(!1),e.attr("id","series_var_val_y1_"+(a+1)),e.css("display",""),d=$("<button>",{text:"Select multiple options",id:"series_var_val_y1_date_period_button_"+(a+1)}).css("display","none"),f=$("#series_var_val_y1_date_period_"+a).clone(!1).appendTo("body"),
f.prop("id","series_var_val_y1_date_period_"+(a+1)),f.find("#series_var_val_y1_date_period_start_"+a).prop("id","series_var_val_y1_date_period_start_"+(a+1)),f.find("#series_var_val_y1_date_period_end_"+a).prop("id","series_var_val_y1_date_period_end_"+(a+1)),f.find("#series_var_val_y1_date_period_by_"+a).prop("id","series_var_val_y1_date_period_by_"+(a+1)),f.find("#series_var_val_y1_date_period_by_unit_"+a).prop("id","series_var_val_y1_date_period_by_unit_"+(a+1)),$("body").append(f),$("#series_var_table_y1").append($("<tr>").append($("<td>").append(c)).append($("<td>").append(b)).append($("<td>").append(e)).append($("<td>").append(d))));
createValDatePeriodDialog("series_var_val_y1",a+1);e.multiselect({selectedList:100,noneSelectedText:"Select value",click:function(){updateSeriesRhist()},checkAll:function(){updateSeriesRhist()},uncheckAll:function(){updateSeriesRhist()}});try{e.multiselect("uncheckAll")}catch(g){}b.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(b,d){$("#series_var_val_y1_date_period_start_"+(a+1)).empty();$("#series_var_val_y1_date_period_end_"+(a+1)).empty();"fcst_init_beg"==d.value||
"fcst_valid_beg"==d.value||"fcst_valid"==d.value||"fcst_init"==d.value?$("#series_var_val_y1_date_period_button_"+(a+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+(a+1)).css("display","none");var c=this.id.split("_");updateSeriesVarValRhist(c[c.length-1],[])}});d.button({icons:{primary:"ui-icon-check",secondary:"ui-icon-circlesmall-plus"},text:!1}).click(function(){$("#series_var_val_y1_date_period_"+(a+1)).dialog("open")})}
function addSeriesVar(a){var b;"y1"==a?(b=0<series_var_y1_indexes.length?series_var_y1_indexes[series_var_y1_indexes.length-1]:0,series_var_y1_indexes.push(b+1)):(b=0<series_var_y2_indexes.length?series_var_y2_indexes[series_var_y2_indexes.length-1]:0,series_var_y2_indexes.push(b+1));var c,e,d,f,g,h,k;0==b?($("#series_var_table_"+a).css("display",""),c=$("#series_var_"+a+"_"+(b+1)),$("#remove_series_var_"+a+"_"+(b+1)).button({icons:{primary:"ui-icon-trash"},text:!1}).click(function(){removeSeriesVar($(this).attr("id"))}),
...
createValDatePeriodDialog("series_var_val_"+a,b+1);d.multiselect({selectedList:100,noneSelectedText:"Select value",click:function(){updateSeries()},checkAll:function(){updateSeries(!0)},uncheckAll:function(){updateSeries()}});try{d.multiselect("uncheckAll")}catch(l){}c.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(d,c){$("#series_var_val_"+a+"_date_period_start_"+(b+1)).empty();$("#series_var_val_"+a+"_date_period_end_"+(b+1)).empty();"fcst_init_beg"==c.value||"fcst_valid_beg"==
c.value||"fcst_valid"==c.value||"fcst_init"==c.value?$("#series_var_val_"+a+"_date_period_button_"+(b+1)).css("display","block"):$("#series_var_val_"+a+"_date_period_button_"+(b+1)).css("display","none");var e=this.id.split("_");updateSeriesVarVal(e[e.length-2],e[e.length-1],[])}});h.button({icons:{primary:"ui-icon-check",secondary:"ui-icon-circlesmall-plus"},text:!1}).click(function(){$("#series_var_val_"+a+"_date_period_"+(b+1)).dialog("open")})}
...
click:function(a,b){var d=this.id.split("_");$("#fcst_stat_mode_config_"+d[d.length-2]+"_"+d[d.length-1]).css("display","none");try{$("#fcst_stat_mode_"+d[d.length-2]+"_"+d[d.length-1]).multiselect("uncheckAll")}catch(c){console.log("Error "+c)}updateSeries()},checkAll:function(){updateSeries()},uncheckAll:function(){updateSeries()}}),"N/A"==$("#fcst_var_"+a+"_"+(b+1)).multiselect("getChecked").val()?f.multiselect("disable"):f.multiselect("enable"),g.multiselect({multiple:!1,selectedList:1,header:!1,
minWidth:225,height:300,noneSelectedText:"Select attribute stat",allUnselected:!0,position:{my:"right center",at:"right center"},click:function(a,b){var d=this.id.split("_");try{$("#fcst_stat_"+d[d.length-2]+"_"+d[d.length-1]).multiselect("uncheckAll")}catch(c){}d=$("#fcst_stat_mode_config_"+d[d.length-2]+"_"+d[d.length-1]);d.css("display","block");"ACOV"==b.value?d.find(".non-acov").attr("disabled",!0):d.find(".non-acov").removeAttr("disabled");-1<listStatModePair.indexOf(b.value)?d.find(".non-pair").attr("disabled",
!0):d.find(".non-pair").removeAttr("disabled");updateSeries()}}))}
function loadXMLRoc(){var a;if(0<initXML.find("plot").find("series1").children().length)for(var b=initXML.find("plot").find("series1").children(),c=0;c<b.length;c++){a=[];addSeriesVarRhist();var e=$(b[c]).attr("name");try{$("#series_var_y1_"+(c+1)).val(e).multiselect("refresh")}catch(d){}$(b[c]).find("val").each(function(){a.push($(this).text())});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#series_var_val_y1_date_period_button_"+(c+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+
...
function loadXMLRhist(){var a;if(0<initXML.find("plot").find("series1").children().length)for(var b=initXML.find("plot").find("series1").children(),c=0;c<b.length;c++){a=[];addSeriesVarRhist();var e=$(b[c]).attr("name");try{$("#series_var_y1_"+(c+1)).val(e).multiselect("refresh")}catch(d){}$(b[c]).find("val").each(function(){for(var b=$(this).text().split(","),c=0;c<b.length;c++)a.push(b[c])});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#series_var_val_y1_date_period_button_"+
...
function loadXMLRely(){var a;if(0<initXML.find("plot").find("series1").children().length)for(var b=initXML.find("plot").find("series1").children(),c=0;c<b.length;c++){a=[];addSeriesVarRhist();var e=$(b[c]).attr("name");try{$("#series_var_y1_"+(c+1)).val(e).multiselect("refresh")}catch(d){}$(b[c]).find("val").each(function(){a.push($(this).text())});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#series_var_val_y1_date_period_button_"+(c+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+
...
for(c=0;c<b.length;c++)$("#summary_curve option[value='"+b[c]+"']").prop("selected",!0);try{$("#summary_curve").multiselect("refresh")}catch(d){}}}
...
function updatePlotFix(){if(0<$(initXML.find("plot").find("plot_fix")).children().length)for(var a=$(initXML.find("plot").find("plot_fix")).children(),b=0;b<a.length;b++){var c=[];0<b&&addFixedVarRhist();var e=$(a[b]).attr("name");try{$("#fixed_var_"+(b+1)).val(e).multiselect("refresh")}catch(d){}$(a[b]).find("set").find("val").each(function(){c.push($(this).text())});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#fixed_var_val_date_period_button_"+(b+1)).css("display",
...
a+'">'+b+"</option>")}):$.each(fix_var_value_to_title_mode_map,function(a,b){$("#fixed_var_1").append('<option value="'+a+'">'+b+"</option>")});var c,e;e="Perf"===currentTab||"Taylor"===currentTab?1:2;for(var d=1;d<=e;d++){var f="y"+d,g=0;if(0<initXML.find("plot").find("dep").find("dep"+d).children().length)for(var h=initXML.find("plot").find("dep").find("dep"+d).children(),a=0;a<h.length;a++){var k=[];c=$(h[a]).attr("name");0===g&&1===d||addFcstVar(f);try{$("#fcst_var_"+f+"_"+(g+1)).val(c).multiselect("refresh")}catch(r){}$(h[a]).find("stat").each(function(){k.push($(this).text())});
if("stat"===b)updateStats(f,g+1,k),g++;else if($.isArray(k))for(c=0;c<k.length;c++){if(0<c){addFcstVar(f);try{$("#fcst_var_"+f+"_"+(g+1)).val($(h[a]).attr("name")).multiselect("refresh")}catch(r){}}updateMode(f,g+1,k[c]);g++}else updateMode(f,g+1,k),g++}else"stat"===b?updateStats(f,1,""):updateMode(f,1,"");var l,p;if(0<initXML.find("plot").find("series"+d).children().length)for(g=initXML.find("plot").find("series"+d).children(),a=0;a<g.length;a++){l=[];p=!1;c=$(g[a]).attr("name");if(0!==a||1!==d)"Taylor"===
currentTab?addSeriesVarRhist():addSeriesVar(f);try{$("#series_var_"+f+"_"+(a+1)).val(c).multiselect("refresh")}catch(r){}$(g[a]).find("val").each(function(){var a=$(this).text().split(",");1<a.length&&(p=!0);for(var b=0;b<a.length;b++)l.push(a[b])});"fcst_init_beg"==c||"fcst_valid_beg"==c||"fcst_valid"==c||"fcst_init"==c?$("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","block"):$("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","none");updateSeriesVarVal(f,a+1,l);
p&&$("#group_series_var_"+f+"_"+(a+1)).prop("checked",!0)}else l=$("#series_var_"+f+"_1").first().val(),updateSeriesVarVal(f,1,l)}updatePlotFixSeries();$("#txtPlotCond").val(initXML.find("plot").find("plot_cond").text());"mode"===b&&updateIndyVar(b);c=$(initXML.find("plot").find("indep")[0]).attr("name");try{$("#indy_var").val(c).multiselect("refresh")}catch(r){}"fcst_init_beg"==c||"fcst_valid_beg"==c||"fcst_valid"==c||"fcst_init"==c?$("#date_period_button").css("display","block"):$("#date_period_button").css("display",
...
a+"_"+b).multiselect("refresh")}catch(t){console.log(t)}"fixed_var_val"!==a&&("Series"===currentTab||"Box"===currentTab||"Bar"===currentTab?updateSeries():"Rhist"===currentTab||"Phist"===currentTab||"Roc"===currentTab||"Rely"===currentTab?updateSeriesRhist():(currentTab="Ens_ss",updateSeriesEns()))}$(this).dialog("close")}},open:function(d,f){var g;"fixed_var_val"==a?g=$(fixVarValResponse[b]).find("val"):"series_var_val_y1"==a?g=$(seriesY1VarValResponse[b]).find("val"):"series_var_val_y2"==a&&(g=
...
"min"==g?f*=60:"hours"==g?f*=3600:"days"==g&&(f*=86400);g=$(previousIndVarValResponse).find("val");for(e=e.clone();e<=d;){for(var h=e.format("YYYY-MM-DD HH:mm:ss"),k=!1,l=0;l<g.length;l++)if($(g[l]).text()==h){k=!0;break}k&&c.find('option[value="'+h+'"]').prop("selected",!0);e.add(f,"seconds")}try{c.multiselect("refresh")}catch(p){}}$(this).dialog("close")}},open:function(e,d){var f=c.val();null==f&&(f=[]);populateIndyVarVal(f);var f=$(previousIndVarValResponse).find("val"),g=a.val(),h=b.val();a.empty();
b.empty();for(var k=0;k<f.length;k++){var l=$(f[k]),p=$("<option/>").attr("value",l.text()).text(l.text());l.text()==g&&p.prop("selected",!0);a.append(p);l.text()==h?p.prop("selected",!0):p.prop("selected",!1);b.append(p.clone())}b.find("option").last().prop("selected",!0);1<f.length&&(g=moment($(f[0]).text(),"YYYY-MM-DD HH:mm:ss"),f=moment($(f[1]).text(),"YYYY-MM-DD HH:mm:ss"),f=moment.duration({from:g,to:f}),$("#date_period_by").val(f.as("hours")))}})}
