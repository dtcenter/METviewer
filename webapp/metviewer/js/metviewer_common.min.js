...
c+"</db_con><list_val><"+f+"_field>"+h+"</"+f+"_field>"+g+"</list_val></request>",dataType:"xml",processData:!1,contentType:"application/xml",error:function(){},success:function(c){"y1"===a?seriesY1VarValResponse[b]=c:seriesY2VarValResponse[b]=c;var f=$(c).find("val");c=[];if(0<f.length){for(var g=0;g<f.length;g++){var k=$(f[g]);var h="string"===$.type(d)?k.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&")===d:0<=$.inArray(k.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;",
"&"),d);0===g||0!==g&&k.text()!==$(f[g-1]).text()?(h=$("<option />",{value:k.text(),text:k.text(),selected:h}),c.push(h)):0!==g&&k.text()===$(f[g-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}h=$("input[name='multiselect_database']:checked").length-1;for(g=0;g<c.length;g++)c[g].text()!==c[g][0].value&&(f=c[g].text().substring(c[g][0].value.length,c[g].text().length).length,f===h?c[g].text(c[g][0].value+"*"):c[g].text(c[g][0].value)),c[g].appendTo(e);try{e.multiselect("refresh")}catch(u){}}else{h=
...
"Eclv"===currentTab&&(f="eclv");$.ajax({async:!1,url:"servlet",type:"POST",processData:!1,contentType:"application/xml",dataType:"xml",data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+e+"</db_con><list_val><"+f+"_field>"+c+"</"+f+'_field><stat><fcst_var name="NA"><val>NA</val></fcst_var></stat></list_val></request>',error:function(){},success:function(c){seriesY1VarValResponse[a]=c;var e=$(c).find("val");c=[];if(0<e.length){0<b.length&&$("#series_var_table_y"+a).css("display","");for(var f=
0;f<e.length;f++){var g=$(e[f]);var m="string"===$.type(b)?g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&")===b:0<=$.inArray(g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&"),b);0===f||0!==f&&g.text()!==$(e[f-1]).text()?(m=$("<option />",{value:g.text(),text:g.text(),selected:m}),c.push(m)):0!==f&&g.text()===$(e[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}m=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==
...
function loadXMLRoc(){$("#event_equal").prop("checked","true"==$(initXML.find("plot").find("event_equal")).text()).trigger("change");if(0<initXML.find("plot").find("series1").children().length)for(var a=initXML.find("plot").find("series1").children(),b=0;b<a.length;b++){var d=[];var e=$(a[b]).attr("name");try{$("#series_var_y1_"+(b+1)).val(e).multiselect("refresh")}catch(c){}$(a[b]).find("val").each(function(){d.push($(this).text())});"fcst_init_beg"===e||"fcst_valid_beg"===e||"fcst_valid"===e||"fcst_init"===
e?$("#series_var_val_y1_date_period_button_"+(b+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+(b+1)).css("display","none");updateSeriesVarValHist(b+1,d);0===b&&($("#series_var_val_y1_1").multiselect({selectedList:100,noneSelectedText:"Select value",click:function(){updateSeriesHist()},checkAll:function(){updateSeriesHist()},uncheckAll:function(){updateSeriesHist()}}),$("#series_var_y1_1").multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(a,b){$("#series_var_val_y1_date_period_start_"+
...
text:!1}).click(function(){$("#series_var_val_y1_date_period_1").dialog("open")}));b<a.length-1&&addSeriesVarHist()}else d=$("#series_var_y1_1").first().val(),updateSeriesVarValHist(1,d);updatePlotFix();b=$(initXML.find("plot").find("roc_calc").find("roc_pct")).text();a=$(initXML.find("plot").find("roc_calc").find("roc_ctc")).text();$("input[name=roc_type][value=pct]").prop("checked","TRUE"==b);$("input[name=roc_type][value=ctc]").prop("checked","TRUE"==a);$("#roc_type").buttonset("refresh");if(initXML.find("plot").find("summary_curve")&&
