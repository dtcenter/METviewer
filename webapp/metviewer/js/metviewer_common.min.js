...
data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+d+"</db_con><list_val><"+selected_mode+"_field>FCST_VAR</"+selected_mode+"_field></list_val></request>",error:function(){},success:function(c){var d=$(c).find("val");if(0<d.length){c=[];for(var e=0;e<d.length;e++){var h=$(d[e]);if(0===e||0!==e&&h.text()!==$(d[e-1]).text()){var l=h.text().formatAll();h=$("<option />",{value:l,text:l,title:value_to_desc_map[h.text()]});c.push(h)}else 0!==e&&h.text()===$(d[e-1]).text()&&c[c.length-1].text(c[c.length-
1].text()+"*")}h=$("input[name='multiselect_database']:checked").length-1;for(e=0;e<c.length;e++)c[e].text()!==c[e][0].value&&(d=c[e].text().substring(c[e][0].value.length,c[e].text().length).length,d===h?c[e].text(c[e][0].value+"*"):c[e].text(c[e][0].value)),c[e].appendTo(a),c[e].clone().appendTo(b);try{a.multiselect("refresh")}catch(n){}try{b.multiselect("refresh")}catch(n){}}else"stat"===selected_mode?$("#plot_data").val("mode"):"mode"===selected_mode?$("#plot_data").val("mtd"):"mtd"===selected_mode&&
...
$("#fcst_stat_mode_"+a+"_"+b),n="",m="";0<d.length&&2==d.split("_").length&&(n=d.split("_")[0],m=d.split("_")[1]);"ACOVACOV"==n&&(n="ACOV");d=!1;l.empty();try{l.multiselect("destroy")}catch(q){}for(c=0;c<listStatMode.length;c++)g=listStatMode[c],(h=g==n)&&(d=!0),g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(l);l.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:225,height:300,noneSelectedText:"Select attribute stat",allUnselected:!d,click:function(a,
...
$("#fcst_stat_mode_"+a+"_"+b),n="",m="";if(0<d.length){m=d.split("_");for(c=0;c<m.length-1;c++)n+=m[c],c!==m.length-2&&(n+="_");m=m[m.length-1]}d=!1;l.empty();try{l.multiselect("destroy")}catch(q){}for(c=0;c<listStatMtd.length;c++)g=listStatMtd[c],(h=g===n)&&(d=!0),g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(l);l.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:225,height:300,noneSelectedText:"Select attribute stat",allUnselected:!d,click:function(a,
...
[]}})}var l=$("#series_var_"+a+"_"+b);l.empty();$.each(series_var_value_to_title_stat_map,function(a,b){l.append('<option value="'+a+'">'+b+"</option>")});try{l.multiselect("refresh")}catch(n){}}}
...
error:function(){},success:function(c){seriesY1VarValResponse[a]=c;var e=$(c).find("val");c=[];if(0<e.length){for(var f=0;f<e.length;f++){var l=$(e[f]);var n=0<=$.inArray(l.text(),b);0==f||0!=f&&l.text()!==$(e[f-1]).text()?(l=l.text().formatAll(),n=$("<option />",{value:l,text:l,selected:n}),c.push(n)):0!=f&&l.text()===$(e[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}n=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(e=c[f].text().substring(c[f][0].value.length,
c[f].text().length).length,e===n?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(d);try{d.multiselect("refresh")}catch(m){}}else{n=$("<option />",{value:"N/A",text:"N/A"});n.appendTo(d);try{d.multiselect("refresh")}catch(m){}}b=[]}})}}
...
c+"</db_con><list_val><"+f+"_field>"+h+"</"+f+"_field>"+g+"</list_val></request>",dataType:"xml",processData:!1,contentType:"application/xml",error:function(){},success:function(c){"y1"===a?seriesY1VarValResponse[b]=c:seriesY2VarValResponse[b]=c;var g=$(c).find("val");c=[];if(0<g.length){for(var f=0;f<g.length;f++){var l=$(g[f]);var h="string"===$.type(d)?l.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&")===d:0<=$.inArray(l.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;",
"&"),d);0===f||0!==f&&l.text()!==$(g[f-1]).text()?(h=$("<option />",{value:l.text(),text:l.text(),selected:h}),c.push(h)):0!==f&&l.text()===$(g[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}h=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(g=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,g===h?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(u){}}else{h=
...
d+"</db_con><list_val><stat_field>"+e+"</stat_field>"+c+"</list_val></request>",error:function(){try{b.multiselect("refresh")}catch(f){}},success:function(c){var d=$(c).find("val");c=[];if(0<d.length){for(var e=0;e<d.length;e++){var f=$(d[e]);var n=0<=$.inArray(f.text(),a);0==e||0!=e&&f.text()!==$(d[e-1]).text()?(f=f.text().formatAll(),n=$("<option />",{value:f,text:f,selected:n}),c.push(n)):0!=e&&f.text()===$(d[e-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}n=$("input[name='multiselect_database']:checked").length-
...
error:function(){try{e.multiselect("refresh")}catch(h){}},success:function(c){fixVarValResponse[a]=c;var d=$(c).find("val");c=[];if(0<d.length){for(var f=0;f<d.length;f++){var g=$(d[f]);var q=0<=$.inArray(g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&"),b);0===f||0!==f&&g.text()!==$(d[f-1]).text()?(g=g.text().formatAll(),q=$("<option />",{value:g,text:g,selected:q}),c.push(q)):0!==f&&g.text()===$(d[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}q=$("input[name='multiselect_database']:checked").length-
...
function createSeriesMapForPermutationEns(a,b){for(var d={},e,c,f=a.length-1;0<=f;f--){var g=[];try{c=$("#series_var_"+b+"_"+a[f]).multiselect("getChecked")[0].value}catch(l){c=$("#series_var_"+b+"_"+a[f]).val()}try{e=$("#series_var_val_"+b+"_"+a[f]).multiselect("getChecked")}catch(l){e=[]}for(var h=0;h<e.length;h++)g.push(e[h].value);h=d[c];void 0!=h&&(g=h.concat(g));d[c]=g}return d}
function updateSeriesHist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d=0,e={},c=$("#seriesLock").is(":checked"),f=[],g=[],h=[],l=[],n=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A;if("Hist"===currentTab){var p=$("input:radio[name='hist_line_type']:checked").val();"rhist"===p?A="Rank Histogram":"phist"===p?A="Probability Histogram":"relp"===p&&(A="Relative Histogram")}else"Roc"===currentTab?A="ROC Curve":"Rely"===currentTab?A="Reliability Curve":
...
u=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(v.length!=x.length)for(p=0;p<x.length;p++)v[p]=""}p=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,
"y1"),0);0==p.length&&(p[0]="");if("Roc"==currentTab||"Rely"==currentTab||"Eclv"==currentTab){try{var r=$("#summary_curve").multiselect("getChecked")}catch(w){r=[]}for(k=0;k<r.length;k++)"none"!=r[k].value&&p.push(r[k].value)}for(var y=0;y<p.length;y++){r=p[y]+" "+A;var B=!1;for(k=0;k<b.length;k++)if(b[k].title==r){e={};e.title=r;e.y_axis="Y1";e.order=d+1;e.hide=b[k].hide;e.plot_ci=b[k].plot_ci;e.show_signif=b[k].show_signif;e.color=b[k].color;e.pch=b[k].pch;e.type=b[k].type;e.lty=b[k].lty;e.lwd=
b[k].lwd;e.con_series=b[k].con_series;e.legend=b[k].legend;e.id=d+1;B=!0;break}B||(c&&0<d&&null==initXML?(e=jQuery.extend(!0,{},f[f.length-1]),e.title=r,e.y_axis="Y1",e.id=d+1,e.order=d+1):null!=initXML?(e={},e.title=r,e.y_axis="Y1",e.order=x[d]&&""!=x[d]?parseInt(x[d]):parseInt(k+1),e.hide=h[d],e.plot_ci=g[d].trim(),e.show_signif=l[d],e.pch=m[d].trim(),e.type=z[d].trim(),e.lty=q[d].trim(),e.lwd=t[d].trim(),e.con_series=u[d].trim(),e.legend=v[d].trim(),e.color=n[d].trim(),e.id=d+1):(e=jQuery.extend(!0,
{},firstSeriesFormatting),e.title=r,e.y_axis="Y1",e.id=d+1,e.order=d+1));d++;f.push(e)}if(null==initXML)for(n=rainbow(f.length),k=0;k<f.length;k++)f[k].color=n[k];f.sort(SortByOrder);for(k=0;k<f.length;k++)a.jqGrid("addRowData",k+1,f[k]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
...
function updateSeriesEns(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d,e=0,c={},f=$("#seriesLock").is(":checked"),g=[],h=[],l=[],n=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A=[];if(null!=initXML){h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");l=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");n=initXML.find("plot").find("show_signif").text().replace("c(",
...
"").replace(/\)$/,"").replace(/"/g,"").split(",");if(z.length!=v.length)for(d=0;d<v.length;d++)z[d]=""}d=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);if("true"===$("input:radio[name='ensss_pts_disp']:checked").val()){var k=[];var r=0;for(p=0;p<d.length;p++)k[r]=d[p]+" MSE",k[r+1]=d[p]+" #PTS",r+=2;d=k}else for(p=0;p<d.length;p++)d[p]+=" MSE";for(r=0;r<d.length;r++){var y=d[r],B=!1;k=0<y.indexOf("#PTS")?"Y2":"Y1";for(p=0;p<b.length;p++)if(b[p].title==y){c={};c.title=
y;c.y_axis=k;c.order=e+1;c.hide=b[p].hide;c.plot_ci=b[p].plot_ci;c.show_signif=b[p].show_signif;c.color=b[p].color;c.pch=b[p].pch;c.type=b[p].type;c.lty=b[p].lty;c.lwd=b[p].lwd;c.con_series=b[p].con_series;c.legend=b[p].legend;c.id=e+1;B=!0;break}B||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=y,c.y_axis=k,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=y,c.y_axis=k,c.order=v[e]&&""!=v[e]?parseInt(v[e]):parseInt(p+1),c.hide=l[e],c.plot_ci=h[e].trim(),c.show_signif=n[e],c.pch=
q[e].trim(),c.type=A[e].trim(),c.lty=t[e].trim(),c.lwd=u[e].trim(),c.con_series=x[e].trim(),c.legend=z[e].trim(),c.color=m[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=y,c.y_axis=k,c.id=e+1,c.order=e+1));e++;g.push(c)}if(null==initXML)for(m=rainbow(g.length),p=0;p<g.length;p++)g[p].color=m[p];g.sort(SortByOrder);for(p=0;p<g.length;p++)a.jqGrid("addRowData",p+1,g[p]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesPerf(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d,e=0,c={},f=$("#seriesLock").is(":checked"),g=[],h=[],l=[],n=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A=[];if(null!=initXML){h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");l=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");n=initXML.find("plot").find("show_signif").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");for(var p=0;p<l.length;p++)"TRUE"==l[p].trim()?l[p]="No":l[p]="Yes","TRUE"==n[p].trim()?n[p]="No":n[p]="Yes";m=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(p=0;p<m.length;p++)m[p]=m[p].replace(/FF$/,"");q=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");t=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,
"").split(",");u=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("legend").text().replace("c(",
"").replace(/\)$/,"").replace(/"/g,"").split(",");if(z.length!=v.length)for(d=0;d<v.length;d++)z[d]=""}d=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var k=0;k<d.length;k++){var r=d[k],y=!1;for(p=0;p<b.length;p++)if(b[p].title==r){c={};c.title=r;c.y_axis="";c.order=e+1;c.hide=b[p].hide;c.plot_ci=b[p].plot_ci;c.show_signif=b[p].show_signif;c.color=b[p].color;c.pch=b[p].pch;c.type=b[p].type;c.lty=b[p].lty;c.lwd=b[p].lwd;c.con_series=b[p].con_series;c.legend=b[p].legend;
c.id=e+1;y=!0;break}y||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=r,c.y_axis=void 0,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=r,c.y_axis=void 0,c.order=v[e]&&""!=v[e]?parseInt(v[e]):parseInt(p+1),c.hide=l[e],c.plot_ci=h[e].trim(),c.show_signif=n[e],c.pch=q[e].trim(),c.type=A[e].trim(),c.lty=t[e].trim(),c.lwd=u[e].trim(),c.con_series=x[e].trim(),c.legend=z[e].trim(),c.color=m[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=r,c.y_axis=void 0,
...
function updateSeries(a){var b=$("#listdt");b.saveCell(lastSelRow,lastSelCol);var d=b.jqGrid("getRowData");b.jqGrid("clearGridData");group_name_to_value_map={};var e=0,c={},f=$("#seriesLock").is(":checked"),g=[],h=$("#plot_data").multiselect("getChecked").val(),l=[],n=[],m=[],q=[],t=[],u=[],x=[],v=[],z=[],A=[],p=[];if(null!=initXML){l=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");n=initXML.find("plot").find("plot_disp").text().replace("c(",
...
else for(z=[],k=0;k<q.length;k++)z.push(k+1);p=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(A.length!=z.length)for(k=0;k<z.length;k++)A[k]=""}for(var r=1;2>=r;r++){var y="y"+r;if(1==r){var B=fcst_var_y1_indexes;var w=permuteSeries(createSeriesMapForPermutation(series_var_y1_indexes,y),0)}else B=fcst_var_y2_indexes,w=permuteSeries(createSeriesMapForPermutation(series_var_y2_indexes,
y),0);for(var G=0;G<B.length;G++){var L=$("#fcst_var_"+y+"_"+B[G]).val(),J=[];var D=$("#fcst_stat_"+y+"_"+B[G]).multiselect("getChecked");var E=[];if("Contour"==currentTab){for(k=0;k<fixed_var_indexes.length;k++){var F=$("#fixed_var_val_"+fixed_var_indexes[k]).val();if(F)for(w=0;w<F.length;w++)E.push({value:F[w]})}w=[""]}E&&0!=E.length||(E=[{value:""}]);0!=D.length||"mode"!=h&&"mtd"!=h||(D=$("#fcst_stat_mode_"+y+"_"+B[G]).multiselect("getChecked"));for(k=0;k<D.length;k++)J.push(D[k].value);for(D=
0;D<J.length;D++)for(F=0;F<w.length;F++){k=J[D];if("mode"==h&&-1==listStatModeRatio.indexOf(k)||"mtd"==h&&-1==listStatMtdRatio.indexOf(k))k=createStatNameForModeArrt(k,y,B[G]);for(var M=w[F]+" "+L+" "+k,I=0;I<E.length;I++){var C=M;-1<C.indexOf("<")&&(C=C.replace(/</g,"&lt;"));0<E[I].value.length&&(C=C+" "+E[I].value);var K=!1;for(k=0;k<d.length;k++)if(d[k].title==C&&d[k].y_axis.match(r+"$")==r){c={};c.title=C;c.y_axis="Y"+r;c.order=e+1;c.hide=d[k].hide;c.plot_ci=d[k].plot_ci;c.show_signif=d[k].show_signif;
c.color=d[k].color;c.pch=d[k].pch;c.type=d[k].type;c.lty=d[k].lty;c.lwd=d[k].lwd;c.con_series=d[k].con_series;c.legend=d[k].legend;c.id=e+1;K=!0;break}K||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=C,c.y_axis="Y"+r,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=C,c.y_axis="Y"+r,c.order=parseInt(z[e]),c.hide=n[e],c.plot_ci=l[e].trim(),c.show_signif=m[e],c.pch=t[e].trim(),c.type=p[e].trim(),c.lty=u[e].trim(),c.lwd=x[e].trim(),c.con_series=v[e].trim(),c.legend=A[e].trim(),
c.color=q[e].trim(),c.id=e+1):(c=f&&0<d.length?jQuery.extend(!0,{},d[d.length-1]):jQuery.extend(!0,{},firstSeriesFormatting),c.title=C,c.y_axis="Y"+r,c.id=e+1,c.order=e+1));"Contour"!==currentTab&&e++;g.push(c)}}}}d=[];for(k=0;k<seriesDiffY1.length;k++){c=seriesDiffY1[k].split(",")[0];h=seriesDiffY1[k].split(",")[1];r=seriesDiffY1[k].split(",")[2];B=y=!1;var H='"and"';"DIFF"===r?H='"-"':"RATIO"===r&&(H='"/"');for(w=0;w<g.length;w++)c==g[w].title&&(y=!0),h==g[w].title&&(B=!0);y&&B&&(d.push(seriesDiffY1[k]),
h=r+' ("'+c+H+h+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=h,c.y_axis="Y1",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=h,c.y_axis="Y1",c.order=parseInt(z[e]),c.hide=n[e],c.plot_ci=l[e].trim(),c.show_signif=m[e].trim(),c.pch=t[e].trim(),c.type=p[e].trim(),c.lty=u[e].trim(),c.lwd=x[e].trim(),c.con_series=v[e].trim(),c.legend=A[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=h,c.y_axis="Y1",c.id=e+1,c.order=e+1),e++,g.push(c))}seriesDiffY1=
d;d=[];for(k=0;k<seriesDiffY2.length;k++){c=seriesDiffY2[k].split(",")[0];h=seriesDiffY2[k].split(",")[1];r=seriesDiffY1[k].split(",")[2];B=y=!1;"DIFF"===r?H='"-"':"RATIO"===r&&(H='"/"');for(w=0;w<g.length;w++)c==g[w].title&&(y=!0),h==g[w].title&&(B=!0);y&&B&&(d.push(seriesDiffY2[k]),h=r+' ("'+c+H+h+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=h,c.y_axis="Y2",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=h,c.y_axis="Y2",c.order=parseInt(z[e]),c.hide=n[e],c.plot_ci=l[e].trim(),
c.show_signif=m[e],c.pch=t[e].trim(),c.type=p[e].trim(),c.lty=u[e].trim(),c.lwd=x[e].trim(),c.con_series=v[e].trim(),c.legend=A[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=h,c.y_axis="Y2",c.id=e+1,c.order=e+1),e++,g.push(c))}seriesDiffY2=d;for(k=0;k<lines.length;k++)c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=lines[k].type+"("+lines[k].line_pos+")",c.y_axis="Y1",c.order="",c.id=e+1,e++,g.push(c);if(null==initXML){if("Contour"===currentTab)for(k=
0;k<g.length;k++)q.push("#000000");else q=rainbow(g.length);e=[];if(a)e=q;else{for(k=0;k<q.length;k++)for(w=0;w<g.length;w++)g[w].color&&0<g[w].color.length&&g[w].color.substr(g[w].color.length-6).toUpperCase()===q[k].substr(q[k].length-6).toUpperCase()&&k!=w&&(e[w]=q[k],e[k]=q[w]);for(k=0;k<q.length;k++)e[k]||(e[k]=q[k])}for(k=0;k<g.length;k++)g[k].color&&0!=g[k].color.length||(g[k].color=e[k])}"Contour"===currentTab?g.reverse():g.sort(SortByOrder);for(k=0;k<g.length;k++)b.jqGrid("addRowData",k+
1,g[k]),"Contour"===currentTab&&0!==k&&$("#"+(k+1),"#gbox_listdt").css({display:"none"});outerLayout.sizePane("south",$("#gbox_listdt").height())}function SortByOrder(a,b){return a.order-b.order}function rainbow(a){if(1>a)return[];if(1==a)return["#FF0000"];for(var b=[],d=1/(a-1),e=0,c=0;c<a;c++,e+=d)b.push("#"+interpolateColor(e));return b}
...
a.append($("<caption_offset />").text($("#caption_offset").val()));a.append($("<caption_align />").text($("#caption_align").val()));a.append($("<ci_alpha />").text($("#ci_alpha").val()));a.append($("<eqbound_low />").text($("#eqbound_low").val()));a.append($("<eqbound_high />").text($("#eqbound_high").val()));var g=sortSeries();b=[];d=[];c=[];f=[];var h=[],l=[],n=[],m=[],q=[],t=[],u=[],x=[];for(e=0;e<g.length;e++)if(""!==g[e].order)b.push('"'+g[e].plot_ci+'"'),"No"===g[e].hide?d.push("TRUE"):d.push("FALSE"),
"No"===g[e].show_signif?u.push("FALSE"):u.push("TRUE"),c.push('"'+g[e].color+'FF"'),f.push(g[e].pch),h.push('"'+g[e].type+'"'),l.push(g[e].lty),n.push(g[e].lwd),m.push(g[e].con_series),q.push(g[e].order),t.push('"'+g[e].legend+'"');else{var v=g[e].title,z=v.split("(")[0];v=v.split("(")[1].split(")")[0];x.push($("<line />").attr("type",z).attr("line_pos",v).attr("color",g[e].color).attr("lty",g[e].lty).attr("lwd",g[e].lwd))}if(0<x.length){e=$("<lines />");for(g=0;g<x.length;g++)e.append(x[g]);a.append(e)}a.append($("<plot_ci />").text("c("+
b.join()+")"));a.append($("<show_signif />").text("c("+u.join()+")"));a.append($("<plot_disp />").text("c("+d.join()+")"));a.append($("<colors />").text("c("+c.join()+")"));a.append($("<pch />").text("c("+f.join()+")"));a.append($("<type />").text("c("+h.join()+")"));a.append($("<lty />").text("c("+l.join()+")"));a.append($("<lwd />").text("c("+n.join()+")"));a.append($("<con_series />").text("c("+m.join()+")"));a.append($("<order_series />").text("c("+q.join()+")"));a.append($("<plot_cmd />").text($("#plot_cmd").val()));
...
a).find("label").size()&&(c=$('<label for="fix_var_event_equal_'+b+'">Equalize</label>'));$("body").append(n);$("#fixed_var_table").append($("<tr>").append($("<td>").append(d)).append($("<td>").append(f)).append($("<td>").append(g)).append($("<td>").append(h)).append($("<td>").append(m).append(l)).append($("<td>").append(e).append(c)))}createValDatePeriodDialog("fixed_var_val",b);g.multiselect({selectedList:100,noneSelectedText:"Select value",position:{my:"center center",at:"right center"}});f.multiselect({multiple:!1,
...
!c),0<$("#fix_var_event_equal_"+a).find("label").size()&&(d=$('<label for="fix_var_event_equal_'+b+'">Equalize</label>')),$("body").append(m),$("#fixed_var_table").append($("<tr>").append($("<td>").append(e)).append($("<td>").append(f)).append($("<td>").append(g)).append($("<td>").append(h)).append($("<td>").append(n).append(l)).append($("<td>").append(c).append(d))),e.button({icons:{primary:"ui-icon-trash"},text:!1}).click(function(){removeFixedVar($(this).attr("id"))}));createValDatePeriodDialog("fixed_var_val",
...
for(var b=0;b<a.length;b++){var d=[];addSeriesVarHist();var e=$(a[b]).attr("name");try{$("#series_var_y1_"+(b+1)).val(e).multiselect("refresh")}catch(c){}$(a[b]).find("val").each(function(){var a=$(this).text().match(/\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/g);a&&0!==a.length||(a=$(this).text().split(":"));for(var b=0;b<a.length;b++)d.push(a[b])});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#series_var_val_y1_date_period_button_"+(b+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+
(b+1)).css("display","none");updateSeriesVarValHist(b+1,d)}}else d=$("#series_var_1_1").first().val(),updateSeriesVarValHist(1,d);updatePlotFix();$("input[name=normalized_histogram][value="+$(initXML.find("plot").find("normalized_histogram")).text()+"]").prop("checked",!0)}
...
currentTab||"Taylor"===currentTab||"Contour"===currentTab?1:2;for(var e,c=1;c<=d;c++){var f="y"+c,g=0;if(0<initXML.find("plot").find("dep").find("dep"+c).children().length){var h=initXML.find("plot").find("dep").find("dep"+c).children();for(a=0;a<h.length;a++){e=[];var l=$(h[a]).attr("name");if(0===g&&1===c)try{$("#fcst_var_"+f+"_"+(g+1)).val(l).multiselect("refresh")}catch(t){}else{addFcstVar(f);try{$("#fcst_var_"+f+"_"+(g+1)).val(l).multiselect("refresh")}catch(t){}}$(h[a]).find("stat").each(function(){e.push($(this).text())});
...
c).children(),a=0;a<g.length;a++){var n=[];var m=!1;l=$(g[a]).attr("name");if(0===a&&1===c)try{$("#series_var_"+f+"_"+(a+1)).val(l).multiselect("refresh")}catch(t){}else{"Taylor"===currentTab?addSeriesVarHist():"Perf"===currentTab?addSeriesVarPerf():addSeriesVar(f);try{$("#series_var_"+f+"_"+(a+1)).val(l).multiselect("refresh")}catch(t){}}$(g[a]).find("val").each(function(){var a=$(this).text().match(/\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/g);a&&0!==a.length||(a=$(this).text().split(":"));2<a.length&&
(m=!0);for(var b=0;b<a.length;b++)n.push(a[b])});"fcst_init_beg"==l||"fcst_valid_beg"==l||"fcst_valid"==l||"fcst_init"==l?($("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","block"),$("#series_var_val_"+f+"_date_range_button_"+(a+1)).css("display","block")):($("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","none"),$("#series_var_val_"+f+"_date_range_button_"+(a+1)).css("display","none"));updateSeriesVarVal(f,a+1,n);!0===m?$("#group_series_var_"+f+"_"+(a+1)).prop("checked",
!0):$("#group_series_var_"+f+"_"+(a+1)).prop("checked",!1)}else n=$("#series_var_"+f+"_1").first().val(),updateSeriesVarVal(f,1,n)}updatePlotFixSeries();$("#txtPlotCond").val(initXML.find("plot").find("plot_cond").text());try{$("#annotation").val(initXML.find("plot").find("annotation_template").text())}catch(t){}"mode"===b&&updateIndyVar(b);l=$(initXML.find("plot").find("indep")[0]).attr("name");try{$("#indy_var").val(l).multiselect("refresh")}catch(t){}"fcst_init_beg"===l||"fcst_valid_beg"===l||
...
b).val();0===g.length&&(g=3600);g=parseFloat(g);"min"===h?g*=60:"hours"===h?g*=3600:"days"===h&&(g*=86400);var l;"fixed_var_val"===a?l=$(fixVarValResponse[b]).find("val"):"series_var_val_y1"===a?l=$(seriesY1VarValResponse[b]).find("val"):"series_var_val_y2"===a&&(l=$(seriesY2VarValResponse[b]).find("val"));for(c=c.clone();c<=f;){h=c.format("YYYY-MM-DD HH:mm:ss");for(var n=!1,m=0;m<l.length;m++)if($(l[m]).text()==h){n=!0;break}n&&$("#"+a+"_"+b).find('option[value="'+h+'"]').prop("selected",!0);c.add(g,
...
3600);f=parseInt(f);"min"===g?f*=60:"hours"===g?f*=3600:"days"===g&&(f*=86400);g=$(previousIndVarValResponse).find("val");for(e=e.clone();e<=c;){for(var h=e.format("YYYY-MM-DD HH:mm:ss"),l=!1,n=0;n<g.length;n++)if($(g[n]).text()===h){l=!0;break}l&&d.find('option[value="'+h+'"]').prop("selected",!0);e.add(f,"seconds")}try{d.multiselect("refresh")}catch(m){}}$(this).dialog("close")}},open:function(e,c){e=d.val();null==e&&(e=[]);populateIndyVarVal(e);e=$(previousIndVarValResponse).find("val");c=a.val();
var f=b.val();a.empty();b.empty();for(var g=0;g<e.length;g++){var h=$(e[g]),l=$("<option/>").attr("value",h.text()).text(h.text());h.text()==c&&l.prop("selected",!0);a.append(l);h.text()===f?l.prop("selected",!0):l.prop("selected",!1);b.append(l.clone())}b.find("option").last().prop("selected",!0);1<e.length&&(c=moment.tz($(e[0]).text(),"YYYY-MM-DD HH:mm:ss","UTC"),e=moment.tz($(e[1]).text(),"YYYY-MM-DD HH:mm:ss","UTC"),e=moment.duration({from:c,to:e}),$("#date_period_by").val(e.as("hours")))}})}
...
$("#series2Y1").removeProp("disabled");$("#y2AxisDiff").removeProp("disabled");$("#y1AxisDiff").removeProp("disabled");series1Names=[];series2Names=[];for(b=0;b<a.length;b++){var d=!1;if(0!=a[b].title.indexOf("DIFF")&&0!=a[b].title.indexOf("RATIO")&&0!=a[b].title.indexOf("SS")&&0!=a[b].title.indexOf("ETB"))if("mode"==c){var e=a[b].title.split(" ");-1<listStatModeRatio.indexOf(e[e.length-1])&&(d=!0)}else d=!0;d&&(-1!==a[b].y_axis.indexOf("2")?($("#series1Y2").append($("<option></option>").attr("value",
a[b].title).text(a[b].title)),$("#series2Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series2Names.push(a[b].title)):($("#series1Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series1Names.push(a[b].title)))}0<$("#series1Y2 option").length&&0<$("#series1Y1 option").length?createNewDerivedSeriesName(1):(0==$("#series1Y2 option").length&&($("#y2AxisDiff").attr("disabled",
!0),createNewDerivedSeriesName(1)),0==$("#series1Y1 option").length&&($("#y1AxisDiff").attr("disabled",!0),$("#y1AxisDiff").removeAttr("checked"),$("#y2AxisDiff").prop("checked",!0),$("#series1Y2").removeAttr("disabled"),$("#series2Y2").removeAttr("disabled"),$("#series1Y1").attr("disabled",!0),$("#series2Y1").attr("disabled",!0),createNewDerivedSeriesName(2)))},close:function(){}});$("#listdt").jqGrid("navButtonAdd","#pagerdt",{caption:"Add Derived Curve",title:"Add Derived Curve",buttonicon:"ui-icon-plus",
onClickButton:function(){"Roc"===currentTab||"Rely"===currentTab||"Ens_ss"===currentTab||"Perf"===currentTab||"Hist"===currentTab||"Eclv"===currentTab||"Contour"===currentTab?$("#unavailableDiffCurveDialogForm").dialog("open"):1<$("#listdt").jqGrid("getRowData").length?a.dialog("open"):$("#incorrectDiffCurveDialogForm").dialog("open")}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Remove Derived Curve",title:"Remove Derived Curve",buttonicon:"ui-icon-trash",onClickButton:function(){var a=$(this).jqGrid("getGridParam",
"selrow");if(a){var b=$(this).getRowData(a);if(b.title.startsWith("DIFF")||b.title.startsWith("RATIO")||b.title.startsWith("SS")||b.title.startsWith("ETB")){$(this).jqGrid("delRowData",a);$("#1","#gbox_listdt").css({display:""});a=b.title.slice(0,-1);if(b.title.startsWith("DIFF")){var c=a.replace("DIFF (","").split('"-"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",DIFF"}else if(b.title.startsWith("RATIO")){c=a.replace("RATIO (","").split('"/"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',
"");a=c.join()+",RATIO"}else if(b.title.startsWith("SS")){c=a.replace("SS (","").split('"and"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",SS"}else{c=a.replace("ETB (","").split('"and"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",ETB"}"Y1"==b.y_axis?(b=seriesDiffY1.indexOf(a),-1<b&&seriesDiffY1.splice(b,1)):(b=seriesDiffY2.indexOf(a),-1<b&&seriesDiffY2.splice(b,1));b=$("#listdt").jqGrid("getRowData");for(a=0;a<b.length;a++)$("#listdt").jqGrid("setCell",b[a].id,
"order",a+1),$("#listdt").jqGrid("getLocalRow",b[a].id).order=a+1}else b="#alertmod_"+this.p.id,$.jgrid.viewModal(b,{gbox:"#gbox_"+$.jgrid.jqID(this.p.id),jqm:!0}),$(b).find("#alertcnt_listdt").empty().append("<div>This is not a derived curve</div>"),$(b).find(".ui-jqdialog-titlebar-close").focus()}else b="#alertmod_"+this.p.id,$.jgrid.viewModal(b,{gbox:"#gbox_"+$.jgrid.jqID(this.p.id),jqm:!0}),$(b).find(".ui-jqdialog-titlebar-close").focus()}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Add Line",
title:"Add Line",buttonicon:"ui-icon-plus",onClickButton:function(){"Perf"===currentTab||"Taylor"===currentTab||"Contour"===currentTab?$("#unavailableLineDialogForm").dialog("open"):b.dialog("open")}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Remove Line",title:"Remove Line",buttonicon:"ui-icon-plus",onClickButton:function(){var a=$(this).jqGrid("getGridParam","selrow");if(a){var b=$(this).getRowData(a);if(b.title.startsWith("vert_line")||b.title.startsWith("horiz_line")){$(this).jqGrid("delRowData",
a);a=null;for(var c=0;c<lines.length;c++){var d=b.title,e=d.split("(")[0];d=d.split("(")[1].split(")")[0];if(lines[c].type===e&&lines[c].line_pos==d){a=c;break}}null!==a&&lines.splice(a,1)}}else b="#alertmod_"+this.p.id,$.jgrid.viewModal(b,{gbox:"#gbox_"+$.jgrid.jqID(this.p.id),jqm:!0}),$(b).find(".ui-jqdialog-titlebar-close").focus()}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Apply defaults",title:"Apply defaults",buttonicon:"ui-icon-transferthick-e-w",onClickButton:function(){alert("defaults..")}});
$("#listdt").setGridWidth($(window).width()-20);$("#unavailableDiffCurveDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){$(this).dialog("close")}}});$("#incorrectDiffCurveDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){$(this).dialog("close")}}});$(window).bind("resize",function(){$("#listdt").setGridWidth($(window).width()-20)}).trigger("resize");$("#listdt").jqGrid("sortableRows",{update:function(a,b){a=$("#listdt").jqGrid("getRowData");
for(b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",a[b].id).order=b+1}});$("#pagerdt_left table.navtable tbody tr").append('<td><div ><input type="checkbox"  style="margin:0 4px;" id="seriesLock"/><label for="seriesLock">Lock Formatting</label></div></td>');$("#seriesLock").change(function(){$(this).is(":checked")?$(this).attr("checked","checked"):$(this).removeAttr("checked")});$("#generate_plot").button({}).click(function(){sendXml()});var d=
$("#upload_file_dialog").dialog({modal:!0,autoOpen:!1,buttons:{OK:function(){$("#formUpload").submit()},Cancel:function(){d.dialog("close")}}});$("#load_xml").button({}).click(function(){d.dialog("open")});$("#reload_databases").button({}).click(function(){requestDBUpdate()});$("input[name=show_history_choice][value=show_history_success]").prop("checked",!0);$("#show_history_choice").buttonset();$("#grid_col").colorpicker({parts:"header map bar hex rgb alpha preview swatches footer".split(" "),showOn:"focus alt click",
altField:"#grid_col"});$("#grid_col").colorpicker("setColor","#CCCCCC");$("#caption_col").colorpicker({parts:"header map bar hex rgb alpha preview swatches footer".split(" "),showOn:"focus alt click",altField:"#caption_col"});$("#caption_col").colorpicker("setColor","#333333");$("#error_message").dialog({modal:!0,autoOpen:!1,buttons:{Ok:function(){$(this).dialog("close")}}});$("#r_error_message").dialog({modal:!1,autoOpen:!1,height:360,buttons:{Ok:function(){$(this).dialog("close")}}});$("#refresh_history").button({icons:{primary:"ui-icon-refresh"},
text:!1}).click(function(){refreshHistory()});$.ajaxSetup({beforeSend:function(){$("#modal").css("display","block");$("#fade").css("display","block")},complete:function(){$("#modal").css("display","none");$("#fade").css("display","none")}});$("#download_plot").button({icons:{primary:"ui-icon-disk"},text:!1}).click(function(){downloadPlot()});$("#reset_formatting").button({icons:{primary:"ui-icon-arrowrefresh-1-w"},text:!1}).click(function(){resetFormatting()});$("#plot_image").click(function(){"IMG"===
document.getElementById("plot_image").firstChild.tagName&&viewImage(resultName.replace("plot_",""))});var e=0;if(null!=initXML){var c=initXML.find("plot").find("template")[0].innerHTML;"series_plot.R_tmpl"===c||"revision_series_plot.R_tmpl"===c?currentTab="Series":"box_plot.R_tmpl"===c||"revision_box_plot.R_tmpl"===c?currentTab="Box":"bar_plot.R_tmpl"===c?currentTab="Bar":"rhist.R_tmpl"===c||"phist.R_tmpl"===c||"relp.R_tmpl"===c?currentTab="Hist":"roc.R_tmpl"===c?currentTab="Roc":"eclv.R_tmpl"===
c?currentTab="Eclv":"rely.R_tmpl"===c?currentTab="Rely":"ens_ss.R_tmpl"===c?currentTab="Ens_ss":"performance.R_tmpl"===c?currentTab="Perf":"taylor_plot.R_tmpl"===c?currentTab="Taylor":"contour_plot.R_tmpl"===c&&(currentTab="Contour");var f=$("#plot_config").find("a");for(c=0;c<f.length;c++)if(f[c].text===currentTab){e=c;break}}$("#plot_config").tabs({active:e,beforeActivate:function(a,b){a=b.oldTab.attr("aria-controls");$("#"+a).empty();cleanUp();currentTab=b.newTab.text()},beforeLoad:function(a,
b){$.ajaxSetup({cache:!0});b.jqXHR.error(function(){b.panel.html("Couldn't load this tab. We'll try to fix this as soon as possible. If this wouldn't be a demo.")})}});null!=initXML?($("#plot_title").val($(initXML.find("plot").find("tmpl").find("title")).text()),$("#x_label_title").val($(initXML.find("plot").find("tmpl").find("x_label")).text()),$("#y1_label_title").val($(initXML.find("plot").find("tmpl").find("y1_label")).text()),$("#y2_label_title").val($(initXML.find("plot").find("tmpl").find("y2_label")).text()),
...
$("#dump_points2").prop("checked","true"==$(initXML.find("plot").find("dump_points2")).text()),$("#indy1_stag").prop("checked","true"==$(initXML.find("plot").find("indy1_stag")).text()),$("#indy2_stag").prop("checked","true"==$(initXML.find("plot").find("indy2_stag")).text()),$("#varianceInflationFactor").prop("checked","true"===$(initXML.find("plot").find("varianceinflationfactor")).text()),$("#ci_alpha").val($(initXML.find("plot").find("ci_alpha")).text()),$("#eqbound_low").val($(initXML.find("plot").find("eqbound_low")).text()),
$("#eqbound_high").val($(initXML.find("plot").find("eqbound_high")).text()),$("#plot_type").val($(initXML.find("plot").find("plot_type")).text()),$("#plot_height").val($(initXML.find("plot").find("plot_height")).text()),$("#plot_width").val($(initXML.find("plot").find("plot_width")).text()),$("#plot_units").val($(initXML.find("plot").find("plot_units")).text()),$("#cex").val($(initXML.find("plot").find("cex")).text()),$("#plot_res").val($(initXML.find("plot").find("plot_res")).text()),c=$(initXML.find("plot").find("mar")).text().replace("c(",
"").replace(")","").split(","),$("#mar_bottom").val(c[0]),$("#mar_left").val(c[1]),$("#mar_top").val(c[2]),$("#mar_right").val(c[3]),c=$(initXML.find("plot").find("mgp")).text().replace("c(","").replace(")","").split(","),$("#mgp_title").val(c[0]),$("#mgp_labels").val(c[1]),$("#mgp_line").val(c[2]),$("#title_align").val($(initXML.find("plot").find("title_align")).text()),$("#title_offset").val($(initXML.find("plot").find("title_offset")).text()),$("#title_size").val($(initXML.find("plot").find("title_size")).text()),
$("#title_weight").val($(initXML.find("plot").find("title_weight")).text()),$("#grid_lty").val($(initXML.find("plot").find("grid_lty")).text()),$("#grid_lwd").val($(initXML.find("plot").find("grid_lwd")).text()),$("#grid_col").colorpicker("setColor",$(initXML.find("plot").find("grid_col")).text()),$("#grid_x").val($(initXML.find("plot").find("grid_x")).text()),$("#plot_cmd").val($(initXML.find("plot").find("plot_cmd")).text()),$("#xlab_align").val($(initXML.find("plot").find("xlab_align")).text()),
$("#xlab_offset").val($(initXML.find("plot").find("xlab_offset")).text()),$("#xlab_size").val($(initXML.find("plot").find("xlab_size")).text()),$("#xlab_weight").val($(initXML.find("plot").find("xlab_weight")).text()),$("#xtlab_horiz").val($(initXML.find("plot").find("xtlab_horiz")).text()),$("#xtlab_perp").val($(initXML.find("plot").find("xtlab_perp")).text()),$("#xtlab_size").val($(initXML.find("plot").find("xtlab_size")).text()),$("#xtlab_freq").val($(initXML.find("plot").find("xtlab_freq")).text()),
$("#xtlab_orient").val($(initXML.find("plot").find("xtlab_orient")).text()),$("#x2lab_align").val($(initXML.find("plot").find("x2lab_align")).text()),$("#x2lab_offset").val($(initXML.find("plot").find("x2lab_offset")).text()),$("#x2lab_size").val($(initXML.find("plot").find("x2lab_size")).text()),$("#x2lab_weight").val($(initXML.find("plot").find("x2lab_weight")).text()),$("#x2tlab_horiz").val($(initXML.find("plot").find("x2tlab_horiz")).text()),$("#x2tlab_perp").val($(initXML.find("plot").find("x2tlab_perp")).text()),
$("#x2tlab_size").val($(initXML.find("plot").find("x2tlab_size")).text()),$("#x2tlab_orient").val($(initXML.find("plot").find("x2tlab_orient")).text()),$("#ylab_align").val($(initXML.find("plot").find("ylab_align")).text()),$("#ylab_offset").val($(initXML.find("plot").find("ylab_offset")).text()),$("#ylab_size").val($(initXML.find("plot").find("ylab_size")).text()),$("#ylab_weight").val($(initXML.find("plot").find("ylab_weight")).text()),$("#ytlab_horiz").val($(initXML.find("plot").find("ytlab_horiz")).text()),
$("#ytlab_perp").val($(initXML.find("plot").find("ytlab_perp")).text()),$("#ytlab_size").val($(initXML.find("plot").find("ytlab_size")).text()),$("#ytlab_orient").val($(initXML.find("plot").find("ytlab_orient")).text()),c=$(initXML.find("plot").find("y1_lim")).text().replace("c(","").replace(")","").split(","),$("#y1_lim_min").val(c[0]),$("#y1_lim_max").val(c[1]),$("#y1_bufr").val($(initXML.find("plot").find("y1_bufr")).text()),c=$(initXML.find("plot").find("x1_lim")).text().replace("c(","").replace(")",
"").split(","),$("#x1_lim_min").val(c[0]),$("#x1_lim_max").val(c[1]),$("#y2lab_align").val($(initXML.find("plot").find("y2lab_align")).text()),$("#y2lab_offset").val($(initXML.find("plot").find("y2lab_offset")).text()),$("#y2lab_size").val($(initXML.find("plot").find("y2lab_size")).text()),$("#y2lab_weight").val($(initXML.find("plot").find("y2lab_weight")).text()),$("#y2tlab_horiz").val($(initXML.find("plot").find("y2tlab_horiz")).text()),$("#y2tlab_perp").val($(initXML.find("plot").find("y2tlab_perp")).text()),
$("#y2tlab_size").val($(initXML.find("plot").find("y2tlab_size")).text()),$("#y2tlab_orient").val($(initXML.find("plot").find("y2tlab_orient")).text()),c=$(initXML.find("plot").find("y2_lim")).text().replace("c(","").replace(")","").split(","),$("#y2_lim_min").val(c[0]),$("#y2_lim_max").val(c[1]),$("#y2_bufr").val($(initXML.find("plot").find("y1_bufr")).text()),$("#legend_size").val($(initXML.find("plot").find("legend_size")).text()),c=$(initXML.find("plot").find("legend_inset")).text().replace("c(",
"").replace(")","").split(","),$("#legend_inset_min").val(c[0]),$("#legend_inset_max").val(c[1]),$("#legend_box").val($(initXML.find("plot").find("legend_box")).text()),$("#legend_ncol").val($(initXML.find("plot").find("legend_ncol")).text()),$("#caption_align").val($(initXML.find("plot").find("caption_align")).text()),$("#caption_offset").val($(initXML.find("plot").find("caption_offset")).text()),$("#caption_size").val($(initXML.find("plot").find("caption_size")).text()),$("#caption_weight").val($(initXML.find("plot").find("caption_weight")).text()),
c=$(initXML.find("plot").find("caption_col")).text(),7<c.length&&(c=c.substring(0,7)),$("#caption_col").colorpicker("setColor",c)):resetFormatting();$("#release").on("click",function(){var a=window.open("https://metviewer.readthedocs.io/en/latest/");a?a.focus():alert("Please allow popups for this site")});e=$("#categories11");for(c=0;c<categories.length;c++){var g=$(categories[c]);f=$('<ul data-label="'+g.attr("name")+'">');g=g.find("db");for(var h=0;h<g.length;h++){var l=$(g[h]),n=l.find("val"),
m=l.find("desc");l=$('<li data-value="'+n.text()+'" title="'+m.text()+'">');n=$('<input name="multiselect_database" type="checkbox" disabled value="'+n.text()+'" title="'+m.text()+'" id="'+n.text()+'" /><label for="'+n.text()+'" >'+n.text()+"</label>lab");l.prepend(n);f.append(l)}e.append(f)}$(".multilevel-dropdown").multilevelDropdown();$("input[name='multiselect_database']").on("change",function(a){if(null!=$(this).val()){var b=[];$("input[name='multiselect_database']:checked").each(function(){b.push($(this).val())});
...
d=parseInt(d);"min"===e?d*=60:"hours"===e?d*=3600:"days"===e&&(d*=86400);e=moment.tz(a.date1,"UTC");var h=moment.tz(a.date2,"UTC");g.multiselect("uncheckAll");for(var l=$('[name="multiselect_indy_var_val"]');e<=h;){for(var n=e.format("YYYY-MM-DD HH:mm:ss"),m=!1,q=0;q<c.length;q++)if($(c[q]).text()===n){m=!0;break}if(m){try{var t=e.format(f)}catch(x){console.log("formatting error"),console.error(x),t=n}m=g.find('option[value="'+n+'"]');m.prop("selected",!0);if(!b)for(indy_var_vals_to_attr={},q=0;q<
l.length;q++){var u=$(l[q]);if(u.attr("value")===n){n=u.attr("id");$("#"+n+"_label").val(t);indy_var_vals_to_attr[l[q].value]=a;break}}m.text(t)}e.add(d,"seconds")}b&&g.multiselect("option","indy_var_vals_to_attr",indy_var_vals_to_attr);try{g.multiselect("refresh")}catch(x){console.log(x)}}
