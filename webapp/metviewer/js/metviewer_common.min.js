...
function updateFixedVarValHist(a,b){var c=$("#fixed_var_val_"+a);c.empty();var e=$("#database").multiselect("getChecked").val(),d;try{d=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(g){d=$("#fixed_var_"+a+" option:first-child").val()}var f;"Hist"===currentTab?f=$("input:radio[name='hist_line_type']:checked").val():"Roc"===currentTab?f="roc":"Rely"===currentTab?f="rely":"Ens_ss"===currentTab?f="ensss":"Perf"===currentTab?f="perf":"Taylor"===currentTab&&(f="taylor");$.ajax({async:!1,url:"servlet",
...
function updateSeriesHist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],m=[],p=[],t=[],r=[],y=[],v=[],w=[],x=[],B=[],q;"Hist"===currentTab?(c=$("input:radio[name='hist_line_type']:checked").val(),"rhist"===c?q="Rank Histogram":"phist"===c?q="Probability Histogram":"relp"===c&&(q="Relative Histogram")):"Roc"===currentTab?q="ROC Curve":"Rely"===currentTab?q="Reliability Curve":
"Taylor"===currentTab&&(q=$("#fcst_var_y1_1").val());if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=0;l<k.length;l++)"TRUE"===k[l].trim()?k[l]="No":k[l]="Yes","TRUE"===m[l].trim()?m[l]="No":m[l]=
"Yes";p=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(l=0;l<p.length;l++)p[l]=p[l].replace(/FF$/,"");t=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");r=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("con_series").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(x.length!=w.length)for(c=0;c<w.length;c++)x[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),
0);var u;0==c.length&&(c[0]="");if("Roc"==currentTab||"Rely"==currentTab){try{u=$("#summary_curve").multiselect("getChecked")}catch(z){u=[]}for(l=0;l<u.length;l++)"none"!=u[l].value&&c.push(u[l].value)}for(var A=0;A<c.length;A++){u=c[A]+" "+q;for(var n=!1,l=0;l<b.length;l++)if(b[l].title==u){d={};d.title=u;d.y_axis="Y1";d.order=e+1;d.hide=b[l].hide;d.plot_ci=b[l].plot_ci;d.show_signif=b[l].show_signif;d.color=b[l].color;d.pch=b[l].pch;d.type=b[l].type;d.lty=b[l].lty;d.lwd=b[l].lwd;d.con_series=b[l].con_series;
d.legend=b[l].legend;d.id=e+1;n=!0;break}n||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=u,d.y_axis="Y1",d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=u,d.y_axis="Y1",d.order=w[e]&&""!=w[e]?parseInt(w[e]):parseInt(l+1),d.hide=k[e],d.plot_ci=h[e].trim(),d.show_signif=m[e],d.pch=t[e].trim(),d.type=B[e].trim(),d.lty=r[e].trim(),d.lwd=y[e].trim(),d.con_series=v[e].trim(),d.legend=x[e].trim(),d.color=p[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=
u,d.y_axis="Y1",d.id=e+1,d.order=e+1));e++;g.push(d)}if(null==initXML)for(p=rainbow(g.length),l=0;l<g.length;l++)g[l].color=p[l];g.sort(SortByOrder);for(l=0;l<g.length;l++)a.jqGrid("addRowData",l+1,g[l]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesVarValHist(a,b){$("#listdt").jqGrid("clearGridData");var c=$("#series_var_val_y1_"+a);c.empty();var e=$("#database").multiselect("getChecked").val(),d;try{d=$("#series_var_y1_"+a).multiselect("getChecked").val()}catch(g){d=$("#series_var_y1_"+a+" option:first-child").val()}var f;"Roc"===currentTab?f="roc":"Rely"===currentTab?f="rely":"Ens_ss"===currentTab?f="ensss":"Perf"===currentTab?f="perf":"Taylor"===currentTab?f="taylor":"Hist"===currentTab&&(f=$("input:radio[name='hist_line_type']:checked").val());
$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+'_field><stat><fcst_var name="NA"><val>NA</val></fcst_var></stat></list_val></request>',error:function(){},success:function(d){seriesY1VarValResponse[a]=d;d=$($.parseXML(d)).find("val");var e;if(0<d.length)for(var f=0;f<d.length;f++){var m=$(d[f]);e=0<=$.inArray(m.text(),b);e=$("<option />",{value:m.text(),text:m.text(),selected:e});e.appendTo(c)}else e=$("<option />",{value:"N/A",
text:"N/A"}),e.appendTo(c);try{c.multiselect("refresh")}catch(p){}b=[]}})}
...
function sendXml(){var a,b=$("<request />");b.append($("<db_con />").text($("#database").multiselect("getChecked").val()));var c=$("<plot />");if("Series"===currentTab)a=$("<template>series_plot.R_tmpl</template>"),c.append(a),c=createXMLSeries(c),c=createXMLCommon(c);else if("Box"===currentTab)a=$("<template>box_plot.R_tmpl</template>"),c.append(a),c=createXMLBox(c);else if("Bar"===currentTab)a=$("<template>bar_plot.R_tmpl</template>"),c.append(a),c=createXMLBox(c);else if("Hist"===currentTab){var e=
$("input:radio[name='hist_line_type']:checked").val();"rhist"===e?a=$("<template>rhist.R_tmpl</template>"):"phist"===e?a=$("<template>phist.R_tmpl</template>"):"relp"===e&&(a=$("<template>relp.R_tmpl</template>"));c.append(a);c=createXMLHist(c)}else"Roc"===currentTab?(a=$("<template>roc.R_tmpl</template>"),c.append(a),c=createXMLRoc(c)):"Rely"===currentTab?(a=$("<template>rely.R_tmpl</template>"),c.append(a),c=createXMLRely(c)):"Ens_ss"===currentTab?(a=$("<template>ens_ss.R_tmpl</template>"),c.append(a),
c=createXMLEns(c)):"Perf"===currentTab?(a=$("<template>performance.R_tmpl</template>"),c.append(a),c=createXMLPerf(c),c=createXMLCommon(c)):"Taylor"===currentTab&&(a=$("<template>taylor_plot.R_tmpl</template>"),c.append(a),c=createXMLTaylor(c),c=createXMLCommon(c));b.append(c);a=$("<root />").append(b).html().replace(/label=\"</g,'label="&lt;').replace(/label=\">/g,'label="&gt;');$.ajax({url:"servlet",processData:!1,contentType:"application/json",type:"POST",data:a,success:function(a){var b;try{b=
$.parseXML(a);var c=$(b),e=c.find("plot");if(0==c.find("error").length){updateResult(e.text());var k=c.find("r_error");k&&0<k.length&&(updateResult(e.text()),$("#r_error_message_text").empty().text(k.text()),$("#r_error_message").dialog("open"))}else updateResult(e.text()),$("#error_message_text").empty().text(c.find("error").text()),$("#error_message").dialog("open")}catch(m){$("#error_message_text").empty().text(a),$("#error_message").dialog("open")}},error:function(a){alert("error creating a plot or Proxy timeout: "+
...
currentTab?addSeriesVarHist():addSeriesVar(g);try{$("#series_var_"+g+"_"+(a+1)).val(c).multiselect("refresh")}catch(r){}$(h[a]).find("val").each(function(){var a=$(this).text().split(",");1<a.length&&(p=!0);for(var b=0;b<a.length;b++)m.push(a[b])});"fcst_init_beg"==c||"fcst_valid_beg"==c||"fcst_valid"==c||"fcst_init"==c?$("#series_var_val_"+g+"_date_period_button_"+(a+1)).css("display","block"):$("#series_var_val_"+g+"_date_period_button_"+(a+1)).css("display","none");updateSeriesVarVal(g,a+1,m);
...
a+"_"+b).multiselect("refresh")}catch(t){console.log(t)}"fixed_var_val"!==a&&("Series"===currentTab||"Box"===currentTab||"Bar"===currentTab?updateSeries():"Roc"===currentTab||"Rely"===currentTab||"Hist"===currentTab?updateSeriesHist():(currentTab="Ens_ss",updateSeriesEns()))}$(this).dialog("close")}},open:function(d,f){var g;"fixed_var_val"==a?g=$(fixVarValResponse[b]).find("val"):"series_var_val_y1"==a?g=$(seriesY1VarValResponse[b]).find("val"):"series_var_val_y2"==a&&(g=$(seriesY2VarValResponse[b]).find("val"));
c.empty();e.empty();for(var h=0;h<g.length;h++){var k=$(g[h]),m=$("<option/>").attr("value",k.text()).text(k.text());k.text()==c.val()&&m.prop("selected",!0);c.append(m);k.text()==e.val()?m.prop("selected",!0):m.prop("selected",!1);e.append(m.clone())}e.find("option").last().prop("selected",!0);1<g.length&&(h=moment($(g[0]).text(),"YYYY-MM-DD HH:mm:ss"),g=moment($(g[1]).text(),"YYYY-MM-DD HH:mm:ss"),g=moment.duration({from:h,to:g}),$("#"+a+"_date_period_by_"+b).val(g.as("hours")))}})}
...
title:"Add Derived Curve",buttonicon:"ui-icon-plus",onClickButton:function(){"Roc"==currentTab||"Rely"==currentTab||"Ens_ss"==currentTab||"Perf"==currentTab||"Hist"===currentTab?$("#unavailableDiffCurveDialogForm").dialog("open"):1<$("#listdt").jqGrid("getRowData").length?$("#addDiffCurveDialogForm").dialog("open"):$("#incorrectDiffCurveDialogForm").dialog("open")}}).jqGrid("navButtonAdd","#pagerdt",{caption:"Remove Derived Curve",title:"Remove Derived Curve",buttonicon:"ui-icon-trash",onClickButton:function(){var a=
$(this).jqGrid("getGridParam","selrow");if(a){var b=$(this).getRowData(a);if(b.title.startsWith("DIFF")||b.title.startsWith("RATIO")||b.title.startsWith("SS")){$(this).jqGrid("delRowData",a);var c;if(b.title.startsWith("DIFF")){c=b.title.replace("DIFF (","").replace(")","").split('"-"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");a=c.join()+",DIFF"}else if(b.title.startsWith("RATIO")){c=b.title.replace("RATIO (","").replace(")","").split('"/"');for(a=0;a<c.length;a++)c[a]=c[a].replace('"',"");
...
$("#y1AxisDiff").removeProp("disabled");series1Names=[];series2Names=[];for(b=0;b<a.length;b++){var g=!1;if(0!=a[b].title.indexOf("DIFF")&&0!=a[b].title.indexOf("RATIO")&&0!=a[b].title.indexOf("SS"))if("mode"==c){var h=a[b].title.split(" ");-1<listStatModelRatio.indexOf(h[h.length-1])&&(g=!0)}else g=!0;g&&(-1!==a[b].y_axis.indexOf("2")?($("#series1Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series1Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),
series2Names.push(a[b].title)):($("#series1Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series1Names.push(a[b].title)))}0<$("#series1Y2 option").length&&0<$("#series1Y1 option").length?createNewDerivedSeriesName(1):(0==$("#series1Y2 option").length&&($("#y2AxisDiff").attr("disabled",!0),createNewDerivedSeriesName(1)),0==$("#series1Y1 option").length&&($("#y1AxisDiff").attr("disabled",
!0),$("#y1AxisDiff").removeAttr("checked"),$("#y2AxisDiff").prop("checked",!0),$("#series1Y2").removeAttr("disabled"),$("#series2Y2").removeAttr("disabled"),$("#series1Y1").attr("disabled",!0),$("#series2Y1").attr("disabled",!0),createNewDerivedSeriesName(2)))},close:function(){}});$(window).bind("resize",function(){$("#listdt").setGridWidth($(window).width()-20)}).trigger("resize");$("#listdt").jqGrid("sortableRows",{update:function(a,b){for(var c=$("#listdt").jqGrid("getRowData"),g=0;g<c.length;g++)$("#listdt").jqGrid("setCell",
c[g].id,"order",g+1),$("#listdt").jqGrid("getLocalRow",c[g].id).order=g+1}});$("#pagerdt_left table.navtable tbody tr").append('<td><div ><input type="checkbox"  style="margin:0 4px;" id="seriesLock"/><label for="seriesLock">Lock Formatting</label></div></td>');$("#seriesLock").change(function(){$(this).is(":checked")?$(this).attr("checked","checked"):$(this).removeAttr("checked")});$("#generate_plot").button({}).click(function(){sendXml()});$("#load_xml").button({}).click(function(){$("#upload_file_dialog").dialog("open")});
$("#reload_databases").button({}).click(function(){requestDBUpdate()});$("input[name=show_history_choice][value=show_history_success]").prop("checked",!0);$("#show_history_choice").buttonset();$("#grid_col").colorpicker({parts:"header map bar hex rgb alpha preview swatches footer".split(" "),showOn:"focus alt click",buttonColorize:!0,buttonImageOnly:!0});$("#grid_col").colorpicker("setColor","#CCCCCC");$("#caption_col").colorpicker({parts:"header map bar hex rgb alpha preview swatches footer".split(" "),
