...
m.join()+")"));a.append($("<order_series />").text("c("+q.join()+")"));a.append($("<plot_cmd />").text($("#plot_cmd").val()));a.append($("<legend />").text("c("+t.join()+")"));a.append($("<create_html />").text("TRUE"));0===$("#y1_lim_min").val().trim().length||0===$("#y1_lim_max").val().trim().length?a.append($("<y1_lim />").text("c()")):a.append($("<y1_lim />").text("c("+$("#y1_lim_min").val()+","+$("#y1_lim_max").val()+")"));0===$("#x1_lim_min").val().trim().length||0===$("#x1_lim_max").val().trim().length?
a.append($("<x1_lim />").text("c()")):a.append($("<x1_lim />").text("c("+$("#x1_lim_min").val()+","+$("#x1_lim_max").val()+")"));a.append($("<y1_bufr />").text($("#y1_bufr").val()));0===$("#y2_lim_min").val().trim().length||0===$("#y2_lim_max").val().trim().length?a.append($("<y2_lim />").text("c()")):a.append($("<y2_lim />").text("c("+$("#y2_lim_min").val()+","+$("#y2_lim_max").val()+")"));return a}
...
","+$("#series2Y1").val()+","+d),b=!0):$("#series1Y2").val()&&$("#series2Y2").val()&&(seriesDiffY2.push($("#series1Y2").val()+","+$("#series2Y2").val()+","+d),b=!0);a.dialog("close");b&&(updateSeries(),$("#event_equal").prop("checked",!0).trigger("change"))},Cancel:function(){a.dialog("close")}},open:function(){$("input[name=derive_oper][value=DIFF]").prop("checked",!0);for(var a=$("#listdt").jqGrid("getRowData"),b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",
a[b].id).order=b+1;var c=$("#plot_data").multiselect("getChecked").val();$("#series1Y2").empty();$("#series1Y1").empty();$("#series2Y2").empty();$("#series2Y1").empty();$("#y1AxisDiff").prop("checked",!0);$("#y2AxisDiff").removeAttr("checked");$("#series1Y2").prop("disabled",!0);$("#series2Y2").prop("disabled",!0);$("#series1Y1").removeProp("disabled");$("#series2Y1").removeProp("disabled");$("#y2AxisDiff").removeProp("disabled");$("#y1AxisDiff").removeProp("disabled");series1Names=[];series2Names=
[];for(b=0;b<a.length;b++){var d=!1;if(0!=a[b].title.indexOf("DIFF")&&0!=a[b].title.indexOf("RATIO")&&0!=a[b].title.indexOf("SS"))if("mode"==c){var e=a[b].title.split(" ");-1<listStatModeRatio.indexOf(e[e.length-1])&&(d=!0)}else d=!0;d&&(-1!==a[b].y_axis.indexOf("2")?($("#series1Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series2Names.push(a[b].title)):($("#series1Y1").append($("<option></option>").attr("value",
...
$("#reset_formatting").button({icons:{primary:"ui-icon-arrowrefresh-1-w"},text:!1}).click(function(){resetFormatting()});$("#plot_image").click(function(){"IMG"===document.getElementById("plot_image").firstChild.tagName&&viewImage(resultName.replace("plot_",""))});var d=0;if(null!=initXML){var e=initXML.find("plot").find("template")[0].innerHTML;"series_plot.R_tmpl"===e||"revision_series_plot.R_tmpl"===e?currentTab="Series":"box_plot.R_tmpl"===e||"revision_box_plot.R_tmpl"===e?currentTab="Box":"bar_plot.R_tmpl"===
e?currentTab="Bar":"rhist.R_tmpl"===e||"phist.R_tmpl"===e||"relp.R_tmpl"===e?currentTab="Hist":"roc.R_tmpl"===e?currentTab="Roc":"eclv.R_tmpl"===e?currentTab="Eclv":"rely.R_tmpl"===e?currentTab="Rely":"ens_ss.R_tmpl"===e?currentTab="Ens_ss":"performance.R_tmpl"===e?currentTab="Perf":"taylor_plot.R_tmpl"===e?currentTab="Taylor":"contour_plot.R_tmpl"===e&&(currentTab="Contour");var c=$("#plot_config").find("a");for(e=0;e<c.length;e++)if(c[e].text===currentTab){d=e;break}}$("#plot_config").tabs({active:d,
