...
b.length;d++)b[d].text()!==b[d][0].value&&(c=b[d].text().substring(b[d][0].value.length,b[d].text().length).length,c===e?b[d].text(b[d][0].value+"*"):b[d].text(b[d][0].value)),b[d].appendTo(a);try{a.multiselect("refresh")}catch(h){}updateStatVal()}}})}
...
data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+d+"</db_con><list_val><"+selected_mode+"_field>FCST_VAR</"+selected_mode+"_field></list_val></request>",error:function(){},success:function(c){var d=$(c).find("val");if(0<d.length){c=[];for(var e=0;e<d.length;e++){var k=$(d[e]);if(0===e||0!==e&&k.text()!==$(d[e-1]).text()){var h=k.text().formatAll();k=$("<option />",{value:h,text:h,title:value_to_desc_map[k.text()]});c.push(k)}else 0!==e&&k.text()===$(d[e-1]).text()&&c[c.length-1].text(c[c.length-
...
b===g?a[e].text(a[e][0].value+"*"):a[e].text(a[e][0].value)),a[e].appendTo(d)}else g=$("<option />",{value:"N/A",text:"N/A"}),g.appendTo(d);try{d.multiselect("refresh")}catch(h){console.log(h)}updateSeries()}})}}
...
c+"</db_con><list_val><"+f+"_field>"+k+"</"+f+"_field>"+g+"</list_val></request>",dataType:"xml",processData:!1,contentType:"application/xml",error:function(){},success:function(c){"y1"===a?seriesY1VarValResponse[b]=c:seriesY2VarValResponse[b]=c;var g=$(c).find("val");c=[];if(0<g.length){for(var f=0;f<g.length;f++){var h=$(g[f]);var k="string"===$.type(d)?h.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&")===d:0<=$.inArray(h.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;",
"&"),d);0===f||0!==f&&h.text()!==$(g[f-1]).text()?(k=$("<option />",{value:h.text(),text:h.text(),selected:k}),c.push(k)):0!==f&&h.text()===$(g[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}k=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(g=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,g===k?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(u){}}else{k=
...
function updateFixedVarValHist(a,b,d){d&&"false"===d?$("#fix_var_event_equal_"+a).prop("checked",!1).prop("disabled",!1):$("#fix_var_event_equal_"+a).prop("checked",!0).prop("disabled",!1);var e=$("#fixed_var_val_"+a);e.empty();if(d=getSelectedDatabases()){try{var c=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(g){c=$("#fixed_var_"+a+" option:first-child").val()}var f;"Hist"===currentTab?f=$("input:radio[name='hist_line_type']:checked").val():"Roc"===currentTab?f="roc":"Eclv"===currentTab?
f="eclv":"Rely"===currentTab?f="rely":"Ens_ss"===currentTab?f="ensss":"Perf"===currentTab?f="perf":"Taylor"===currentTab&&(f="taylor");$.ajax({async:!1,url:"servlet",type:"POST",dataType:"xml",processData:!1,contentType:"application/xml",data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+d+"</db_con><list_val><"+f+"_field>"+c+"</"+f+"_field></list_val></request>",error:function(){},success:function(c){fixVarValResponse[a]=c;var d=$(c).find("val");if(0<d.length){c=[];for(var f=0;f<d.length;f++){var g=
$(d[f]);var m=0<=$.inArray(g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&"),b);0===f||0!==f&&g.text()!==$(d[f-1]).text()?(g=g.text().formatAll(),m=$("<option />",{value:g,text:g,selected:m}),c.push(m)):0!=f&&g.text()===$(d[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}m=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(d=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,d===m?c[f].text(c[f][0].value+
"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(q){}}else{m=$("<option />",{value:"N/A",text:"N/A"});m.appendTo(e);try{e.multiselect("refresh")}catch(q){}}b=[]}})}}
...
b.length;e++)b[e].text()!==b[e][0].value&&(c=b[e].text().substring(b[e][0].value.length,b[e].text().length).length,c===d?b[e].text(b[e][0].value+"*"):b[e].text(b[e][0].value)),b[e].appendTo(a);try{a.multiselect("refresh")}catch(h){}}else{d=$("<option />",{value:"N/A",text:"N/A"});d.appendTo(a);try{a.multiselect("refresh")}catch(h){}}updateSeries()}})}}
function updateFixedVarVal(a,b,d){d&&"false"===d?$("#fix_var_event_equal_"+a).prop("checked",!1):$("#fix_var_event_equal_"+a).prop("checked",!0);var e=$("#fixed_var_val_"+a);e.empty();if(d=getSelectedDatabases()){if("Perf"===currentTab){var c="stat";var f="<stat><fcst_var><val>FAR</val><val>NBR_FAR</val></fcst_var></stat>"}else"Eclv"===currentTab?(c="eclv",f="<stat><fcst_var><val>NA</val></fcst_var></stat>"):"Taylor"===currentTab?(c="taylor",f=convertVarsAndStatsToXml()):"Contour"===currentTab?(c=
"stat",f='<stat><fcst_var name="'+$("#fcst_var_y1_1").val()+'" ><val>'+$("#fcst_stat_y1_1").val()+"</val></fcst_var></stat>"):(c=$("#plot_data").multiselect("getChecked").val(),f=convertVarsAndStatsToXml());try{var g=$("#fixed_var_"+a).multiselect("getChecked").val()}catch(k){g=$("#fixed_var_"+a+" option:first-child").val()}$.ajax({async:!1,url:"servlet",type:"POST",dataType:"xml",processData:!1,contentType:"application/xml",data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+d+"</db_con><list_val><"+
c+"_field>"+g+"</"+c+"_field>"+f+"</list_val></request>",error:function(){try{e.multiselect("refresh")}catch(k){}},success:function(c){fixVarValResponse[a]=c;var d=$(c).find("val");c=[];if(0<d.length){for(var f=0;f<d.length;f++){var g=$(d[f]);var k=0<=$.inArray(g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&"),b);0===f||0!==f&&g.text()!==$(d[f-1]).text()?(g=g.text().formatAll(),k=$("<option />",{value:g,text:g,selected:k}),c.push(k)):0!==f&&g.text()===$(d[f-1]).text()&&c[c.length-
1].text(c[c.length-1].text()+"*")}k=$("input[name='multiselect_database']:checked").length-1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(d=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,d===k?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(t){}}else{k=$("<option />",{value:"N/A",text:"N/A"});k.appendTo(e);try{e.multiselect("refresh")}catch(t){}}b=[]}})}}
...
function createSeriesMapForPermutationEns(a,b){for(var d={},e,c,f=a.length-1;0<=f;f--){var g=[];try{c=$("#series_var_"+b+"_"+a[f]).multiselect("getChecked")[0].value}catch(h){c=$("#series_var_"+b+"_"+a[f]).val()}try{e=$("#series_var_val_"+b+"_"+a[f]).multiselect("getChecked")}catch(h){e=[]}for(var k=0;k<e.length;k++)g.push(e[k].value);k=d[c];void 0!=k&&(g=k.concat(g));d[c]=g}return d}
function updateSeriesHist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d=0,e={},c=$("#seriesLock").is(":checked"),f=[],g=[],k=[],h=[],p=[],m=[],q=[],t=[],u=[],A=[],z=[],w=[],y=[],B;if("Hist"===currentTab){var n=$("input:radio[name='hist_line_type']:checked").val();"rhist"===n?B="Rank Histogram":"phist"===n?B="Probability Histogram":"relp"===n&&(B="Relative Histogram")}else"Roc"===currentTab?B="ROC Curve":"Rely"===currentTab?B="Reliability Curve":
"Taylor"===currentTab?B=$("#fcst_var_y1_1").val():"Eclv"===currentTab&&(B="Economic value");if(null!=initXML){g=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");h=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(var l=0;l<k.length;l++)"TRUE"===k[l].trim()?k[l]="No":k[l]="Yes",
"TRUE"===h[l].trim()?h[l]="No":h[l]="Yes";m=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(l=0;l<m.length;l++)m[l]=m[l].replace(/FF$/,"");q=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");t=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");u=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");
A=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");p=initXML.find("plot").find("show_legend").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,
"").replace(/"/g,"").split(",");if(w.length!=z.length)for(n=0;n<z.length;n++)w[n]=""}n=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);0===n.length&&(n[0]="");if("Roc"===currentTab||"Rely"===currentTab||"Eclv"===currentTab){try{var r=$("#summary_curve").multiselect("getChecked")}catch(v){r=[]}for(l=0;l<r.length;l++)"none"!=r[l].value&&n.push(r[l].value)}for(var x=0;x<n.length;x++){r=n[x]+" "+B;var C=!1;for(l=0;l<b.length;l++)if(b[l].title==r){e={};e.title=r;e.y_axis=
"Y1";e.order=d+1;e.hide=b[l].hide;e.plot_ci=b[l].plot_ci;e.show_signif=b[l].show_signif;e.color=b[l].color;e.pch=b[l].pch;e.type=b[l].type;e.lty=b[l].lty;e.lwd=b[l].lwd;e.con_series=b[l].con_series;e.show_legend=b[l].show_legend;e.legend=b[l].legend;e.id=d+1;C=!0;break}C||(c&&0<d&&null==initXML?(e=jQuery.extend(!0,{},f[f.length-1]),e.title=r,e.y_axis="Y1",e.id=d+1,e.order=d+1):null!=initXML?(e={},e.title=r,e.y_axis="Y1",e.order=z[d]&&""!=z[d]?parseInt(z[d]):parseInt(l+1),e.hide=k[d],e.plot_ci=g[d].trim(),
e.show_signif=h[d],e.pch=q[d].trim(),e.type=y[d].trim(),e.lty=t[d].trim(),e.lwd=u[d].trim(),e.con_series=A[d].trim(),e.show_legend=p[d].trim(),e.legend=w[d].trim(),e.color=m[d].trim(),e.id=d+1):(e=jQuery.extend(!0,{},firstSeriesFormatting),e.title=r,e.y_axis="Y1",e.id=d+1,e.order=d+1));d++;f.push(e)}for(l=0;l<lines.length;l++)e=jQuery.extend(!0,{},firstSeriesFormatting),e.title=lines[l].type+"("+lines[l].line_pos+")",e.y_axis="Y1",e.order="",e.id=d+1,lines[l].hasOwnProperty("color")&&(e.color=lines[l].color),
lines[l].hasOwnProperty("lty")&&(e.lty=lines[l].lty),lines[l].hasOwnProperty("lwd")&&(e.lwd=lines[l].lwd),d++,f.push(e);if(null==initXML)for(m=rainbow(f.length),l=0;l<f.length;l++)f[l].color=m[l];f.sort(SortByOrder);for(l=0;l<f.length;l++)a.jqGrid("addRowData",l+1,f[l]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
...
"").split(",");u=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("show_legend").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(y.length!=w.length)for(d=0;d<w.length;d++)y[d]=""}d=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);if("true"===$("input:radio[name='ensss_pts_disp']:checked").val()){var l=[];var r=0;for(n=0;n<d.length;n++)l[r]=d[n]+" MSE",l[r+1]=d[n]+" #PTS",r+=2;d=l}else for(n=0;n<d.length;n++)d[n]+=" MSE";for(r=0;r<d.length;r++){var x=
d[r],C=!1;l=0<x.indexOf("#PTS")?"Y2":"Y1";for(n=0;n<b.length;n++)if(b[n].title==x){c={};c.title=x;c.y_axis=l;c.order=e+1;c.hide=b[n].hide;c.plot_ci=b[n].plot_ci;c.show_signif=b[n].show_signif;c.color=b[n].color;c.pch=b[n].pch;c.type=b[n].type;c.lty=b[n].lty;c.lwd=b[n].lwd;c.con_series=b[n].con_series;c.show_legend=b[n].show_legend;c.legend=b[n].legend;c.id=e+1;C=!0;break}C||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=x,c.y_axis=l,c.id=e+1,c.order=e+1):null!=initXML?(c={},
c.title=x,c.y_axis=l,c.order=w[e]&&""!=w[e]?parseInt(w[e]):parseInt(n+1),c.hide=h[e],c.plot_ci=k[e].trim(),c.show_signif=p[e],c.pch=q[e].trim(),c.type=B[e].trim(),c.lty=t[e].trim(),c.lwd=u[e].trim(),c.con_series=A[e].trim(),c.show_legend=z[e].trim(),c.legend=y[e].trim(),c.color=m[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=x,c.y_axis=l,c.id=e+1,c.order=e+1));e++;g.push(c)}for(n=0;n<lines.length;n++)c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=lines[n].type+"("+
lines[n].line_pos+")",c.y_axis="Y1",c.order="",c.id=e+1,lines[n].hasOwnProperty("color")&&(c.color=lines[n].color),lines[n].hasOwnProperty("lty")&&(c.lty=lines[n].lty),lines[n].hasOwnProperty("lwd")&&(c.lwd=lines[n].lwd),e++,g.push(c);if(null==initXML)for(m=rainbow(g.length),n=0;n<g.length;n++)g[n].color=m[n];g.sort(SortByOrder);for(n=0;n<g.length;n++)a.jqGrid("addRowData",n+1,g[n]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesPerf(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var d,e=0,c={},f=$("#seriesLock").is(":checked"),g=[],k=[],h=[],p=[],m=[],q=[],t=[],u=[],A=[],z=[],w=[],y=[],B=[];if(null!=initXML){k=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");h=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");p=initXML.find("plot").find("show_signif").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");for(var n=0;n<h.length;n++)"TRUE"==h[n].trim()?h[n]="No":h[n]="Yes","TRUE"==p[n].trim()?p[n]="No":p[n]="Yes";m=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(n=0;n<m.length;n++)m[n]=m[n].replace(/FF$/,"");q=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");t=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,
"").split(",");u=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("show_legend").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(y.length!=w.length)for(d=0;d<w.length;d++)y[d]=""}d=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var l=0;l<d.length;l++){var r=d[l],x=!1;for(n=0;n<b.length;n++)if(b[n].title==r){c={};c.title=r;c.y_axis="";c.order=e+1;c.hide=b[n].hide;c.plot_ci=b[n].plot_ci;c.show_signif=b[n].show_signif;c.color=b[n].color;
c.pch=b[n].pch;c.type=b[n].type;c.lty=b[n].lty;c.lwd=b[n].lwd;c.con_series=b[n].con_series;c.show_legend=b[n].show_legend;c.legend=b[n].legend;c.id=e+1;x=!0;break}x||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=r,c.y_axis=void 0,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=r,c.y_axis=void 0,c.order=w[e]&&""!=w[e]?parseInt(w[e]):parseInt(n+1),c.hide=h[e],c.plot_ci=k[e].trim(),c.show_signif=p[e],c.pch=q[e].trim(),c.type=B[e].trim(),c.lty=t[e].trim(),c.lwd=u[e].trim(),c.con_series=
A[e].trim(),c.show_legend=z[e].trim(),c.legend=y[e].trim(),c.color=m[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=r,c.y_axis=void 0,c.id=e+1,c.order=e+1));e++;g.push(c)}if(null==initXML)for(m=rainbow(g.length),n=0;n<g.length;n++)g[n].color=m[n];g.sort(SortByOrder);for(n=0;n<g.length;n++)a.jqGrid("addRowData",n+1,g[n]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeries(a){var b=$("#listdt");b.saveCell(lastSelRow,lastSelCol);var d=b.jqGrid("getRowData");b.jqGrid("clearGridData");group_name_to_value_map={};var e=0,c={},f=$("#seriesLock").is(":checked"),g=[];try{var k=$("#plot_data").multiselect("getChecked").val()}catch(O){k="stat"}var h=[],p=[],m=[],q=[],t=[],u=[],A=[],z=[],w=[],y=[],B=[],n=[];if(null!=initXML){h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");p=initXML.find("plot").find("plot_disp").text().replace("c(",
...
"").split(",");u=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");A=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");z=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("show_legend").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");if((y=initXML.find("plot").find("order_series").text())&&0<
y.length)y=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");else for(y=[],l=0;l<q.length;l++)y.push(l+1);n=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(B.length!=y.length)for(l=0;l<y.length;l++)B[l]=""}for(var r=1;2>=r;r++){var x="y"+r;if(1==r){var C=fcst_var_y1_indexes;var v=
permuteSeries(createSeriesMapForPermutation(series_var_y1_indexes,x),0)}else C=fcst_var_y2_indexes,v=permuteSeries(createSeriesMapForPermutation(series_var_y2_indexes,x),0);for(var H=0;H<C.length;H++){var M=$("#fcst_var_"+x+"_"+C[H]).val(),K=[];var E=$("#fcst_stat_"+x+"_"+C[H]).multiselect("getChecked");var F=[];if("Contour"==currentTab){for(l=0;l<fixed_var_indexes.length;l++){var G=$("#fixed_var_val_"+fixed_var_indexes[l]).val();if(G)for(v=0;v<G.length;v++)F.push({value:G[v]})}v=[""]}F&&0!=F.length||
(F=[{value:""}]);0!=E.length||"mode"!=k&&"mtd"!=k||(E=$("#fcst_stat_mode_"+x+"_"+C[H]).multiselect("getChecked"));for(l=0;l<E.length;l++)K.push(E[l].value);for(E=0;E<K.length;E++)for(G=0;G<v.length;G++){l=K[E];if("mode"==k&&-1==listStatModeRatio.indexOf(l)||"mtd"==k&&-1==listStatMtdRatio.indexOf(l))l=createStatNameForModeArrt(l,x,C[H]);for(var N=v[G]+" "+M+" "+l,J=0;J<F.length;J++){var D=N;-1<D.indexOf("<")&&(D=D.replace(/</g,"&lt;"));0<F[J].value.length&&(D=D+" "+F[J].value);var L=!1;for(l=0;l<d.length;l++)if(d[l].title==
D&&d[l].y_axis.match(r+"$")==r){c={};c.title=D;c.y_axis="Y"+r;c.order=e+1;c.hide=d[l].hide;c.plot_ci=d[l].plot_ci;c.show_signif=d[l].show_signif;c.color=d[l].color;c.pch=d[l].pch;c.type=d[l].type;c.lty=d[l].lty;c.lwd=d[l].lwd;c.con_series=d[l].con_series;c.show_legend=d[l].show_legend;c.legend=d[l].legend;c.id=e+1;L=!0;break}L||(f&&0<e&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=D,c.y_axis="Y"+r,c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=D,c.y_axis="Y"+r,c.order=parseInt(y[e]),
c.hide=p[e],c.plot_ci=h[e].trim(),c.show_signif=m[e],c.pch=t[e].trim(),c.type=n[e].trim(),c.lty=u[e].trim(),c.lwd=A[e].trim(),c.con_series=z[e].trim(),c.show_legend=w[e].trim(),c.legend=B[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=f&&0<d.length?jQuery.extend(!0,{},d[d.length-1]):jQuery.extend(!0,{},firstSeriesFormatting),c.title=D,c.y_axis="Y"+r,c.id=e+1,c.order=e+1));"Contour"!==currentTab&&e++;g.push(c)}}}}d=[];for(l=0;l<seriesDiffY1.length;l++){c=seriesDiffY1[l].split(",")[0];k=seriesDiffY1[l].split(",")[1];
r=seriesDiffY1[l].split(",")[2];C=x=!1;var I='"and"';"DIFF"===r?I='"-"':"RATIO"===r&&(I='"/"');for(v=0;v<g.length;v++)c==g[v].title&&(x=!0),k==g[v].title&&(C=!0);x&&C&&(d.push(seriesDiffY1[l]),k=r+' ("'+c+I+k+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=k,c.y_axis="Y1",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=k,c.y_axis="Y1",c.order=parseInt(y[e]),c.hide=p[e],c.plot_ci=h[e].trim(),c.show_signif=m[e].trim(),c.pch=t[e].trim(),c.type=n[e].trim(),c.lty=u[e].trim(),c.lwd=
A[e].trim(),c.con_series=z[e].trim(),c.show_legend=w[e].trim(),c.legend=B[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=k,c.y_axis="Y1",c.id=e+1,c.order=e+1),e++,g.push(c))}seriesDiffY1=d;d=[];for(l=0;l<seriesDiffY2.length;l++){c=seriesDiffY2[l].split(",")[0];k=seriesDiffY2[l].split(",")[1];r=seriesDiffY1[l].split(",")[2];C=x=!1;"DIFF"===r?I='"-"':"RATIO"===r&&(I='"/"');for(v=0;v<g.length;v++)c==g[v].title&&(x=!0),k==g[v].title&&(C=!0);x&&C&&(d.push(seriesDiffY2[l]),
k=r+' ("'+c+I+k+'")',f&&null==initXML?(c=jQuery.extend(!0,{},g[g.length-1]),c.title=k,c.y_axis="Y2",c.id=e+1,c.order=e+1):null!=initXML?(c={},c.title=k,c.y_axis="Y2",c.order=parseInt(y[e]),c.hide=p[e],c.plot_ci=h[e].trim(),c.show_signif=m[e],c.pch=t[e].trim(),c.type=n[e].trim(),c.lty=u[e].trim(),c.lwd=A[e].trim(),c.con_series=z[e].trim(),c.show_legend=w[e].trim(),c.legend=B[e].trim(),c.color=q[e].trim(),c.id=e+1):(c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=k,c.y_axis="Y2",c.id=e+1,c.order=
e+1),e++,g.push(c))}seriesDiffY2=d;for(l=0;l<lines.length;l++)c=jQuery.extend(!0,{},firstSeriesFormatting),c.title=lines[l].type+"("+lines[l].line_pos+")",c.y_axis="Y1",c.order="",c.id=e+1,lines[l].hasOwnProperty("color")&&(c.color=lines[l].color),lines[l].hasOwnProperty("lty")&&(c.lty=lines[l].lty),lines[l].hasOwnProperty("lwd")&&(c.lwd=lines[l].lwd),e++,g.push(c);if(null==initXML){if("Contour"===currentTab)for(l=0;l<g.length;l++)q.push("#000000");else q=rainbow(g.length);e=[];if(a)e=q;else{for(l=
0;l<q.length;l++)for(v=0;v<g.length;v++)g[v].color&&0<g[v].color.length&&g[v].color.substr(g[v].color.length-6).toUpperCase()===q[l].substr(q[l].length-6).toUpperCase()&&l!=v&&(e[v]=q[l],e[l]=q[v]);for(l=0;l<q.length;l++)e[l]||(e[l]=q[l])}for(l=0;l<g.length;l++)g[l].color&&0!=g[l].color.length||(g[l].color=e[l])}"Contour"===currentTab?g.reverse():g.sort(SortByOrder);for(l=0;l<g.length;l++)b.jqGrid("addRowData",l+1,g[l]),"Contour"===currentTab&&0!==l&&$("#"+(l+1),"#gbox_listdt").css({display:"none"});
outerLayout.sizePane("south",$("#gbox_listdt").height())}function SortByOrder(a,b){return a.order-b.order}function rainbow(a){if(1>a)return[];if(1==a)return["#FF0000"];for(var b=[],d=1/(a-1),e=0,c=0;c<a;c++,e+=d)b.push("#"+interpolateColor(e));return b}
...
d.append($("<eveq_dis />").text($("#eveq_dis").is(":checked")));d.append($("<cl_step />").text($("#cl_step").val()));a.append(d);d=$("#reference_line_col").val();d.startsWith("#")?a.append($("<reference_line_col />").text(d)):a.append($("<reference_line_col />").text("#"+d));d=$("#noskill_line_col").val();d.startsWith("#")?a.append($("<noskill_line_col />").text(d)):a.append($("<noskill_line_col />").text("#"+d));return a=createXMLCommon(a)}
...
function createXMLSeries(a){try{$("#listdt").saveCell(lastSelRow,lastSelCol)}catch(p){console.log("Can't save row "+lastSelRow)}try{var b=$("#plot_data").multiselect("getChecked").val()}catch(p){b="stat"}var d=$("<dep />");d.append(createDepElementForAxis(1,fcst_var_y1_indexes,b));d.append(createDepElementForAxis(2,fcst_var_y2_indexes,b));a.append(d);a.append(createSeriesElementForAxis(1,series_var_y1_indexes));a.append(createSeriesElementForAxis(2,series_var_y2_indexes));a=createXMLPlotFix(a);a.append($("<plot_cond />").text($("#txtPlotCond").val()));
...
a.append($("<event_equal />").text($("#event_equal").is(":checked")));a.append($("<vert_plot />").text($("#vert_plot").is(":checked")));a.append($("<x_reverse />").text($("#x_reverse").is(":checked")));a.append($("<num_stats />").text($("#num_stats").is(":checked")));a.append($("<indy1_stag />").text($("#indy1_stag").is(":checked")));a.append($("<indy2_stag />").text($("#indy2_stag").is(":checked")));a.append($("<start_from_zero />").text($("#start_from_zero").is(":checked")));a.append($("<grid_on />").text($("#grid_on").is(":checked")));
a.append($("<sync_axes />").text($("#sync_axes").is(":checked")));a.append($("<dump_points1 />").text($("#dump_points1").is(":checked")));a.append($("<dump_points2 />").text($("#dump_points2").is(":checked")));a.append($("<log_y1 />").text($("#log_y1").is(":checked")));a.append($("<log_y2 />").text($("#log_y2").is(":checked")));a.append($("<varianceInflationFactor />").text($("#varianceInflationFactor").is(":checked")));a.append($("<plot_type />").text($("#plot_type").val()));a.append($("<plot_height />").text($("#plot_height").val()));
a.append($("<plot_width />").text($("#plot_width").val()));a.append($("<plot_res />").text($("#plot_res").val()));a.append($("<plot_units />").text($("#plot_units").val()));e=$("#mar_bottom").val().trim();b=$("#mar_left").val().trim();d=$("#mar_top").val().trim();c=$("#mar_right").val().trim();0===e.length&&(e=0);0===b.length&&(b=0);0===d.length&&(d=0);0===c.length&&(c=0);a.append($("<mar />").text("c("+e+","+b+","+d+","+c+")"));e=$("#mgp_title").val().trim();b=$("#mgp_labels").val().trim();d=$("#mgp_line").val().trim();
0===e.length&&(e=0);0===b.length&&(b=0);0===d.length&&(d=0);a.append($("<mgp />").text("c("+e+","+b+","+d+")"));a.append($("<cex />").text($("#cex").val()));a.append($("<title_weight />").text($("#title_weight").val()));a.append($("<title_size />").text($("#title_size").val()));a.append($("<title_offset />").text($("#title_offset").val()));a.append($("<title_align />").text($("#title_align").val()));a.append($("<xtlab_orient />").text($("#xtlab_orient").val()));a.append($("<xtlab_perp />").text($("#xtlab_perp").val()));
a.append($("<xtlab_horiz />").text($("#xtlab_horiz").val()));a.append($("<xtlab_freq />").text($("#xtlab_freq").val()));a.append($("<xtlab_size />").text($("#xtlab_size").val()));a.append($("<xlab_weight />").text($("#xlab_weight").val()));a.append($("<xlab_size />").text($("#xlab_size").val()));a.append($("<xlab_offset />").text($("#xlab_offset").val()));a.append($("<xlab_align />").text($("#xlab_align").val()));a.append($("<ytlab_orient />").text($("#ytlab_orient").val()));a.append($("<ytlab_perp />").text($("#ytlab_perp").val()));
a.append($("<ytlab_horiz />").text($("#ytlab_horiz").val()));a.append($("<ytlab_size />").text($("#ytlab_size").val()));a.append($("<ylab_weight />").text($("#ylab_weight").val()));a.append($("<ylab_size />").text($("#ylab_size").val()));a.append($("<ylab_offset />").text($("#ylab_offset").val()));a.append($("<ylab_align />").text($("#ylab_align").val()));a.append($("<grid_lty />").text($("#grid_lty").val()));e=$("#grid_col").val();e.startsWith("#")?a.append($("<grid_col />").text(e)):a.append($("<grid_col />").text("#"+
e));a.append($("<grid_lwd />").text($("#grid_lwd").val()));a.append($("<grid_x />").text($("#grid_x").val()));a.append($("<x2tlab_orient />").text($("#x2tlab_orient").val()));a.append($("<x2tlab_perp />").text($("#x2tlab_perp").val()));a.append($("<x2tlab_horiz />").text($("#x2tlab_horiz").val()));a.append($("<x2tlab_size />").text($("#x2tlab_size").val()));a.append($("<x2lab_size />").text($("#x2lab_size").val()));a.append($("<x2lab_offset />").text($("#x2lab_offset").val()));a.append($("<x2lab_align />").text($("#x2lab_align").val()));
a.append($("<y2tlab_orient />").text($("#y2tlab_orient").val()));a.append($("<y2tlab_perp />").text($("#y2tlab_perp").val()));a.append($("<y2tlab_horiz />").text($("#y2tlab_horiz").val()));a.append($("<y2tlab_size />").text($("#y2tlab_size").val()));a.append($("<y2lab_size />").text($("#y2lab_size").val()));a.append($("<y2lab_offset />").text($("#y2lab_offset").val()));a.append($("<y2lab_align />").text($("#y2lab_align").val()));a.append($("<legend_box />").text($("#legend_box").val()));0==$("#legend_inset_min").val().trim().length||
0==$("#legend_inset_max").val().trim().length?a.append($("<legend_inset />").text("c()")):a.append($("<legend_inset />").text("c("+$("#legend_inset_min").val()+","+$("#legend_inset_max").val()+")"));a.append($("<legend_ncol />").text($("#legend_ncol").val()));a.append($("<legend_size />").text($("#legend_size").val()));a.append($("<caption_weight />").text($("#caption_weight").val()));e=$("#caption_col").val();e.startsWith("#")?a.append($("<caption_col />").text(e)):a.append($("<caption_col />").text("#"+
e));a.append($("<caption_size />").text($("#caption_size").val()));a.append($("<caption_offset />").text($("#caption_offset").val()));a.append($("<caption_align />").text($("#caption_align").val()));a.append($("<ci_alpha />").text($("#ci_alpha").val()));a.append($("<eqbound_low />").text($("#eqbound_low").val()));a.append($("<eqbound_high />").text($("#eqbound_high").val()));var g=sortSeries();b=[];d=[];c=[];f=[];var k=[],h=[],p=[],m=[],q=[],t=[],u=[],A=[],z=[];for(e=0;e<g.length;e++)if(""!==g[e].order)b.push('"'+
g[e].plot_ci+'"'),"No"===g[e].hide?d.push("TRUE"):d.push("FALSE"),"No"===g[e].show_signif?A.push("FALSE"):A.push("TRUE"),c.push('"'+g[e].color+'FF"'),f.push(g[e].pch),k.push('"'+g[e].type+'"'),h.push(g[e].lty),p.push(g[e].lwd),m.push(g[e].con_series),q.push(g[e].show_legend),t.push(g[e].order),u.push('"'+g[e].legend+'"');else{var w=g[e].title,y=w.split("(")[0];w=w.split("(")[1].split(")")[0];z.push($("<line />").attr("type",y).attr("line_pos",w).attr("color",g[e].color).attr("lty",g[e].lty).attr("lwd",
g[e].lwd))}if(0<z.length){e=$("<lines />");for(g=0;g<z.length;g++)e.append(z[g]);a.append(e)}a.append($("<plot_ci />").text("c("+b.join()+")"));a.append($("<show_signif />").text("c("+A.join()+")"));a.append($("<plot_disp />").text("c("+d.join()+")"));a.append($("<colors />").text("c("+c.join()+")"));a.append($("<pch />").text("c("+f.join()+")"));a.append($("<type />").text("c("+k.join()+")"));a.append($("<lty />").text("c("+h.join()+")"));a.append($("<lwd />").text("c("+p.join()+")"));a.append($("<con_series />").text("c("+
m.join()+")"));a.append($("<show_legend />").text("c("+q.join()+")"));a.append($("<order_series />").text("c("+t.join()+")"));a.append($("<plot_cmd />").text($("#plot_cmd").val()));a.append($("<legend />").text("c("+u.join()+")"));a.append($("<create_html />").text("TRUE"));0===$("#y1_lim_min").val().trim().length||0===$("#y1_lim_max").val().trim().length?a.append($("<y1_lim />").text("c()")):a.append($("<y1_lim />").text("c("+$("#y1_lim_min").val()+","+$("#y1_lim_max").val()+")"));0===$("#x1_lim_min").val().trim().length||
0===$("#x1_lim_max").val().trim().length?a.append($("<x1_lim />").text("c()")):a.append($("<x1_lim />").text("c("+$("#x1_lim_min").val()+","+$("#x1_lim_max").val()+")"));a.append($("<y1_bufr />").text($("#y1_bufr").val()));0===$("#y2_lim_min").val().trim().length||0===$("#y2_lim_max").val().trim().length?a.append($("<y2_lim />").text("c()")):a.append($("<y2_lim />").text("c("+$("#y2_lim_min").val()+","+$("#y2_lim_max").val()+")"));return a}
...
$("#dump_points2").prop("checked",!1);$("#indy1_stag").prop("checked",!1);$("#indy2_stag").prop("checked",!1);$("#start_from_zero").prop("checked",!1);$("#varianceInflationFactor").prop("checked",!0);$("#ci_alpha").val("0.05");$("#eqbound_low").val("-0.001");$("#eqbound_high").val("0.001");$("#plot_type").val("png16m");$("#plot_height").val("8.5");$("#plot_width").val("11");$("#plot_units").val("in");$("#cex").val("1");$("#plot_res").val("72");$("#mar_bottom").val("8");$("#mar_left").val("4");$("#mar_top").val("5");
$("#mar_right").val("4");$("#mgp_title").val("1");$("#mgp_labels").val("1");$("#mgp_line").val("0");$("#title_align").val("0.5");$("#title_offset").val("-2");$("#title_size").val("1.4");$("#title_weight").val("2");$("#grid_lty").val("3");$("#grid_lwd").val("1");$("#grid_x").val("listX");$("#plot_cmd").val("");$("#xlab_align").val("0.5");$("#xlab_offset").val("2");$("#xlab_size").val("1");$("#xlab_weight").val("1");$("#xtlab_horiz").val("0.5");$("#xtlab_perp").val("-0.75");$("#xtlab_size").val("1");
$("#xtlab_freq").val("0");$("#xtlab_orient").val("1");$("#x2lab_align").val("0.5");$("#x2lab_offset").val("-0.5");$("#x2lab_size").val("0.8");$("#x2lab_weight").val("1");$("#x2tlab_horiz").val("0.5");$("#x2tlab_perp").val("1");$("#x2tlab_size").val("0.8");$("#x2tlab_orient").val("1");$("#ylab_align").val("0.5");$("#ylab_offset").val("-2");$("#ylab_size").val("1");$("#ylab_weight").val("1");$("#ytlab_horiz").val("0.5");$("#ytlab_perp").val("0.5");$("#ytlab_size").val("1");$("#ytlab_orient").val("1");
$("#y1_lim_min").val("");$("#y1_lim_max").val("");$("#y1_bufr").val("0.04");$("#y2lab_align").val("0.5");$("#y2lab_offset").val("1");$("#y2lab_size").val("1");$("#y2lab_weight").val("1");$("#y2tlab_horiz").val("0.5");$("#y2tlab_perp").val("0.5");$("#y2tlab_size").val("1");$("#y2tlab_orient").val("1");$("#y2_bufr").val("0");$("#legend_size").val("0.8");$("#legend_inset_min").val("0");$("#legend_inset_max").val(" -.25");$("#legend_box").val("o");$("#legend_ncol").val("3");$("#caption_align").val("0");
$("#caption_offset").val("3");$("#caption_size").val("0.8");$("#caption_weight").val("1");$("#x1_lim_min").val("");$("#x1_lim_max").val("")}
...
$("#fixed_var_val_"+b);var k=$("#fixed_var_val_date_period_button_"+b);var h=$("#fixed_var_val_date_range_button_"+b);var p=$("#fixed_var_val_date_range_"+b);var m=$("#fixed_var_val_date_period_"+b);$("#fix_var_event_equal_"+b).prop("checked",c).prop("disabled",!c)}else b=parseInt(a)+1,fixed_var_indexes.push(b),f=$("#fixed_var_"+a).clone(!1),f.prop("id","fixed_var_"+b),f.css("display",""),f.val(f.find(" option:first").val());e||(e=$('<button id="remove_fixed_var_'+b+'" class="remove_fixed_var">Remove</button>'),
...
$(this).text().match(/\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/g);a&&0!==a.length||(a=$(this).text().split(":"));for(var b=0;b<a.length;b++)d.push(a[b])});"fcst_init_beg"==e||"fcst_valid_beg"==e||"fcst_valid"==e||"fcst_init"==e?$("#series_var_val_y1_date_period_button_"+(b+1)).css("display","block"):$("#series_var_val_y1_date_period_button_"+(b+1)).css("display","none");updateSeriesVarValHist(b+1,d)}else series_var_y1_indexes=[],d=$("#series_var_1_1").first().val(),updateSeriesVarValHist(1,d);updatePlotFix();
$("input[name=normalized_histogram][value="+$(initXML.find("plot").find("normalized_histogram")).text()+"]").prop("checked",!0)}
...
0<initXML.find("plot").find("summary_curve").children().length){b=initXML.find("plot").find("summary_curve").children();for(a=0;a<b.length;a++)$("#summary_curve option[value='"+b[a]+"']").prop("selected",!0);try{$("#summary_curve").multiselect("refresh")}catch(c){}}a=!0;a=$(initXML.find("plot").find("rely_event_hist")).text();"TRUE"===a||"true"===a?$("#rely_event_hist").prop("checked",!0):$("#rely_event_hist").prop("checked",!1);initXML.find("plot").find("add_skill_line")&&(a=$(initXML.find("plot").find("add_skill_line")).text(),
"true"===a?$("#add_skill_line").prop("checked",!0):$("#add_skill_line").prop("checked",!1));initXML.find("plot").find("inset_hist")&&(a=$(initXML.find("plot").find("inset_hist")).text(),"true"===a?$("#inset_hist").prop("checked",!0):$("#inset_hist").prop("checked",!1));initXML.find("plot").find("add_noskill_line")&&(a=$(initXML.find("plot").find("add_noskill_line")).text(),"true"===a?$("#add_noskill_line").prop("checked",!0):$("#add_noskill_line").prop("checked",!1));initXML.find("plot").find("add_reference_line")&&
(a=$(initXML.find("plot").find("add_reference_line")).text(),"true"===a?$("#add_reference_line").prop("checked",!0):$("#add_reference_line").prop("checked",!1));initXML.find("plot").find("reference_line_col")&&(a=$(initXML.find("plot").find("reference_line_col")).text(),7<a.length&&(a=a.substring(0,7)),$("#reference_line_col").colorpicker("setColor",a));initXML.find("plot").find("noskill_line_col")&&(a=$(initXML.find("plot").find("noskill_line_col")).text(),7<a.length&&(a=a.substring(0,7)),$("#noskill_line_col").colorpicker("setColor",
a));loadXMLStatistics();$("#event_equal").prop("checked","true"==$(initXML.find("plot").find("event_equal")).text()).trigger("change")}
...
c).children(),a=0;a<g.length;a++){var p=[];var m=!1;h=$(g[a]).attr("name");if(0===a&&1===c)try{$("#series_var_"+f+"_"+(a+1)).val(h).multiselect("refresh")}catch(t){}else{"Taylor"===currentTab?addSeriesVarHist():"Perf"===currentTab?addSeriesVarPerf():addSeriesVar(f);try{$("#series_var_"+f+"_"+(a+1)).val(h).multiselect("refresh")}catch(t){}}$(g[a]).find("val").each(function(){var a=$(this).text().match(/\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/g);a&&0!==a.length||(a=$(this).text().split(":"));2<a.length&&
(m=!0);for(var b=0;b<a.length;b++)p.push(a[b])});"fcst_init_beg"==h||"fcst_valid_beg"==h||"fcst_valid"==h||"fcst_init"==h?($("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","block"),$("#series_var_val_"+f+"_date_range_button_"+(a+1)).css("display","block")):($("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","none"),$("#series_var_val_"+f+"_date_range_button_"+(a+1)).css("display","none"));updateSeriesVarVal(f,a+1,p);!0===m?$("#group_series_var_"+f+"_"+(a+1)).prop("checked",
!0):$("#group_series_var_"+f+"_"+(a+1)).prop("checked",!1)}else p=$("#series_var_"+f+"_1").first().val(),updateSeriesVarVal(f,1,p)}updatePlotFixSeries();$("#txtPlotCond").val(initXML.find("plot").find("plot_cond").text());try{$("#annotation").val(initXML.find("plot").find("annotation_template").text())}catch(t){}"mode"===b&&updateIndyVar(b);h=$(initXML.find("plot").find("indep")[0]).attr("name");try{$("#indy_var").val(h).multiselect("refresh")}catch(t){}"fcst_init_beg"===h||"fcst_valid_beg"===h||
"fcst_valid"===h||"fcst_init"===h?($("#date_period_button").css("display","block"),$("#date_range_button").css("display","block")):($("#date_period_button").css("display","none"),$("#date_range_button").css("display","none"));var q=[];indy_var_vals_to_attr={};$(initXML.find("plot").find("indep")[0]).find("val").each(function(){q.push($(this).text());var a={};a.label=$(this)[0].attributes.label.nodeValue;a.plot_val=$(this)[0].attributes.plot_val.nodeValue;indy_var_vals_to_attr[$(this).text()]=a});
...
b).val();0===g.length&&(g=3600);g=parseFloat(g);"min"===k?g*=60:"hours"===k?g*=3600:"days"===k&&(g*=86400);var h;"fixed_var_val"===a?h=$(fixVarValResponse[b]).find("val"):"series_var_val_y1"===a?h=$(seriesY1VarValResponse[b]).find("val"):"series_var_val_y2"===a&&(h=$(seriesY2VarValResponse[b]).find("val"));for(c=c.clone();c<=f;){k=c.format("YYYY-MM-DD HH:mm:ss");for(var p=!1,m=0;m<h.length;m++)if($(h[m]).text()==k){p=!0;break}p&&$("#"+a+"_"+b).find('option[value="'+k+'"]').prop("selected",!0);c.add(g,
...
3600);f=parseInt(f);"min"===g?f*=60:"hours"===g?f*=3600:"days"===g&&(f*=86400);g=$(previousIndVarValResponse).find("val");for(e=e.clone();e<=c;){for(var k=e.format("YYYY-MM-DD HH:mm:ss"),h=!1,p=0;p<g.length;p++)if($(g[p]).text()===k){h=!0;break}h&&d.find('option[value="'+k+'"]').prop("selected",!0);e.add(f,"seconds")}try{d.multiselect("refresh")}catch(m){}}$(this).dialog("close")}},open:function(e,c){e=d.val();null==e&&(e=[]);populateIndyVarVal(e);e=$(previousIndVarValResponse).find("val");c=a.val();
var f=b.val();a.empty();b.empty();for(var g=0;g<e.length;g++){var k=$(e[g]),h=$("<option/>").attr("value",k.text()).text(k.text());k.text()==c&&h.prop("selected",!0);a.append(h);k.text()===f?h.prop("selected",!0):h.prop("selected",!1);b.append(h.clone())}b.find("option").last().prop("selected",!0);1<e.length&&(c=moment.tz($(e[0]).text(),"YYYY-MM-DD HH:mm:ss","UTC"),e=moment.tz($(e[1]).text(),"YYYY-MM-DD HH:mm:ss","UTC"),e=moment.duration({from:c,to:e}),$("#date_period_by").val(e.as("hours")))}})}
...
a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_sql").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"data/"+resultName+".yaml",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("plot_yaml").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_yaml").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".log",dataType:"text",contentType:"text/plain",
mimeType:"text",complete:function(a){document.getElementById("plot_log").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_log").innerHTML=""}});$("#ui-tabs-1").empty();$("#plot_data_url").prop("href",urlOutput+"data/"+resultName+".data");$("#y1_points_url").prop("href",urlOutput+"data/"+resultName+".points1");$("#y2_points_url").prop("href",urlOutput+"data/"+resultName+".points2")}
...
width:30,align:"center"},{name:"show_legend",index:"show_legend",width:30,align:"center",editable:!0,edittype:"select",formatter:"select",editoptions:{value:"1:Yes;0:No"},sortable:!1},{name:"legend",index:"legend",sortable:!1,width:150,align:"left",editable:!0}],rowNum:-1,height:"auto",pager:"#pagerdt",pginput:!1,pgtext:!1,pgbuttons:!1,viewrecords:!0,caption:"Series Formatting",cmTemplate:{title:!1},cellEdit:!0,hoverrows:!1,url:"clientArray",cellsubmit:"clientArray",afterInsertRow:function(a,b){$("#color_"+
...
b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",a[b].id).order=b+1;var c=$("#plot_data").multiselect("getChecked").val();$("#series1Y2").empty();$("#series1Y1").empty();$("#series2Y2").empty();$("#series2Y1").empty();$("#y1AxisDiff").prop("checked",!0);$("#y2AxisDiff").removeAttr("checked");$("#series1Y2").prop("disabled",!0);$("#series2Y2").prop("disabled",!0);$("#series1Y1").removeProp("disabled");$("#series2Y1").removeProp("disabled");$("#y2AxisDiff").removeProp("disabled");
$("#y1AxisDiff").removeProp("disabled");series1Names=[];series2Names=[];for(b=0;b<a.length;b++){var d=!1;if(0!=a[b].title.indexOf("DIFF")&&0!=a[b].title.indexOf("RATIO")&&0!=a[b].title.indexOf("SS")&&0!=a[b].title.indexOf("ETB"))if("mode"==c){var e=a[b].title.split(" ");-1<listStatModeRatio.indexOf(e[e.length-1])&&(d=!0)}else d=!0;d&&(-1!==a[b].y_axis.indexOf("2")?($("#series1Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y2").append($("<option></option>").attr("value",
a[b].title).text(a[b].title)),series2Names.push(a[b].title)):($("#series1Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series1Names.push(a[b].title)))}0<$("#series1Y2 option").length&&0<$("#series1Y1 option").length?createNewDerivedSeriesName(1):(0==$("#series1Y2 option").length&&($("#y2AxisDiff").attr("disabled",!0),createNewDerivedSeriesName(1)),0==$("#series1Y1 option").length&&
...
lines.push({type:a,line_pos:c});b.dialog("close");"Hist"===currentTab||"Roc"===currentTab||"Rely"===currentTab||"Eclv"===currentTab?updateSeriesHist():"Ens_ss"===currentTab?updateSeriesEns():updateSeries()},Cancel:function(){b.dialog("close")}},open:function(){$("input[name=derive_oper][value=DIFF]").prop("checked",!0);for(var a=$("#listdt").jqGrid("getRowData"),b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",a[b].id).order=b+1;var c=$("#plot_data").multiselect("getChecked").val();
$("#series1Y2").empty();$("#series1Y1").empty();$("#series2Y2").empty();$("#series2Y1").empty();$("#y1AxisDiff").prop("checked",!0);$("#y2AxisDiff").removeAttr("checked");$("#series1Y2").prop("disabled",!0);$("#series2Y2").prop("disabled",!0);$("#series1Y1").removeProp("disabled");$("#series2Y1").removeProp("disabled");$("#y2AxisDiff").removeProp("disabled");$("#y1AxisDiff").removeProp("disabled");series1Names=[];series2Names=[];for(b=0;b<a.length;b++){var d=!1;if(0!=a[b].title.indexOf("DIFF")&&0!=
a[b].title.indexOf("RATIO")&&0!=a[b].title.indexOf("SS")&&0!=a[b].title.indexOf("ETB"))if("mode"==c){var e=a[b].title.split(" ");-1<listStatModeRatio.indexOf(e[e.length-1])&&(d=!0)}else d=!0;d&&(-1!==a[b].y_axis.indexOf("2")?($("#series1Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),$("#series2Y2").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series2Names.push(a[b].title)):($("#series1Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),
$("#series2Y1").append($("<option></option>").attr("value",a[b].title).text(a[b].title)),series1Names.push(a[b].title)))}0<$("#series1Y2 option").length&&0<$("#series1Y1 option").length?createNewDerivedSeriesName(1):(0==$("#series1Y2 option").length&&($("#y2AxisDiff").attr("disabled",!0),createNewDerivedSeriesName(1)),0==$("#series1Y1 option").length&&($("#y1AxisDiff").attr("disabled",!0),$("#y1AxisDiff").removeAttr("checked"),$("#y2AxisDiff").prop("checked",!0),$("#series1Y2").removeAttr("disabled"),
...
"#pagerdt",{caption:"Apply defaults",title:"Apply defaults",buttonicon:"ui-icon-transferthick-e-w",onClickButton:function(){alert("defaults..")}});$("#listdt").setGridWidth($(window).width()-20);var d=$("#unavailableDiffCurveDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){d.dialog("close")}}}),e=$("#unavailableLineDialogForm").dialog({autoOpen:!1,height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){e.dialog("close")}}}),c=$("#incorrectDiffCurveDialogForm").dialog({autoOpen:!1,
height:"auto",width:"auto",modal:!0,buttons:{Ok:function(){c.dialog("close")}}});$(window).bind("resize",function(){$("#listdt").setGridWidth($(window).width()-20)}).trigger("resize");$("#listdt").jqGrid("sortableRows",{update:function(a,b){a=$("#listdt").jqGrid("getRowData");for(b=0;b<a.length;b++)$("#listdt").jqGrid("setCell",a[b].id,"order",b+1),$("#listdt").jqGrid("getLocalRow",a[b].id).order=b+1}});$("#pagerdt_left table.navtable tbody tr").append('<td><div ><input type="checkbox"  style="margin:0 4px;" id="seriesLock"/><label for="seriesLock">Lock Formatting</label></div></td>');
...
$("#reset_formatting").button({icons:{primary:"ui-icon-arrowrefresh-1-w"},text:!1}).click(function(){resetFormatting()});$("#plot_image").click(function(){"IMG"===document.getElementById("plot_image").firstChild.tagName&&viewImage(resultName.replace("plot_",""))});var g=0,k;if(null!=initXML){if((k=initXML.find("plot"))&&0<k.length){var h=initXML.find("plot").find("template")[0].innerHTML;"series_plot.R_tmpl"===h||"revision_series_plot.R_tmpl"===h?currentTab="Series":"box_plot.R_tmpl"===h||"revision_box_plot.R_tmpl"===
h?currentTab="Box":"bar_plot.R_tmpl"===h?currentTab="Bar":"rhist.R_tmpl"===h||"phist.R_tmpl"===h||"relp.R_tmpl"===h?currentTab="Hist":"roc.R_tmpl"===h?currentTab="Roc":"eclv.R_tmpl"===h?currentTab="Eclv":"rely.R_tmpl"===h?currentTab="Rely":"ens_ss.R_tmpl"===h?currentTab="Ens_ss":"performance.R_tmpl"===h?currentTab="Perf":"taylor_plot.R_tmpl"===h?currentTab="Taylor":"contour_plot.R_tmpl"===h&&(currentTab="Contour")}else currentTab="Series";var p=$("#plot_config").find("a");for(h=0;h<p.length;h++)if(p[h].text===
currentTab){g=h;break}}$("#plot_config").tabs({active:g,beforeActivate:function(a,b){a=b.oldTab.attr("aria-controls");$("#"+a).empty();cleanUp();currentTab=b.newTab.text()},beforeLoad:function(a,b){$.ajaxSetup({cache:!0});b.jqXHR.error(function(){b.panel.html("Couldn't load this tab. We'll try to fix this as soon as possible. If this wouldn't be a demo.")})}});null!=initXML&&k&&0<k.length?($("#plot_title").val($(initXML.find("plot").find("tmpl").find("title")).text()),$("#x_label_title").val($(initXML.find("plot").find("tmpl").find("x_label")).text()),
...
$("#dump_points1").prop("checked","true"==$(initXML.find("plot").find("dump_points1")).text()),$("#dump_points2").prop("checked","true"==$(initXML.find("plot").find("dump_points2")).text()),$("#indy1_stag").prop("checked","true"==$(initXML.find("plot").find("indy1_stag")).text()),$("#indy2_stag").prop("checked","true"==$(initXML.find("plot").find("indy2_stag")).text()),$("#start_from_zero").prop("checked","true"==$(initXML.find("plot").find("start_from_zero")).text()),$("#varianceInflationFactor").prop("checked",
"true"===$(initXML.find("plot").find("varianceinflationfactor")).text()),$("#ci_alpha").val($(initXML.find("plot").find("ci_alpha")).text()),$("#eqbound_low").val($(initXML.find("plot").find("eqbound_low")).text()),$("#eqbound_high").val($(initXML.find("plot").find("eqbound_high")).text()),$("#plot_type").val($(initXML.find("plot").find("plot_type")).text()),$("#plot_height").val($(initXML.find("plot").find("plot_height")).text()),$("#plot_width").val($(initXML.find("plot").find("plot_width")).text()),
$("#plot_units").val($(initXML.find("plot").find("plot_units")).text()),$("#cex").val($(initXML.find("plot").find("cex")).text()),$("#plot_res").val($(initXML.find("plot").find("plot_res")).text()),h=$(initXML.find("plot").find("mar")).text().replace("c(","").replace(")","").split(","),$("#mar_bottom").val(h[0]),$("#mar_left").val(h[1]),$("#mar_top").val(h[2]),$("#mar_right").val(h[3]),h=$(initXML.find("plot").find("mgp")).text().replace("c(","").replace(")","").split(","),$("#mgp_title").val(h[0]),
$("#mgp_labels").val(h[1]),$("#mgp_line").val(h[2]),$("#title_align").val($(initXML.find("plot").find("title_align")).text()),$("#title_offset").val($(initXML.find("plot").find("title_offset")).text()),$("#title_size").val($(initXML.find("plot").find("title_size")).text()),$("#title_weight").val($(initXML.find("plot").find("title_weight")).text()),$("#grid_lty").val($(initXML.find("plot").find("grid_lty")).text()),$("#grid_lwd").val($(initXML.find("plot").find("grid_lwd")).text()),$("#grid_col").colorpicker("setColor",
...
$("#ylab_size").val($(initXML.find("plot").find("ylab_size")).text()),$("#ylab_weight").val($(initXML.find("plot").find("ylab_weight")).text()),$("#ytlab_horiz").val($(initXML.find("plot").find("ytlab_horiz")).text()),$("#ytlab_perp").val($(initXML.find("plot").find("ytlab_perp")).text()),$("#ytlab_size").val($(initXML.find("plot").find("ytlab_size")).text()),$("#ytlab_orient").val($(initXML.find("plot").find("ytlab_orient")).text()),h=$(initXML.find("plot").find("y1_lim")).text().replace("c(","").replace(")",
"").split(","),$("#y1_lim_min").val(h[0]),$("#y1_lim_max").val(h[1]),$("#y1_bufr").val($(initXML.find("plot").find("y1_bufr")).text()),h=$(initXML.find("plot").find("x1_lim")).text().replace("c(","").replace(")","").split(","),$("#x1_lim_min").val(h[0]),$("#x1_lim_max").val(h[1]),$("#y2lab_align").val($(initXML.find("plot").find("y2lab_align")).text()),$("#y2lab_offset").val($(initXML.find("plot").find("y2lab_offset")).text()),$("#y2lab_size").val($(initXML.find("plot").find("y2lab_size")).text()),
$("#y2lab_weight").val($(initXML.find("plot").find("y2lab_weight")).text()),$("#y2tlab_horiz").val($(initXML.find("plot").find("y2tlab_horiz")).text()),$("#y2tlab_perp").val($(initXML.find("plot").find("y2tlab_perp")).text()),$("#y2tlab_size").val($(initXML.find("plot").find("y2tlab_size")).text()),$("#y2tlab_orient").val($(initXML.find("plot").find("y2tlab_orient")).text()),h=$(initXML.find("plot").find("y2_lim")).text().replace("c(","").replace(")","").split(","),$("#y2_lim_min").val(h[0]),$("#y2_lim_max").val(h[1]),
$("#y2_bufr").val($(initXML.find("plot").find("y1_bufr")).text()),$("#legend_size").val($(initXML.find("plot").find("legend_size")).text()),h=$(initXML.find("plot").find("legend_inset")).text().replace("c(","").replace(")","").split(","),$("#legend_inset_min").val(h[0]),$("#legend_inset_max").val(h[1]),$("#legend_box").val($(initXML.find("plot").find("legend_box")).text()),$("#legend_ncol").val($(initXML.find("plot").find("legend_ncol")).text()),$("#caption_align").val($(initXML.find("plot").find("caption_align")).text()),
$("#caption_offset").val($(initXML.find("plot").find("caption_offset")).text()),$("#caption_size").val($(initXML.find("plot").find("caption_size")).text()),$("#caption_weight").val($(initXML.find("plot").find("caption_weight")).text()),h=$(initXML.find("plot").find("caption_col")).text(),7<h.length&&(h=h.substring(0,7)),$("#caption_col").colorpicker("setColor",h)):resetFormatting();$("#release").on("click",function(){var a=window.open("https://metviewer.readthedocs.io/en/latest/");a?a.focus():alert("Please allow popups for this site")});
g=$("#categories11");for(h=0;h<categories.length;h++){p=$(categories[h]);k=$('<ul data-label="'+p.attr("name")+'">');p=p.find("db");for(var m=0;m<p.length;m++){var q=$(p[m]),t=q.find("val"),u=q.find("desc");q=$('<li data-value="'+t.text()+'" title="'+u.text()+'">');t=$('<input name="multiselect_database" type="checkbox" disabled value="'+t.text()+'" title="'+u.text()+'" id="'+t.text()+'" /><label for="'+t.text()+'" >'+t.text()+"</label>lab");q.prepend(t);k.append(q)}g.append(k)}$(".multilevel-dropdown").multilevelDropdown();
...
d=parseInt(d);"min"===e?d*=60:"hours"===e?d*=3600:"days"===e&&(d*=86400);e=moment.tz(a.date1,"UTC");var k=moment.tz(a.date2,"UTC");g.multiselect("uncheckAll");for(var h=$('[name="multiselect_indy_var_val"]');e<=k;){for(var p=e.format("YYYY-MM-DD HH:mm:ss"),m=!1,q=0;q<c.length;q++)if($(c[q]).text()===p){m=!0;break}if(m){try{var t=e.format(f)}catch(A){console.log("formatting error"),console.error(A),t=p}m=g.find('option[value="'+p+'"]');m.prop("selected",!0);if(!b)for(indy_var_vals_to_attr={},q=0;q<
h.length;q++){var u=$(h[q]);if(u.attr("value")===p){p=u.attr("id");$("#"+p+"_label").val(t);indy_var_vals_to_attr[h[q].value]=a;break}}m.text(t)}e.add(d,"seconds")}b&&g.multiselect("option","indy_var_vals_to_attr",indy_var_vals_to_attr);try{g.multiselect("refresh")}catch(A){console.log(A)}}
