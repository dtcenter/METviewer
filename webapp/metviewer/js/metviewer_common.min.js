...
function updateStatVal(){var a=getSelectedDatabases(),b=$("#fcst_var_y1_1").multiselect("getChecked").val();$("#fcst_stat_y1_1").empty();var d=$("#fcst_stat_y1_1");b&&(b=b.replace("&","&amp;").replace(">","&gt;").replace("<","&lt;"));d.empty();$.ajax({async:!1,url:"servlet",type:"POST",dataType:"xml",contentType:"application/xml",data:'<?xml version="1.0" encoding="UTF-8"?><request><db_con>'+a+"</db_con><list_stat><stat_fcst_var>"+b+"</stat_fcst_var></list_stat></request>",error:function(){},success:function(a){var b=
$(a).find("val");a=[];if(0<b.length){for(var e=0;e<b.length;e++){var g=$(b[e]);if(0==e||0!=e&&g.text()!==$(b[e-1]).text()){var h=g.text().formatAll();g=$("<option />",{value:h,text:h,title:value_to_desc_map[g.text()]});a.push(g)}else 0!=e&&g.text()===$(b[e-1]).text()&&a[a.length-1].text(a[a.length-1].text()+"*")}g=$("input[name='multiselect_database']:checked").length-1;for(e=0;e<a.length;e++)a[e].text()!==a[e][0].value&&(b=a[e].text().substring(a[e][0].value.length,a[e].text().length).length,b===
g?a[e].text(a[e][0].value+"*"):a[e].text(a[e][0].value)),a[e].appendTo(d)}else g=$("<option />",{value:"N/A",text:"N/A"}),g.appendTo(d);try{d.multiselect("refresh")}catch(k){console.log(k)}updateSeries()}})}
...
$("#aggregation_statistics").hide();$("#revision_statistics").hide();"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_ctc")).text()?$("#calc_stat").val("ctc"):"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_sl1l2")).text()?$("#calc_stat").val("sl1l2"):"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_grad")).text()?$("#calc_stat").val("grad"):"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_sal1l2")).text()?$("#calc_stat").val("sal1l2"):"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_vl1l2")).text()&&
$("#calc_stat").val("vl1l2");try{$("#calc_stat").multiselect("refresh")}catch(b){}}else if(0<initXML.find("plot").find("revis_stat").length){$("input[name=statistics][value=revision_statistics]").prop("checked",!0);$("#calculations_statistics").hide();$("#aggregation_statistics").hide();$("#revision_statistics").show();"TRUE"==$(initXML.find("plot").find("revis_stat").find("calc_ctc")).text()?$("#revis_stat").val("ctc"):"TRUE"==$(initXML.find("plot").find("calc_stat").find("calc_sl1l2")).text()?$("#revis_stat").val("sl1l2"):
...
"&lt;").replace(/>/g,"&gt;").replace(/ /g,"&nbsp;").replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_xml").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".sql",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("plot_sql").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_sql").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"scripts/"+resultName+".R",dataType:"text",
contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("r_script").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("r_script").innerHTML=""}});$.ajax({type:"GET",url:urlOutput+"xml/"+resultName+".log",dataType:"text",contentType:"text/plain",mimeType:"text",complete:function(a){document.getElementById("plot_log").innerHTML=a.responseText.replace(/\n/g,"<br />")},error:function(){document.getElementById("plot_log").innerHTML=
""}});$("#ui-tabs-1").empty();$("#r_data_url").prop("href",urlOutput+"data/"+resultName+".data");$("#y1_points_url").prop("href",urlOutput+"data/"+resultName+".points1");$("#y2_points_url").prop("href",urlOutput+"data/"+resultName+".points2")}
...
west__fxName:"slide",autoBindCustomButtons:!0,south__togglerLength_open:0,west__onopen_start:function(){refreshHistory()},useStateCookie:!1});$("#plot_display_inner").tabs({heightStyle:"content",beforeActivate:function(a,b){currentPlotTab=b.newPanel.attr("aria-labelledby")},load:function(a,b){a=$("<pre/>").text(b.panel.text());b.panel.empty();b.panel.append(a)},create:function(a,b){}});$("#tab-south").tabs({collapsible:!1});innerLayout=$("#plot_display").layout({name:"inner",closable:!1,center__paneSelector:"#plot_display_inner",
...
d=parseInt(d);"min"===e?d*=60:"hours"===e?d*=3600:"days"===e&&(d*=86400);e=moment.tz(a.date1,"UTC");var h=moment.tz(a.date2,"UTC");g.multiselect("uncheckAll");for(var k=$('[name="multiselect_indy_var_val"]');e<=h;){for(var n=e.format("YYYY-MM-DD HH:mm:ss"),m=!1,q=0;q<c.length;q++)if($(c[q]).text()===n){m=!0;break}if(m){try{var t=e.format(f)}catch(z){console.log("formatting error"),console.error(z),t=n}m=g.find('option[value="'+n+'"]');m.prop("selected",!0);if(!b)for(indy_var_vals_to_attr={},q=0;q<
k.length;q++){var u=$(k[q]);if(u.attr("value")===n){n=u.attr("id");$("#"+n+"_label").val(t);indy_var_vals_to_attr[k[q].value]=a;break}}m.text(t)}e.add(d,"seconds")}b&&g.multiselect("option","indy_var_vals_to_attr",indy_var_vals_to_attr);try{g.multiselect("refresh")}catch(z){console.log(z)}}
