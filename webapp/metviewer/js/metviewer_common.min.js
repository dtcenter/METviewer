...
k||"fcst_valid"==k||"fcst_init"==k?$("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","block"):$("#series_var_val_"+f+"_date_period_button_"+(a+1)).css("display","none");updateSeriesVarVal(f,a+1,n);m&&$("#group_series_var_"+f+"_"+(a+1)).prop("checked",!0)}else n=$("#series_var_"+f+"_1").first().val(),updateSeriesVarVal(f,1,n)}updatePlotFixSeries();$("#txtPlotCond").val(initXML.find("plot").find("plot_cond").text());try{$("#annotation").val(initXML.find("plot").find("annotation_template").text())}catch(t){}"mode"===
b&&updateIndyVar(b);k=$(initXML.find("plot").find("indep")[0]).attr("name");try{$("#indy_var").val(k).multiselect("refresh")}catch(t){}"fcst_init_beg"===k||"fcst_valid_beg"===k||"fcst_valid"===k||"fcst_init"===k?$("#date_period_button").css("display","block"):$("#date_period_button").css("display","none");var q=[];indy_var_vals_to_attr={};$(initXML.find("plot").find("indep")[0]).find("val").each(function(){q.push($(this).text());var a={};a.label=$(this)[0].attributes.label.nodeValue;a.plot_val=$(this)[0].attributes.plot_val.nodeValue;
