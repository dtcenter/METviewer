...
error:function(){try{e.multiselect("refresh")}catch(k){}},success:function(c){fixVarValResponse[a]=c;var d=$(c).find("val");c=[];if(0<d.length){for(var f=0;f<d.length;f++){var g=$(d[f]);var k=0<=$.inArray(g.text().replace("&gt;",">").replace("&lt;","<").replace("&amp;","&"),b);0===f||0!==f&&g.text()!==$(d[f-1]).text()?(g=g.text().formatAll(),k=$("<option />",{value:g,text:g,selected:k}),c.push(k)):0!==f&&g.text()===$(d[f-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}k=$("input[name='multiselect_database']:checked").length-
1;for(f=0;f<c.length;f++)c[f].text()!==c[f][0].value&&(d=c[f].text().substring(c[f][0].value.length,c[f].text().length).length,d===k?c[f].text(c[f][0].value+"*"):c[f].text(c[f][0].value)),c[f].appendTo(e);try{e.multiselect("refresh")}catch(t){}}else{k=$("<option />",{value:"N/A",text:"N/A"});k.appendTo(e);try{e.multiselect("refresh")}catch(t){}}b=[]}})}}
...
(a=$(initXML.find("plot").find("add_reference_line")).text(),"true"===a?$("#add_reference_line").prop("checked",!0):$("#add_reference_line").prop("checked",!1));loadXMLStatistics()}
