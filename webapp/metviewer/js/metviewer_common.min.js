...
fcst_init_beg:"FCST_INIT_BEG",init_hour:"INIT_HOUR",interp_pnts:"INTERP_PNTS",vx_mask:"VX_MASK"},indy_var_value_to_title_mode_map={fcst_lead:"FCST_LEAD",model:"MODEL",descr:"DESC",fcst_lev:"FCST_LEV",fcst_thr:"FCST_THR",fcst_valid:"FCST_VALID",valid_hour:"VALID_HOUR",fcst_init:"FCST_INIT",init_hour:"INIT_HOUR",fcst_rad:"FCST_RAD",vx_mask:"VX_MASK"},fcst_var_y1_indexes=[1],series_var_y1_indexes=[1],fcst_var_y2_indexes=[],series_var_y2_indexes=[],fixed_var_indexes=[],date_period_indexes=[1],firstSeriesFormatting=
{order:"1",hide:"No",plot_ci:"none",color:"",pch:20,type:"b",lty:"1",lwd:"1",show_signif:"No",con_series:"1",legend:"",id:"1"},seriesDiffY1=[],seriesDiffY2=[],previousIndVarValResponse,fixVarValResponse={},seriesY1VarValResponse={},seriesY2VarValResponse={},xml='<?xml version="1.0"?><Request/>',group_name_to_value_map,indy_var_vals_to_attr,fcst_vars=[],fcst_vars_stats=[];
...
$("#fcst_stat_mode_"+a+"_"+b),m="",p="";0<c.length&&2==c.split("_").length&&(m=c.split("_")[0],p=c.split("_")[1]);"ACOVACOV"==m&&(m="ACOV");c=!1;var t=0==k.find("option").length;if(!t)try{k.multiselect("destroy")}catch(r){}for(f=0;f<listStatMode.length;f++)g=listStatMode[f],(h=g==m)&&(c=!0),t?(g=$("<option />",{value:g,text:g,title:value_to_desc_map[g],selected:h}),g.appendTo(k)):h&&k.val(g);k.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:225,height:300,noneSelectedText:"Select attribute stat",
...
$("input[name=statistics][value=calculations_statistics]").prop("checked",!0);$("#calc_stat").val("none");try{$("#calc_stat").multiselect("refresh")}catch(e){}$("#aggregation_statistics ").hide();$("#calculations_statistics ").show();updateSeries()}});c&&(f=$("#fcst_stat_mode_config_"+a+"_"+b),f.css("display","block"),"ACOV"==m?f.find(".non-acov").attr("disabled",!0):f.find(".non-acov").removeAttr("disabled"),2==p.length&&(p="A"+p),"ACOV"==m?"A"==p[0]?(f.find('[name="mode_stat_fcst"]').prop("checked",
...
success:function(e){seriesY1VarValResponse[a]=e;e=$($.parseXML(e)).find("val");var d;if(0<e.length)for(var h=0;h<e.length;h++){var k=$(e[h]);d=0<=$.inArray(k.text(),b);d=$("<option />",{value:k.text(),text:k.text(),selected:d});d.appendTo(c)}else d=$("<option />",{value:"N/A",text:"N/A"}),d.appendTo(c);try{c.multiselect("refresh")}catch(m){}b=[]}})}
...
dataType:"xml",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+"_field></list_val></request>",error:function(){},success:function(d){fixVarValResponse[a]=d;d=$(d).find("val");var e;if(0<d.length)for(var f=0;f<d.length;f++){var m=$(d[f]);e=0<=$.inArray(m.text(),b);e=$("<option />",{value:m.text(),text:m.text(),selected:e});e.appendTo(c)}else e=$("<option />",{value:"N/A",text:"N/A"}),e.appendTo(c);try{c.multiselect("refresh")}catch(p){}b=[]}})}
...
url:"servlet",type:"POST",dataType:"xml",data:"<request><db_con>"+c+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+"_field>"+g+"</list_val></request>",error:function(){try{e.multiselect("refresh")}catch(a){}},success:function(c){fixVarValResponse[a]=c;c=$(c).find("val");var d;if(0<c.length)for(var f=0;f<c.length;f++){var g=$(c[f]);d=0<=$.inArray(g.text(),b);d=$("<option />",{value:g.text(),text:g.text(),selected:d});d.appendTo(e)}else d=$("<option />",{value:"N/A",text:"N/A"}),d.appendTo(e);
...
function updateSeriesRhist(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],m=[],p=[],t=[],r=[],y=[],v=[],w=[],x=[],B=[],q;"Rhist"===currentTab||"Phist"===currentTab?q="Rank Histogram":"Roc"===currentTab?q="ROC Curve":"Rely"===currentTab?q="Reliability Curve":"Taylor"===currentTab&&(q=$("#fcst_var_y1_1").val());if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),l=0;l<k.length;l++)"TRUE"===k[l].trim()?k[l]="No":k[l]="Yes","TRUE"===m[l].trim()?m[l]="No":m[l]="Yes";p=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(l=0;l<p.length;l++)p[l]=
p[l].replace(/FF$/,"");t=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");r=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(x.length!=w.length)for(c=0;c<w.length;c++)x[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);var u;0==c.length&&(c[0]="");if("Roc"==currentTab||"Rely"==currentTab){try{u=$("#summary_curve").multiselect("getChecked")}catch(z){u=
[]}for(l=0;l<u.length;l++)"none"!=u[l].value&&c.push(u[l].value)}for(var A=0;A<c.length;A++){u=c[A]+" "+q;for(var n=!1,l=0;l<b.length;l++)if(b[l].title==u){d={};d.title=u;d.y_axis="Y1";d.order=e+1;d.hide=b[l].hide;d.plot_ci=b[l].plot_ci;d.show_signif=b[l].show_signif;d.color=b[l].color;d.pch=b[l].pch;d.type=b[l].type;d.lty=b[l].lty;d.lwd=b[l].lwd;d.con_series=b[l].con_series;d.legend=b[l].legend;d.id=e+1;n=!0;break}n||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=u,d.y_axis=
"Y1",d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=u,d.y_axis="Y1",d.order=w[e]&&""!=w[e]?parseInt(w[e]):parseInt(l+1),d.hide=k[e],d.plot_ci=h[e].trim(),d.show_signif=m[e],d.pch=t[e].trim(),d.type=B[e].trim(),d.lty=r[e].trim(),d.lwd=y[e].trim(),d.con_series=v[e].trim(),d.legend=x[e].trim(),d.color=p[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=u,d.y_axis="Y1",d.id=e+1,d.order=e+1));e++;g.push(d)}if(null==initXML)for(p=rainbow(g.length),l=0;l<g.length;l++)g[l].color=
p[l];g.sort(SortByOrder);for(l=0;l<g.length;l++)a.jqGrid("addRowData",l+1,g[l]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
...
(f="taylor");$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+d+"</"+f+'_field><stat><fcst_var name="NA"><val>NA</val></fcst_var></stat></list_val></request>',error:function(){},success:function(d){seriesY1VarValResponse[a]=d;d=$($.parseXML(d)).find("val");var e;if(0<d.length)for(var f=0;f<d.length;f++){var m=$(d[f]);e=0<=$.inArray(m.text(),b);e=$("<option />",{value:m.text(),text:m.text(),selected:e});e.appendTo(c)}else e=$("<option />",
{value:"N/A",text:"N/A"}),e.appendTo(c);try{c.multiselect("refresh")}catch(p){}b=[]}})}
...
function updateSeriesEns(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h,k=[],m=[],p=[],t=[],r=[],y=[],v=[],w=[],x=[],B=[],q=[];if(null!=initXML){for(var k=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),p=initXML.find("plot").find("show_signif").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),l=0;l<m.length;l++)"TRUE"==m[l].trim()?m[l]="No":m[l]="Yes","TRUE"==p[l].trim()?p[l]="No":p[l]="Yes";t=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(l=0;l<t.length;l++)t[l]=t[l].replace(/FF$/,"");r=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");y=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");
v=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");q=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,
"").replace(/"/g,"").split(",");if(B.length!=x.length)for(c=0;c<x.length;c++)B[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);if("true"===$("input:radio[name='ensss_pts_disp']:checked").val()){h=[];for(var u=0,l=0;l<c.length;l++)h[u]=c[l]+" MSE",h[u+1]=c[l]+" #PTS",u+=2;c=h}else for(l=0;l<c.length;l++)c[l]+=" MSE";for(u=0;u<c.length;u++){var A=c[u],n=!1;h=0<A.indexOf("#PTS")?"Y2":"Y1";for(l=0;l<b.length;l++)if(b[l].title==A){d={};d.title=A;d.y_axis=h;d.order=
e+1;d.hide=b[l].hide;d.plot_ci=b[l].plot_ci;d.show_signif=b[l].show_signif;d.color=b[l].color;d.pch=b[l].pch;d.type=b[l].type;d.lty=b[l].lty;d.lwd=b[l].lwd;d.con_series=b[l].con_series;d.legend=b[l].legend;d.id=e+1;n=!0;break}n||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=A,d.y_axis=h,d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=A,d.y_axis=h,d.order=x[e]&&""!=x[e]?parseInt(x[e]):parseInt(l+1),d.hide=m[e],d.plot_ci=k[e].trim(),d.show_signif=p[e],d.pch=r[e].trim(),d.type=
q[e].trim(),d.lty=y[e].trim(),d.lwd=v[e].trim(),d.con_series=w[e].trim(),d.legend=B[e].trim(),d.color=t[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=A,d.y_axis=h,d.id=e+1,d.order=e+1));e++;g.push(d)}if(null==initXML)for(t=rainbow(g.length),l=0;l<g.length;l++)g[l].color=t[l];g.sort(SortByOrder);for(l=0;l<g.length;l++)a.jqGrid("addRowData",l+1,g[l]);outerLayout.sizePane("south",$("#gbox_listdt").height())}
function updateSeriesPerf(){var a=$("#listdt");a.saveCell(lastSelRow,lastSelCol);var b=a.jqGrid("getRowData");a.jqGrid("clearGridData");var c,e=0,d={},f=$("#seriesLock").is(":checked"),g=[],h=[],k=[],m=[],p=[],t=[],r=[],y=[],v=[],w=[],x=[],B=[];if(null!=initXML){for(var h=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),k=initXML.find("plot").find("plot_disp").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),m=initXML.find("plot").find("show_signif").text().replace("c(",
...
y=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");v=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");w=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");x=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,
"").replace(/"/g,"").split(",");if(x.length!=w.length)for(c=0;c<w.length;c++)x[c]=""}c=permuteSeries(createSeriesMapForPermutationEns(series_var_y1_indexes,"y1"),0);for(var l=0;l<c.length;l++){for(var u=c[l],A=!1,q=0;q<b.length;q++)if(b[q].title==u){d={};d.title=u;d.y_axis="";d.order=e+1;d.hide=b[q].hide;d.plot_ci=b[q].plot_ci;d.show_signif=b[q].show_signif;d.color=b[q].color;d.pch=b[q].pch;d.type=b[q].type;d.lty=b[q].lty;d.lwd=b[q].lwd;d.con_series=b[q].con_series;d.legend=b[q].legend;d.id=e+1;A=
!0;break}A||(f&&0<e&&null==initXML?(d=jQuery.extend(!0,{},g[g.length-1]),d.title=u,d.y_axis=void 0,d.id=e+1,d.order=e+1):null!=initXML?(d={},d.title=u,d.y_axis=void 0,d.order=w[e]&&""!=w[e]?parseInt(w[e]):parseInt(q+1),d.hide=k[e],d.plot_ci=h[e].trim(),d.show_signif=m[e],d.pch=t[e].trim(),d.type=B[e].trim(),d.lty=r[e].trim(),d.lwd=y[e].trim(),d.con_series=v[e].trim(),d.legend=x[e].trim(),d.color=p[e].trim(),d.id=e+1):(d=jQuery.extend(!0,{},firstSeriesFormatting),d.title=u,d.y_axis=void 0,d.id=e+1,
...
function updateSeries(a){var b=$("#listdt");b.saveCell(lastSelRow,lastSelCol);var c=b.jqGrid("getRowData");b.jqGrid("clearGridData");group_name_to_value_map={};var e,d=0,f={},g=$("#seriesLock").is(":checked"),h=[],k=$("#plot_data").multiselect("getChecked").val(),m,p,t=[],r=[],y=[],v=[],w=[],x=[],B=[],q=[],l=[],u=[],A=[];if(null!=initXML){for(var t=initXML.find("plot").find("plot_ci").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),r=initXML.find("plot").find("plot_disp").text().replace("c(",
"").replace(")","").replace(/"/g,"").split(","),y=initXML.find("plot").find("show_signif").text().replace("c(","").replace(")","").replace(/"/g,"").split(","),n=0;n<r.length;n++)"TRUE"==r[n].trim()?r[n]="No":r[n]="Yes","TRUE"==y[n].trim()?y[n]="Yes":y[n]="No";v=initXML.find("plot").find("colors").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");for(n=0;n<v.length;n++)v[n]=v[n].replace(/FF$/,"");w=initXML.find("plot").find("pch").text().replace("c(","").replace(")","").replace(/"/g,
"").split(",");x=initXML.find("plot").find("lty").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");B=initXML.find("plot").find("lwd").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");q=initXML.find("plot").find("con_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");if((l=initXML.find("plot").find("order_series").text())&&0<l.length)l=initXML.find("plot").find("order_series").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");
else for(l=[],n=0;n<v.length;n++)l.push(n+1);A=initXML.find("plot").find("type").text().replace("c(","").replace(")","").replace(/"/g,"").split(",");u=initXML.find("plot").find("legend").text().replace("c(","").replace(/\)$/,"").replace(/"/g,"").split(",");if(u.length!=l.length)for(n=0;n<l.length;n++)u[n]=""}for(var z=1;2>=z;z++){m="y"+z;1==z?(p=fcst_var_y1_indexes,e=permuteSeries(createSeriesMapForPermutation(series_var_y1_indexes,m),0)):(p=fcst_var_y2_indexes,e=permuteSeries(createSeriesMapForPermutation(series_var_y2_indexes,
m),0));for(var E=0;E<p.length;E++){var J=$("#fcst_var_"+m+"_"+p[E]).val(),G=[],C;C=$("#fcst_stat_"+m+"_"+p[E]).multiselect("getChecked");0==C.length&&"mode"==k&&(C=$("#fcst_stat_mode_"+m+"_"+p[E]).multiselect("getChecked"));for(n=0;n<C.length;n++)G.push(C[n].value);for(C=0;C<G.length;C++)for(var H=0;H<e.length;H++){n=G[C];"mode"==k&&-1==listStatModelRatio.indexOf(n)&&(n=createStatNameForModeArrt(n,m,p[E]));for(var F=e[H]+" "+J+" "+n,I=!1,n=0;n<c.length;n++)if(c[n].title==F&&c[n].y_axis.match(z+"$")==
z){f={};f.title=F;f.y_axis="Y"+z;f.order=d+1;f.hide=c[n].hide;f.plot_ci=c[n].plot_ci;f.show_signif=c[n].show_signif;f.color=c[n].color;f.pch=c[n].pch;f.type=c[n].type;f.lty=c[n].lty;f.lwd=c[n].lwd;f.con_series=c[n].con_series;f.legend=c[n].legend;f.id=d+1;I=!0;break}I||(g&&0<d&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=F,f.y_axis="Y"+z,f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=F,f.y_axis="Y"+z,f.order=parseInt(l[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=y[d],f.pch=
w[d].trim(),f.type=A[d].trim(),f.lty=x[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=u[d].trim(),f.color=v[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=F,f.y_axis="Y"+z,f.id=d+1,f.order=d+1));d++;h.push(f)}}}for(var c=[],D,n=0;n<seriesDiffY1.length;n++){k=seriesDiffY1[n].split(",")[0];e=seriesDiffY1[n].split(",")[1];z=seriesDiffY1[n].split(",")[2];p=m=!1;D='"and"';"DIFF"===z?D='"-"':"RATIO"===z&&(D='"/"');for(f=0;f<h.length;f++)k==h[f].title&&(m=!0),e==
h[f].title&&(p=!0);m&&p&&(c.push(seriesDiffY1[n]),k=z+' ("'+k+D+e+'")',g&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=k,f.y_axis="Y1",f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=k,f.y_axis="Y1",f.order=parseInt(l[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=y[d].trim(),f.pch=w[d].trim(),f.type=A[d].trim(),f.lty=x[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=u[d].trim(),f.color=v[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=
k,f.y_axis="Y1",f.id=d+1,f.order=d+1),d++,h.push(f))}seriesDiffY1=c;c=[];for(n=0;n<seriesDiffY2.length;n++){k=seriesDiffY2[n].split(",")[0];e=seriesDiffY2[n].split(",")[1];z=seriesDiffY1[n].split(",")[2];p=m=!1;"DIFF"===z?D='"-"':"RATIO"===z&&(D='"/"');for(f=0;f<h.length;f++)k==h[f].title&&(m=!0),e==h[f].title&&(p=!0);m&&p&&(c.push(seriesDiffY2[n]),k=z+' ("'+k+D+e+'")',g&&null==initXML?(f=jQuery.extend(!0,{},h[h.length-1]),f.title=k,f.y_axis="Y2",f.id=d+1,f.order=d+1):null!=initXML?(f={},f.title=
k,f.y_axis="Y2",f.order=parseInt(l[d]),f.hide=r[d],f.plot_ci=t[d].trim(),f.show_signif=y[d],f.pch=w[d].trim(),f.type=A[d].trim(),f.lty=x[d].trim(),f.lwd=B[d].trim(),f.con_series=q[d].trim(),f.legend=u[d].trim(),f.color=v[d].trim(),f.id=d+1):(f=jQuery.extend(!0,{},firstSeriesFormatting),f.title=k,f.y_axis="Y2",f.id=d+1,f.order=d+1),d++,h.push(f))}seriesDiffY2=c;if(null==initXML){v=rainbow(h.length);d=[];if(a)d=v;else{for(n=0;n<v.length;n++)for(f=0;f<h.length;f++)h[f].color&&0<h[f].color.length&&h[f].color.substr(h[f].color.length-
6).toUpperCase()===v[n].substr(v[n].length-6).toUpperCase()&&n!=f&&(d[f]=v[n],d[n]=v[f]);for(n=0;n<v.length;n++)d[n]||(d[n]=v[n])}for(n=0;n<h.length;n++)h[n].color&&0!=h[n].color.length||(h[n].color=d[n])}h.sort(SortByOrder);for(n=0;n<h.length;n++)b.jqGrid("addRowData",n+1,h[n]);outerLayout.sizePane("south",$("#gbox_listdt").height())}function SortByOrder(a,b){return a.order-b.order}
...
p=[],t=[],r=[],y=[],e=0;e<b.length;e++)c.push('"'+b[e].plot_ci+'"'),"No"==b[e].hide?d.push("TRUE"):d.push("FALSE"),"No"==b[e].show_signif?y.push("FALSE"):y.push("TRUE"),f.push('"'+b[e].color+'FF"'),g.push(b[e].pch),h.push('"'+b[e].type+'"'),k.push(b[e].lty),m.push(b[e].lwd),p.push(b[e].con_series),t.push(b[e].order),r.push('"'+b[e].legend+'"');a.append($("<plot_ci />").text("c("+c.join()+")"));a.append($("<show_signif />").text("c("+y.join()+")"));a.append($("<plot_disp />").text("c("+d.join()+")"));
...
b);d.multiselect({selectedList:100,noneSelectedText:"Select value",position:{my:"center center",at:"right center"}});c.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",height:"auto",click:function(a,c){$("#fixed_var_val_date_period_start_"+b).empty();$("#fixed_var_val_date_period_end_"+b).empty();"fcst_init_beg"===c.value||"fcst_valid_beg"===c.value||"fcst_valid"===c.value||"fcst_init"===c.value?$("#fixed_var_val_date_period_button_"+b).css("display","block"):$("#fixed_var_val_date_period_button_"+
b).css("display","none");var d=this.id.split("_");updateFixedVarVal(d[d.length-1],[])}});f.button({icons:{primary:"ui-icon-check",secondary:"ui-icon-circlesmall-plus"},text:!1}).click(function(){$("#fixed_var_val_date_period_"+b).dialog("open")});updateFixedVarVal(b,[])}
...
createValDatePeriodDialog("series_var_val_y1",a+1);e.multiselect({selectedList:100,noneSelectedText:"Select value",click:function(){updateSeriesRhist()},checkAll:function(){updateSeriesRhist()},uncheckAll:function(){updateSeriesRhist()}});try{e.multiselect("uncheckAll")}catch(g){}b.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(b,c){$("#series_var_val_y1_date_period_start_"+(a+1)).empty();$("#series_var_val_y1_date_period_end_"+(a+1)).empty();"fcst_init_beg"==c.value||
...
createValDatePeriodDialog("series_var_val_"+a,b+1);d.multiselect({selectedList:100,noneSelectedText:"Select value",click:function(){updateSeries()},checkAll:function(){updateSeries(!0)},uncheckAll:function(){updateSeries()}});try{d.multiselect("uncheckAll")}catch(m){}c.multiselect({multiple:!1,selectedList:1,header:!1,minWidth:"auto",click:function(c,d){$("#series_var_val_"+a+"_date_period_start_"+(b+1)).empty();$("#series_var_val_"+a+"_date_period_end_"+(b+1)).empty();"fcst_init_beg"==d.value||"fcst_valid_beg"==
...
if("stat"===b)updateStats(g,h+1,d),h++;else if($.isArray(d))for(c=0;c<d.length;c++){if(0<c){addFcstVar(g);try{$("#fcst_var_"+g+"_"+(h+1)).val($(k[a]).attr("name")).multiselect("refresh")}catch(r){}}updateMode(g,h+1,d[c]);h++}else updateMode(g,h+1,d),h++}else"stat"===b?updateStats(g,1,""):updateMode(g,1,"");var m,p;if(0<initXML.find("plot").find("series"+f).children().length)for(h=initXML.find("plot").find("series"+f).children(),a=0;a<h.length;a++){m=[];p=!1;c=$(h[a]).attr("name");if(0!==a||1!==f)"Taylor"===
currentTab?addSeriesVarRhist():addSeriesVar(g);try{$("#series_var_"+g+"_"+(a+1)).val(c).multiselect("refresh")}catch(r){}$(h[a]).find("val").each(function(){var a=$(this).text().split(",");1<a.length&&(p=!0);for(var b=0;b<a.length;b++)m.push(a[b])});"fcst_init_beg"==c||"fcst_valid_beg"==c||"fcst_valid"==c||"fcst_init"==c?$("#series_var_val_"+g+"_date_period_button_"+(a+1)).css("display","block"):$("#series_var_val_"+g+"_date_period_button_"+(a+1)).css("display","none");updateSeriesVarVal(g,a+1,m);
p&&$("#group_series_var_"+g+"_"+(a+1)).prop("checked",!0)}else m=$("#series_var_"+g+"_1").first().val(),updateSeriesVarVal(g,1,m)}updatePlotFixSeries();$("#txtPlotCond").val(initXML.find("plot").find("plot_cond").text());"mode"===b&&updateIndyVar(b);c=$(initXML.find("plot").find("indep")[0]).attr("name");try{$("#indy_var").val(c).multiselect("refresh")}catch(r){}"fcst_init_beg"==c||"fcst_valid_beg"==c||"fcst_valid"==c||"fcst_init"==c?$("#date_period_button").css("display","block"):$("#date_period_button").css("display",
