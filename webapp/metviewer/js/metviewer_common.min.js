...
error:function(){},success:function(a){var b=$(a).find("val");a=[];if(0<b.length){for(var c=0;c<b.length;c++){var g=$(b[c]);var f=0<=$.inArray(g.text(),d);0==c||0!=c&&g.text()!==$(b[c-1]).text()?(f=$("<option />",{value:g.text(),text:g.text(),title:value_to_desc_map[g.text()],selected:f}),a.push(f)):0!=c&&g.text()===$(b[c-1]).text()&&a[a.length-1].text(a[a.length-1].text()+"*")}f=$("#database").multiselect("getChecked").length-1;for(c=0;c<a.length;c++)a[c].text()!==a[c][0].value&&(b=a[c].text().substring(a[c][0].value.length,
a[c].text().length).length,b===f?a[c].text(a[c][0].value+"*"):a[c].text(a[c][0].value)),a[c].appendTo(e)}else f=$("<option />",{value:"N/A",text:"N/A"}),f.appendTo(e);try{e.multiselect("refresh")}catch(y){}d=[]}})}var l=$("#series_var_"+a+"_"+b);l.empty();$.each(series_var_value_to_title_stat_map,function(a,b){l.append('<option value="'+a+'">'+b+"</option>")});try{l.multiselect("refresh")}catch(p){}}
...
"*"):c[f].text(c[f][0].value)),c[f].appendTo(d)}else p=$("<option />",{value:"N/A",text:"N/A"}),p.appendTo(d);try{d.multiselect("refresh")}catch(n){}b=[]}})}
...
f+"_field>"+h+"</"+f+"_field>"+g+"</list_val></request>",error:function(){},success:function(c){"y1"===a?seriesY1VarValResponse[b]=c:seriesY2VarValResponse[b]=c;var f=$(c).find("val");c=[];if(0<f.length){for(var g=0;g<f.length;g++){var l=$(f[g]);var h="string"==$.type(d)?l.text()==d:0<=$.inArray(l.text(),d);0==g||0!=g&&l.text()!==$(f[g-1]).text()?(h=$("<option />",{value:l.text(),text:l.text(),selected:h}),c.push(h)):0!=g&&l.text()===$(f[g-1]).text()&&c[c.length-1].text(c[c.length-1].text()+"*")}h=
$("#database").multiselect("getChecked").length-1;for(g=0;g<c.length;g++)c[g].text()!==c[g][0].value&&(f=c[g].text().substring(c[g][0].value.length,c[g].text().length).length,f===h?c[g].text(c[g][0].value+"*"):c[g].text(c[g][0].value)),c[g].appendTo(e)}else h=$("<option />",{value:"N/A",text:"N/A"}),h.appendTo(e);try{e.multiselect("refresh")}catch(z){}1===d.length&&-1!==d[0].indexOf(",")&&$("#group_series_var_"+a+"_"+b).prop("checked",!0);d=[]}})}
...
"Eclv"===currentTab&&(f="eclv");$.ajax({async:!1,url:"servlet",type:"POST",data:"<request><db_con>"+e+"</db_con><list_val><id>0</id><"+f+"_field>"+c+"</"+f+'_field><stat><fcst_var name="NA"><val>NA</val></fcst_var></stat></list_val></request>',error:function(){},success:function(c){seriesY1VarValResponse[a]=c;var e=$(c).find("val");c=[];if(0<e.length){for(var f=0;f<e.length;f++){var g=$(e[f]);var n=0<=$.inArray(g.text(),b);0==f||0!=f&&g.text()!==$(e[f-1]).text()?(n=$("<option />",{value:g.text(),
...
b?currentTab="Perf":"taylor_plot.R_tmpl"===b&&(currentTab="Taylor");b=$("#plot_config").find("a");for(var d=0;d<b.length;d++)if(b[d].text===currentTab){a=d;break}}$("#plot_config").tabs({active:a,beforeActivate:function(a,b){a=b.oldTab.attr("aria-controls");$("#"+a).empty();cleanUp();currentTab=b.newTab.text()},beforeLoad:function(a,b){b.jqXHR.error(function(){b.panel.html("Couldn't load this tab. We'll try to fix this as soon as possible. If this wouldn't be a demo.")})}});null!=initXML?($("#plot_title").val($(initXML.find("plot").find("tmpl").find("title")).text()),
