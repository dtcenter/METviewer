...
function updatePlotFix(){if(0<$(initXML.find("plot").find("plot_fix")).children().length)for(var a=$(initXML.find("plot").find("plot_fix")).children(),b=0;b<a.length;b++){var d=[],e=$(a[b]).attr("equalize");console.log(e);console.log(d);0<b&&addFixedVarHist();var c=$(a[b]).attr("name");try{$("#fixed_var_"+(b+1)).val(c).multiselect("refresh")}catch(f){}$(a[b]).find("set").find("val").each(function(){d.push($(this).text())});"fcst_init_beg"===c||"fcst_valid_beg"===c||"fcst_valid"===c||"fcst_init"===
c?$("#fixed_var_val_date_period_button_"+(b+1)).css("display","block"):$("#fixed_var_val_date_period_button_"+(b+1)).css("display","none");updateFixedVarValHist(b+1,d,e)}}
